...
entryUrl: "https://github.com/openai/openai-agents-python/blob/83c737fd0b8d9a53bd39fa2a0856070417bb0bd3/docs/zh/tools.md"
sourceRel: "docs/zh/tools.md"
rawUrl: "/raw/08-agents/openai-agents-python/docs/zh/tools.md"
sourceSha256: "5eceba040a529bd7e3f5e95d53970081f52793dcca7c01b3be457ca313434b73"
pageSha256: "5eceba040a529bd7e3f5e95d53970081f52793dcca7c01b3be457ca313434b73"
...
-   工具搜索活动会以专用的项目和事件类型出现在 [`RunResult.new_items`](/lib/08-agents/openai-agents-python/docs-zh-results#new-items) 和 [`RunItemStreamEvent`](/lib/08-agents/openai-agents-python/docs-zh-streaming#run-item-event-names) 中。
...
-   `program` 项及其普通的归程序所有的子工具调用会显示为 [`ToolCallItem`][agents.items.ToolCallItem] 条目。对应的 `program_output` 会显示为 [`ToolCallOutputItem`][agents.items.ToolCallOutputItem]。托管 MCP 审批请求和工具目录则使用专门的 MCP 项目和流事件。有关检查详情，请参阅[结果](/lib/08-agents/openai-agents-python/docs-zh-results#new-items)和[流式传输](/lib/08-agents/openai-agents-python/docs-zh-streaming#run-item-event-names)。
...
`Agent.as_tool(..., needs_approval=...)` 使用与 `function_tool` 相同的审批流程。如果需要审批，运行会暂停，待处理项目会出现在 `result.interruptions` 中；随后使用 `result.to_state()`，并在调用 `state.approve(...)` 或 `state.reject(...)` 后恢复运行。有关完整的暂停/恢复模式，请参阅[人在回路指南](/lib/08-agents/openai-agents-python/docs-zh-human_in_the_loop)。
...
在自定义提取器内部，嵌套的 [`RunResult`][agents.result.RunResult] 还会公开 [`agent_tool_invocation`][agents.result.RunResultBase.agent_tool_invocation]。当您需要在后处理嵌套结果时获取外层工具名称、调用 ID 或原始参数，这很有用。请参阅[结果指南](/lib/08-agents/openai-agents-python/docs-zh-results#agent-as-tool-metadata)。
...
对于本地配置的函数工具，Runner 还会在调用前重新求值 `is_enabled`。不过，`is_enabled` 控制可见性和分派；它不能取代基于工具参数或所访问资源的授权。请在工具实现内部强制执行这些检查，或在适当情况下使用[工具输入安全防护措施](/lib/08-agents/openai-agents-python/docs-zh-guardrails#tool-guardrails)和[审批](/lib/08-agents/openai-agents-python/docs-zh-human_in_the_loop)。MCP 服务器必须自行对其受保护操作进行授权。

有关将一项应用程序策略应用于函数工具、MCP 工具和任务转移的模式，请参阅[上下文管理](/lib/08-agents/openai-agents-python/docs-zh-context#use-local-context-for-capability-visibility)。
...
-   [Codex 工具 API 参考](https://github.com/openai/openai-agents-python/blob/83c737fd0b8d9a53bd39fa2a0856070417bb0bd3/docs/zh/ref/extensions/experimental/codex/codex_tool.md)
-   [ThreadOptions 参考](https://github.com/openai/openai-agents-python/blob/83c737fd0b8d9a53bd39fa2a0856070417bb0bd3/docs/zh/ref/extensions/experimental/codex/thread_options.md)
-   [TurnOptions 参考](https://github.com/openai/openai-agents-python/blob/83c737fd0b8d9a53bd39fa2a0856070417bb0bd3/docs/zh/ref/extensions/experimental/codex/turn_options.md)
