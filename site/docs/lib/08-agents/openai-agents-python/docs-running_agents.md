...
entryUrl: "https://github.com/openai/openai-agents-python/blob/83c737fd0b8d9a53bd39fa2a0856070417bb0bd3/docs/running_agents.md"
sourceRel: "docs/running_agents.md"
rawUrl: "/raw/08-agents/openai-agents-python/docs/running_agents.md"
sourceSha256: "4d6e806c61b34ea12a5f23cc98570219aea574f5d4bf8519fbf5188096d3baf3"
pageSha256: "4d6e806c61b34ea12a5f23cc98570219aea574f5d4bf8519fbf5188096d3baf3"
...
Read more in the [results guide](/lib/08-agents/openai-agents-python/docs-results).
...
-   a [`RunState`][agents.run_state.RunState] when resuming a paused run or a run stopped with `cancel(mode="after_turn")`. The state can also carry [input staged for the next resumed model call](/lib/08-agents/openai-agents-python/docs-results#add-input-before-resuming).
...
This is the Responses API over websocket transport, not the [Realtime API](/lib/08-agents/openai-agents-python/docs-realtime-guide).

For transport-selection rules and caveats around concrete model objects or custom providers, see [Models](/lib/08-agents/openai-agents-python/docs-models#responses-websocket-transport).
...
The service processes one response at a time on each websocket connection and limits a connection to 60 minutes. The helper reuses the connection but does not remove those constraints. After a reconnect, `store=False` and ZDR flows cannot recover an uncached `previous_response_id`; start a new chain with full input context or rebuild it from locally managed session state. See the [Responses WebSocket transport notes](/lib/08-agents/openai-agents-python/docs-models#responses-websocket-transport) for the full recovery behavior.
...
-   [`tracing_disabled`][agents.run.RunConfig.tracing_disabled]: Allows you to disable [tracing](/lib/08-agents/openai-agents-python/docs-tracing) for the entire run.
...
For a simpler approach, you can use [Sessions](/lib/08-agents/openai-agents-python/docs-sessions) to automatically handle conversation history without manually calling `.to_input_list()`:
...
See the [Sessions documentation](/lib/08-agents/openai-agents-python/docs-sessions) for more details.
...
!!! note
    ...
    broader opt-in retry behavior on model requests, see [Runner-managed retries](/lib/08-agents/openai-agents-python/docs-models#runner-managed-retries).
...
For tool approval pause/resume patterns, start with the dedicated [Human-in-the-loop guide](/lib/08-agents/openai-agents-python/docs-human_in_the_loop). The integrations below are for durable orchestration when runs may span long waits, retries, or process restarts.
...
-   [`ModelTimeoutError`][agents.exceptions.ModelTimeoutError]: This exception is raised when a model-call attempt exceeds [`ModelSettings.timeout`][agents.model_settings.ModelSettings.timeout]. See [Model-call timeouts](/lib/08-agents/openai-agents-python/docs-models#model-call-timeouts) for scope and retry behavior.
