...
entryUrl: "https://github.com/openai/openai-agents-python/blob/83c737fd0b8d9a53bd39fa2a0856070417bb0bd3/docs/agents.md"
sourceRel: "docs/agents.md"
rawUrl: "/raw/08-agents/openai-agents-python/docs/agents.md"
sourceSha256: "5393ce61e3e936c6adabfd741a31796143237286ca45220a2dbb8f03a8aeb006"
pageSha256: "5393ce61e3e936c6adabfd741a31796143237286ca45220a2dbb8f03a8aeb006"
...
Use this page when you want to define or customize a single base `Agent` rather than a `SandboxAgent`. If you are deciding how multiple agents should collaborate, read [Agent orchestration](/lib/08-agents/openai-agents-python/docs-multi_agent). If the agent should run inside an isolated workspace with manifest-defined files and sandbox-native capabilities, read [Sandbox agent concepts](/lib/08-agents/openai-agents-python/docs-sandbox-guide).
...
| --- | --- |
| Choose a model or provider setup | [Models](/lib/08-agents/openai-agents-python/docs-models) |
| Add capabilities to the agent | [Tools](/lib/08-agents/openai-agents-python/docs-tools) |
| Run an agent against a real repo, document bundle, or isolated workspace | [Sandbox agents quickstart](/lib/08-agents/openai-agents-python/docs-sandbox_agents) |
| Decide between manager-style orchestration and handoffs | [Agent orchestration](/lib/08-agents/openai-agents-python/docs-multi_agent) |
| Configure handoff behavior | [Handoffs](/lib/08-agents/openai-agents-python/docs-handoffs) |
| Run turns, stream events, or manage conversation state | [Running agents](/lib/08-agents/openai-agents-python/docs-running_agents) |
| Inspect final output, run items, or resumable state | [Results](/lib/08-agents/openai-agents-python/docs-results) |
| Share local dependencies and runtime state | [Context management](/lib/08-agents/openai-agents-python/docs-context) |
...
| `handoffs` | no | Delegate the conversation to specialist agents. See [handoffs](/lib/08-agents/openai-agents-python/docs-handoffs). |
| `model` | no | Which LLM to use. See [Models](/lib/08-agents/openai-agents-python/docs-models). |
| `model_settings` | no | Model tuning parameters such as `temperature`, `top_p`, and `tool_choice`. |
| `tools` | no | Tools the agent can call. See [Tools](/lib/08-agents/openai-agents-python/docs-tools). |
| `mcp_servers` | no | MCP servers that provide MCP-backed tools to the agent. See the [MCP guide](/lib/08-agents/openai-agents-python/docs-mcp). |
| `mcp_config` | no | Fine-tune how MCP tools are prepared, such as converting their schemas to strict mode and formatting MCP failures. See the [MCP guide](/lib/08-agents/openai-agents-python/docs-mcp#agent-level-mcp-configuration). |
| `input_guardrails` | no | Guardrails that run on the first user input for this agent chain. See [Guardrails](/lib/08-agents/openai-agents-python/docs-guardrails). |
| `output_guardrails` | no | Guardrails that run on the final output for this agent. See [Guardrails](/lib/08-agents/openai-agents-python/docs-guardrails). |
...
Everything in this section applies to `Agent`. `SandboxAgent` builds on the same ideas, then adds `default_manifest`, `base_instructions`, `capabilities`, and `run_as` for workspace-scoped runs. See [Sandbox agent concepts](/lib/08-agents/openai-agents-python/docs-sandbox-guide).
...
Read the [context guide](/lib/08-agents/openai-agents-python/docs-context) for the full `RunContextWrapper` surface, shared usage tracking, nested `tool_input`, and serialization caveats.
...
The `customer_facing_agent` handles all user interaction and invokes specialized sub‑agents exposed as tools. Read more in the [tools](/lib/08-agents/openai-agents-python/docs-tools#agents-as-tools) documentation.
...
For the full callback surface, see the [Lifecycle API reference](https://github.com/openai/openai-agents-python/blob/83c737fd0b8d9a53bd39fa2a0856070417bb0bd3/docs/ref/lifecycle.md).

## Guardrails

Guardrails allow you to run checks/validations on user input in parallel to the agent running, and on the agent's output once it is produced. For example, you could screen the user's input and agent's output for relevance. Read more in the [guardrails](/lib/08-agents/openai-agents-python/docs-guardrails) documentation.
