...
entryUrl: "https://github.com/openai/openai-agents-python/blob/83c737fd0b8d9a53bd39fa2a0856070417bb0bd3/docs/zh/agents.md"
sourceRel: "docs/zh/agents.md"
rawUrl: "/raw/08-agents/openai-agents-python/docs/zh/agents.md"
sourceSha256: "99edc591758a21b1369ad2583d77d02bd3e0d44850234e20c98f7f9b231d1616"
pageSha256: "99edc591758a21b1369ad2583d77d02bd3e0d44850234e20c98f7f9b231d1616"
...
如果你要定义或自定义单个基础 `Agent`，而不是 `SandboxAgent`，请使用本页面。如果你正在决定多个智能体应如何协作，请阅读[智能体编排](/lib/08-agents/openai-agents-python/docs-zh-multi_agent)。如果智能体应在具有清单定义文件和沙箱原生能力的隔离工作区中运行，请阅读[沙箱智能体概念](/lib/08-agents/openai-agents-python/docs-zh-sandbox-guide)。
...
| 如果你想要…… | 接下来阅读 |
| --- | --- |
| 选择模型或提供商设置 | [模型](/lib/08-agents/openai-agents-python/docs-zh-models) |
| 为智能体添加能力 | [工具](/lib/08-agents/openai-agents-python/docs-zh-tools) |
| 让智能体针对真实代码仓库、文档包或隔离工作区运行 | [沙箱智能体快速入门](/lib/08-agents/openai-agents-python/docs-zh-sandbox_agents) |
| 在管理器式编排与任务转移之间进行选择 | [智能体编排](/lib/08-agents/openai-agents-python/docs-zh-multi_agent) |
| 配置任务转移行为 | [任务转移](/lib/08-agents/openai-agents-python/docs-zh-handoffs) |
| 运行轮次、流式传输事件或管理对话状态 | [运行智能体](/lib/08-agents/openai-agents-python/docs-zh-running_agents) |
| 检查最终输出、运行项或可恢复状态 | [结果](/lib/08-agents/openai-agents-python/docs-zh-results) |
| 共享本地依赖项和运行时状态 | [上下文管理](/lib/08-agents/openai-agents-python/docs-zh-context) |
...
| `handoffs` | 否 | 将对话委派给专业智能体。请参阅[任务转移](/lib/08-agents/openai-agents-python/docs-zh-handoffs)。 |
| `model` | 否 | 要使用的 LLM。请参阅[模型](/lib/08-agents/openai-agents-python/docs-zh-models)。 |
| `model_settings` | 否 | 模型调优参数，例如 `temperature`、`top_p` 和 `tool_choice`。 |
| `tools` | 否 | 智能体可以调用的工具。请参阅[工具](/lib/08-agents/openai-agents-python/docs-zh-tools)。 |
| `mcp_servers` | 否 | 为智能体提供基于 MCP 的工具的 MCP 服务器。请参阅 [MCP 指南](/lib/08-agents/openai-agents-python/docs-zh-mcp)。 |
| `mcp_config` | 否 | 微调 MCP 工具的准备方式，例如将其 schema 转换为严格模式，以及设置 MCP 失败信息的格式。请参阅 [MCP 指南](/lib/08-agents/openai-agents-python/docs-zh-mcp#agent-level-mcp-configuration)。 |
| `input_guardrails` | 否 | 针对此智能体链的第一个用户输入运行的安全防护措施。请参阅[安全防护措施](/lib/08-agents/openai-agents-python/docs-zh-guardrails)。 |
| `output_guardrails` | 否 | 针对此智能体的最终输出运行的安全防护措施。请参阅[安全防护措施](/lib/08-agents/openai-agents-python/docs-zh-guardrails)。 |
...
本节中的所有内容均适用于 `Agent`。`SandboxAgent` 基于相同理念构建，并额外添加了 `default_manifest`、`base_instructions`、`capabilities` 和 `run_as`，用于工作区作用域内的运行。请参阅[沙箱智能体概念](/lib/08-agents/openai-agents-python/docs-zh-sandbox-guide)。
...
有关完整的 `RunContextWrapper` 接口、共享用量追踪、嵌套的 `tool_input` 以及序列化注意事项，请阅读[上下文指南](/lib/08-agents/openai-agents-python/docs-zh-context)。
...
`customer_facing_agent` 负责处理所有用户交互，并调用作为工具公开的专业子智能体。请在[工具](/lib/08-agents/openai-agents-python/docs-zh-tools#agents-as-tools)文档中了解更多信息。
...
配置的任务转移目标是智能体可以委派任务的子智能体。发生任务转移时，被委派的智能体会接收对话历史记录并接管对话。此模式支持模块化的专业智能体，使其能够出色完成单一任务。请在[任务转移](/lib/08-agents/openai-agents-python/docs-zh-handoffs)文档中了解更多信息。
...
有关完整的回调接口，请参阅[生命周期 API 参考](https://github.com/openai/openai-agents-python/blob/83c737fd0b8d9a53bd39fa2a0856070417bb0bd3/docs/zh/ref/lifecycle.md)。

## 安全防护措施 \{#guardrails\}

安全防护措施允许你在智能体运行的同时并行检查/验证用户输入，并在智能体生成输出后对其进行检查/验证。例如，你可以筛查用户输入和智能体输出是否与任务相关。请在[安全防护措施](/lib/08-agents/openai-agents-python/docs-zh-guardrails)文档中了解更多信息。
...
使用 OpenAI Responses 工具搜索时，具名工具选项受到更多限制：不能通过 `tool_choice` 指定纯命名空间名称或仅延迟加载的工具，且 `tool_choice="tool_search"` 不会指定 [`ToolSearchTool`][agents.tool.ToolSearchTool]。在这些情况下，建议使用 `auto` 或 `required`。有关 Responses 特有的限制，请参阅[托管工具搜索](/lib/08-agents/openai-agents-python/docs-zh-tools#hosted-tool-search)。
