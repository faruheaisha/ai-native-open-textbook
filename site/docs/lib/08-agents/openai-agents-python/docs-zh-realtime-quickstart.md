...
entryUrl: "https://github.com/openai/openai-agents-python/blob/83c737fd0b8d9a53bd39fa2a0856070417bb0bd3/docs/zh/realtime/quickstart.md"
sourceRel: "docs/zh/realtime/quickstart.md"
rawUrl: "/raw/08-agents/openai-agents-python/docs/zh/realtime/quickstart.md"
sourceSha256: "1b6eced8bfbd02149ebf8baaf3ce0ca07d7ef4ab33a04574b9db54403f2c30df"
pageSha256: "1b6eced8bfbd02149ebf8baaf3ce0ca07d7ef4ab33a04574b9db54403f2c30df"
...
!!! note "Python SDK 的适用边界"
    ...
    Python SDK **不**提供浏览器 WebRTC 传输。本页仅介绍通过服务端 WebSocket、由 Python 管理的实时会话。此 SDK 适用于服务端编排、工具、审批和电话集成。另请参阅[实时传输](/lib/08-agents/openai-agents-python/docs-zh-realtime-transport)。
...
-   SIP / 电话接入流程。请参阅[实时传输](/lib/08-agents/openai-agents-python/docs-zh-realtime-transport)和 [SIP 部分](/lib/08-agents/openai-agents-python/docs-zh-realtime-guide#sip-and-telephony)。
...
对于手动轮次控制，请使用[实时智能体指南](/lib/08-agents/openai-agents-python/docs-zh-realtime-guide#manual-response-control)中介绍的底层 `session.update` / `input_audio_buffer.commit` / `response.create` 流程。
...
连接 Azure OpenAI 时，请将 `model_config["url"]` 设置为正式发布版 Realtime 端点 URL，并显式传入标头。使用实时智能体时，请避免使用旧版 beta 路径（`/openai/realtime?api-version=...`）。有关详细信息，请参阅[实时智能体指南](/lib/08-agents/openai-agents-python/docs-zh-realtime-guide#low-level-access-and-custom-endpoints)。

## 后续步骤 \{#next-steps\}

-   阅读[实时传输](/lib/08-agents/openai-agents-python/docs-zh-realtime-transport)，以便在服务端 WebSocket 和 SIP 之间进行选择。
-   阅读[实时智能体指南](/lib/08-agents/openai-agents-python/docs-zh-realtime-guide)，了解生命周期、结构化输入、审批、任务转移、安全防护措施和底层控制。
