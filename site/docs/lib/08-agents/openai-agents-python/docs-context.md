...
-   An MCP [`tool_filter`](/lib/08-agents/openai-agents-python/docs-mcp#dynamic-tool-filtering) receives a [`ToolFilterContext`][agents.mcp.ToolFilterContext], whose `run_context` property contains the current `RunContextWrapper`.

Adapt the shared application policy to these callbacks instead of maintaining separate capability lists. The callbacks control which capabilities the SDK exposes for the current run; they cannot authorize a model-generated argument or resource selection. For function tools, enforce those decisions inside the tool implementation or with [tool input guardrails](/lib/08-agents/openai-agents-python/docs-guardrails#tool-guardrails) and [approvals](/lib/08-agents/openai-agents-python/docs-human_in_the_loop) when appropriate. MCP servers must authorize their own protected operations. For a handoff with `input_type`, check the parsed input at the start of `on_handoff`, before application side effects, and raise instead of returning when authorization fails. Tool input guardrails do not run for handoffs. See [handoff inputs](/lib/08-agents/openai-agents-python/docs-handoffs#handoff-inputs) for the callback lifecycle.
