...
entryUrl: "https://github.com/openai/openai-agents-python/blob/83c737fd0b8d9a53bd39fa2a0856070417bb0bd3/docs/zh/running_agents.md"
sourceRel: "docs/zh/running_agents.md"
rawUrl: "/raw/08-agents/openai-agents-python/docs/zh/running_agents.md"
sourceSha256: "9eeb3e92189f2b000683225e8b995c62550a8c11e3d1aeb4aa5e64664ad350ae"
pageSha256: "9eeb3e92189f2b000683225e8b995c62550a8c11e3d1aeb4aa5e64664ad350ae"
...
请参阅[结果指南](/lib/08-agents/openai-agents-python/docs-zh-results)了解更多信息。
...
-   恢复暂停的运行或因 `cancel(mode="after_turn")` 而停止的运行时使用的 [`RunState`][agents.run_state.RunState]。状态还可以携带[为下次恢复后的模型调用暂存的输入](/lib/08-agents/openai-agents-python/docs-zh-results#add-input-before-resuming)。
...
借助流式传输，你还可以在 LLM 运行时接收流式事件。流结束后，[`RunResultStreaming`][agents.result.RunResultStreaming] 将包含此次运行的完整信息，包括生成的所有新输出。你可以调用 `.stream_events()` 获取流式事件。请参阅[流式传输指南](/lib/08-agents/openai-agents-python/docs-zh-streaming)了解更多信息。
...
这是通过 websocket 传输运行的 Responses API，而不是 [Realtime API](/lib/08-agents/openai-agents-python/docs-zh-realtime-guide)。

有关传输方式的选择规则，以及使用具体模型对象或自定义提供商时的注意事项，请参阅[模型](/lib/08-agents/openai-agents-python/docs-zh-models#responses-websocket-transport)。
...
该服务在每条 websocket 连接上一次处理一个响应，并将每条连接限制为 60 分钟。此辅助工具会复用连接，但不会消除这些限制。重新连接后，`store=False` 和 ZDR 流程无法恢复未缓存的 `previous_response_id`；请使用完整的输入上下文开始新的链，或根据本地管理的会话状态重建该链。有关完整的恢复行为，请参阅 [Responses WebSocket 传输说明](/lib/08-agents/openai-agents-python/docs-zh-models#responses-websocket-transport)。
...
-   [`tracing_disabled`][agents.run.RunConfig.tracing_disabled]：允许为整个运行禁用[追踪](/lib/08-agents/openai-agents-python/docs-zh-tracing)。
...
若要采用更简单的方式，可以使用 [Sessions](/lib/08-agents/openai-agents-python/docs-zh-sessions-2) 自动处理对话历史记录，而无需手动调用 `.to_input_list()`：
...
有关更多详细信息，请参阅 [Sessions 文档](/lib/08-agents/openai-agents-python/docs-zh-sessions-2)。
...
!!! note
    ...
    更广泛的选择启用重试行为，请参阅 [Runner 管理的重试](/lib/08-agents/openai-agents-python/docs-zh-models#runner-managed-retries)。
...
有关工具审批的暂停/恢复模式，请先参阅专门的[人在回路指南](/lib/08-agents/openai-agents-python/docs-zh-human_in_the_loop)。以下集成适用于运行可能经历长时间等待、重试或进程重启的持久编排。
...
-   [`ModelTimeoutError`][agents.exceptions.ModelTimeoutError]：当某次模型调用尝试超过 [`ModelSettings.timeout`][agents.model_settings.ModelSettings.timeout] 时，会引发此异常。有关作用范围和重试行为，请参阅[模型调用超时](/lib/08-agents/openai-agents-python/docs-zh-models#model-call-timeouts)。
