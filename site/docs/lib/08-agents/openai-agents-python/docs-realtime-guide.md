...
!!! note "Start here"
    ...
    If you want the default Python path, read the [quickstart](/lib/08-agents/openai-agents-python/docs-realtime-quickstart) first. If you are deciding whether your app should use server-side WebSocket or SIP, read [Realtime transport](/lib/08-agents/openai-agents-python/docs-realtime-transport). Browser WebRTC transport is not part of the Python SDK.
...
For a concrete server-side approval loop, see [`examples/realtime/app/server.py`](https://github.com/openai/openai-agents-python/tree/main/examples/realtime/app/server.py). The human-in-the-loop docs also point back to this flow in [Human in the loop](/lib/08-agents/openai-agents-python/docs-human_in_the_loop).
