...
entryUrl: "https://github.com/openai/openai-agents-python/blob/83c737fd0b8d9a53bd39fa2a0856070417bb0bd3/docs/tools.md"
sourceRel: "docs/tools.md"
rawUrl: "/raw/08-agents/openai-agents-python/docs/tools.md"
sourceSha256: "ba5179b66e7daf4cd315cd0993a8f634fdcb6000ec0a1ef26689608b3fb5fd17"
pageSha256: "ba5179b66e7daf4cd315cd0993a8f634fdcb6000ec0a1ef26689608b3fb5fd17"
...
-   Tool search activity appears in [`RunResult.new_items`](/lib/08-agents/openai-agents-python/docs-results#new-items) and in [`RunItemStreamEvent`](/lib/08-agents/openai-agents-python/docs-streaming#run-item-event-names) with dedicated item and event types.
...
-   A `program` item and its ordinary program-owned child tool calls appear as [`ToolCallItem`][agents.items.ToolCallItem] entries. The matching `program_output` appears as a [`ToolCallOutputItem`][agents.items.ToolCallOutputItem]. Hosted MCP approval requests and tool catalogs use specialized MCP items and stream events instead. See [Results](/lib/08-agents/openai-agents-python/docs-results#new-items) and [Streaming](/lib/08-agents/openai-agents-python/docs-streaming#run-item-event-names) for inspection details.
...
For locally configured function tools, the runner also reevaluates `is_enabled` before invocation. However, `is_enabled` controls visibility and dispatch; it does not replace authorization that depends on the tool arguments or the resource being accessed. Enforce those checks inside the tool implementation, or use [tool input guardrails](/lib/08-agents/openai-agents-python/docs-guardrails#tool-guardrails) and [approvals](/lib/08-agents/openai-agents-python/docs-human_in_the_loop) when appropriate. MCP servers must authorize their own protected operations.

See [context management](/lib/08-agents/openai-agents-python/docs-context#use-local-context-for-capability-visibility) for a pattern that applies one application policy across function tools, MCP tools, and handoffs.
...
-   [Codex tool API reference](https://github.com/openai/openai-agents-python/blob/83c737fd0b8d9a53bd39fa2a0856070417bb0bd3/docs/ref/extensions/experimental/codex/codex_tool.md)
-   [ThreadOptions reference](/lib/08-agents/openai-agents-python/docs-ref-extensions-experimental-codex-thread_options)
-   [TurnOptions reference](/lib/08-agents/openai-agents-python/docs-ref-extensions-experimental-codex-turn_options)
