...
entryUrl: "https://github.com/openai/openai-agents-python/blob/83c737fd0b8d9a53bd39fa2a0856070417bb0bd3/docs/zh/sandbox/guide.md"
sourceRel: "docs/zh/sandbox/guide.md"
rawUrl: "/raw/08-agents/openai-agents-python/docs/zh/sandbox/guide.md"
sourceSha256: "849a74dc8f408517f2d0f1188386f5b53e995bccf16bcacef40e2d0a7bd80d14"
pageSha256: "849a74dc8f408517f2d0f1188386f5b53e995bccf16bcacef40e2d0a7bd80d14"
...
![带计算环境的沙箱智能体运行框架](https://gh-proxy.com/https://raw.githubusercontent.com/openai/openai-agents-python/83c737fd0b8d9a53bd39fa2a0856070417bb0bd3/docs/assets/images/harness_with_compute.png)
...
在本页中，“沙箱会话”指由沙箱客户端管理的实时执行环境。它不同于[会话](/lib/08-agents/openai-agents-python/docs-zh-sessions-2)中介绍的 SDK 对话式 [`Session`][agents.memory.session.Session] 接口。
...
如果 shell 访问只是您偶尔使用的一项工具，请先参阅[工具指南](/lib/08-agents/openai-agents-python/docs-zh-tools)中的托管 shell。当工作区隔离、沙箱客户端选择或沙箱会话恢复行为属于设计的一部分时，再使用沙箱智能体。
...
在大多数情况下，`SandboxAgent` 定义保持不变，只需在 [`SandboxRunConfig`][agents.run_config.SandboxRunConfig] 中更改沙箱客户端及其选项。有关本地、Docker、托管和远程挂载选项，请参阅[沙箱客户端](/lib/08-agents/openai-agents-python/docs-zh-sandbox-clients)。
...
挂载条目描述要公开的存储；挂载策略描述沙箱后端如何附加该存储。有关挂载选项和提供商支持，请参阅[沙箱客户端](/lib/08-agents/openai-agents-python/docs-zh-sandbox-clients#mounts-and-remote-storage)。
...
保持智能体定义不变，只更改运行配置。如果需要容器隔离或镜像一致性，请使用 Docker；如果需要由提供商管理执行，请使用托管提供商。有关代码示例和提供商选项，请参阅[沙箱客户端](/lib/08-agents/openai-agents-python/docs-zh-sandbox-clients)。
...
有关设置、读取/生成行为、多轮对话和布局隔离，请参阅[智能体记忆](/lib/08-agents/openai-agents-python/docs-zh-sandbox-memory)。
...
- [任务转移](/lib/08-agents/openai-agents-python/docs-zh-handoffs)：将文档密集型工作从非沙箱接收智能体转交给沙箱审核智能体。
- [Agents as tools](/lib/08-agents/openai-agents-python/docs-zh-tools#agents-as-tools)：将多个沙箱智能体公开为工具，通常在每次 `Agent.as_tool(...)` 调用时传入 `run_config=RunConfig(sandbox=SandboxRunConfig(...))`，使每个工具拥有自己的沙箱边界。
- [MCP](/lib/08-agents/openai-agents-python/docs-zh-mcp) 和普通函数工具：沙箱功能可与 `mcp_servers` 和普通 Python 工具共存。
- [运行智能体](/lib/08-agents/openai-agents-python/docs-zh-running_agents)：沙箱运行仍使用常规的 `Runner` API。
...
- [快速入门](/lib/08-agents/openai-agents-python/docs-zh-sandbox_agents)：运行一个沙箱智能体。
- [沙箱客户端](/lib/08-agents/openai-agents-python/docs-zh-sandbox-clients)：选择本地、Docker、托管和挂载选项。
- [智能体记忆](/lib/08-agents/openai-agents-python/docs-zh-sandbox-memory)：保留并复用之前沙箱运行中的经验。
