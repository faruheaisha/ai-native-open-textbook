...
entryUrl: "https://github.com/openai/openai-agents-python/blob/83c737fd0b8d9a53bd39fa2a0856070417bb0bd3/docs/results.md"
sourceRel: "docs/results.md"
rawUrl: "/raw/08-agents/openai-agents-python/docs/results.md"
sourceSha256: "f63078dedb566c3881f00d4011263e62823cd1128e48ba54095492273f797093"
pageSha256: "f63078dedb566c3881f00d4011263e62823cd1128e48ba54095492273f797093"
...
In streaming mode, `final_output` stays `None` until the stream has finished processing. See [Streaming](/lib/08-agents/openai-agents-python/docs-streaming) for the event-by-event flow.
...
-   Use [`session=...`](/lib/08-agents/openai-agents-python/docs-sessions) when you want the SDK to load and save history for you.
...
For streaming runs, finish consuming [`stream_events()`][agents.result.RunResultStreaming.stream_events] first, then inspect `result.interruptions` and resume from `result.to_state()`. For the full approval flow, see [Human-in-the-loop](/lib/08-agents/openai-agents-python/docs-human_in_the_loop).
...
This is especially useful inside `custom_output_extractor`, where you may need the enclosing `Agent.as_tool()` call's tool name, call ID, or raw arguments while post-processing the nested result. See [Tools](/lib/08-agents/openai-agents-python/docs-tools) for the surrounding `Agent.as_tool()` patterns.
...
-   [`raw_usage`][agents.items.ModelResponse.raw_usage] is an opt-in, JSON-compatible snapshot of the provider's usage payload before the Agents SDK normalizes the payload. Enable `raw_usage` with `ModelSettings(preserve_raw_usage=True)`; see [Preserving provider usage payloads](/lib/08-agents/openai-agents-python/docs-usage#preserving-provider-usage-payloads).
...
Usage is tracked on `context_wrapper.usage`. For streamed runs, the usage totals can lag until the stream's final chunks have been processed. See [Context management](/lib/08-agents/openai-agents-python/docs-context) for the full wrapper shape and persistence caveats.
