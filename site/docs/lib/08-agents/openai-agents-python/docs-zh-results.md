...
entryUrl: "https://github.com/openai/openai-agents-python/blob/83c737fd0b8d9a53bd39fa2a0856070417bb0bd3/docs/zh/results.md"
sourceRel: "docs/zh/results.md"
rawUrl: "/raw/08-agents/openai-agents-python/docs/zh/results.md"
sourceSha256: "60300a61b1b60a40c70808d6d6c640a0ac5cca66b2a80c1a65d037d3c40fb7de"
pageSha256: "60300a61b1b60a40c70808d6d6c640a0ac5cca66b2a80c1a65d037d3c40fb7de"
...
在流式传输模式下，`final_output` 会一直保持为 `None`，直到流处理完成。有关逐事件的流程，请参阅[流式传输](/lib/08-agents/openai-agents-python/docs-zh-streaming)。
...
- 如果希望 SDK 为你加载和保存历史记录，请使用 [`session=...`](/lib/08-agents/openai-agents-python/docs-zh-sessions-2)。
...
对于流式运行，请先完成对 [`stream_events()`][agents.result.RunResultStreaming.stream_events] 的消费，然后检查 `result.interruptions` 并从 `result.to_state()` 恢复。有关完整审批流程，请参阅[人在回路](/lib/08-agents/openai-agents-python/docs-zh-human_in_the_loop)。
...
这在 `custom_output_extractor` 内尤其有用，因为在对嵌套结果进行后处理时，你可能需要外层 `Agent.as_tool()` 调用的工具名称、调用 ID 或原始参数。有关相关的 `Agent.as_tool()` 模式，请参阅[工具](/lib/08-agents/openai-agents-python/docs-zh-tools)。
...
- [`raw_usage`][agents.items.ModelResponse.raw_usage] 是可选启用、与 JSON 兼容的提供商用量载荷快照，采集于 Agents SDK 规范化该载荷之前。使用 `ModelSettings(preserve_raw_usage=True)` 启用 `raw_usage`；请参阅[保留提供商用量载荷](/lib/08-agents/openai-agents-python/docs-zh-usage#preserving-provider-usage-payloads)。
...
用量在 `context_wrapper.usage` 上追踪。对于流式运行，在处理完流的最终数据块之前，用量总计可能会有所延迟。有关完整的包装器结构和持久化注意事项，请参阅[上下文管理](/lib/08-agents/openai-agents-python/docs-zh-context)。
