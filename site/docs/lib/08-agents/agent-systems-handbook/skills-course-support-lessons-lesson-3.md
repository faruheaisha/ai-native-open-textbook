...
Follow the [Build quickstart](/lib/08-agents/agent-systems-handbook/skills-web-builder-README), then the [Test quickstart](/lib/08-agents/agent-systems-handbook/skills-webapp-testing-README). Inspect the generated page and both PNGs. Expect a basic build pass, two viewport results and a visible synthetic form confirmation. The form does not send a message or create a signup.
...
4. Minutes 18–25: use the preprovisioned Git project, record the committed build and test it, then review the actual preview deployment with [Deploy](/lib/08-agents/agent-systems-handbook/skills-vercel-deploy-README). Compare provider id/target/commit/READY and page marker to the canonical record.
