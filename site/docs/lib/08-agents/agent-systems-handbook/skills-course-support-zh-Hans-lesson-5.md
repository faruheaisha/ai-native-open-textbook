...
运行共享安装器 `--lesson 5`，使用独立 workspace 和虚构内容。按 [Plan](/lib/08-agents/agent-systems-handbook/skills-content-strategy-README) 预览/保存四个主题的策略，再按 [Distribute](/lib/08-agents/agent-systems-handbook/skills-prompthon-social-campaign-manager-README) 生成两个帖子预览。预期 `canonical_social_objects_created: false`，没有创建真实 campaign、post 或 schedule。Windows 等缺少时区库的环境需安装 tzdata；可选 Node 测试拦截所有请求，不连接外部平台。
