---
title: "BabyAgent - 后端工程师的 AI Agent 教学项目 (Go 语言版)"
landing: true
tier: 2
sourceId: "08-agents/baby-agent"
sourceTitle: "BabyAgent - 后端工程师的 AI Agent 教学项目 (Go 语言版)"
sourceKind: "课时教程"
licenseLabel: "可转载"
lang: "中文"
volume: "08-agents"
sourceUrl: "https://github.com/baby-llm/baby-agent"
entryUrl: "https://github.com/baby-llm/baby-agent/blob/55712911ad0c3d1554c94198370b08d9076fac1c/README.md"
zh: ""
---

# BabyAgent - 后端工程师的 AI Agent 教学项目 (Go 语言版)

🏠 **项目地址：** https://github.com/baby-llm/baby-agent

## 课时

1. [第一章：初识 LLM（Raw HTTP 与 OpenAI SDK）](ch01.md)
2. [第二章：赋予 AI “手脚”（Tool Calling 和 Agent）](ch02.md)
3. [第三章：让 Agent “更能看见”（Reasoning 展示、TUI）](ch03.md)
4. [第四章：让 Agent 接入 MCP 生态](ch04.md)
5. [第五章：上下文工程（Context Engineering）](ch05.md)
6. [第六章：记忆机制（Memory System）](ch06.md)
7. [第七章：Agentic RAG（检索增强生成）](ch07.md)
8. [第八章：沙盒与安全防御（Guardrails）](ch08.md)
9. [第九章：Agent 技能插件（Skills）](ch09.md)
10. [第十章：Web 服务化与 SSE 流式传输](ch10.md)
11. [第十一章：Agent 可观测性（Observability）](ch11.md)
12. [React + Vite](frontend.md)

开始学习 → [第一章：初识 LLM（Raw HTTP 与 OpenAI SDK）](ch01.md)
