...
![Coze Studio 本地部署四阶段流程总览图](/mirror/49/49bcd9b585f1f211d1ab8e123c94755994640e06.webp)
...
![在 Docker Desktop 终端中克隆 Coze Studio 仓库的界面](/mirror/64/64cc4af8fb7f9879e8216efbf0350eb692a015b3.webp)
...
![在 Coze Studio 根目录地址栏输入 cmd 的界面](/mirror/9f/9fc9128c020c17baf22195ddb68c11e280b9e681.webp)
...
![火山引擎控制台首页界面](/mirror/ef/ef5b3e7437f5763e15fb7e1c75b059e59ac43e27.webp)

搜索并进入“火山方舟”，在“API Key 管理”中点击“创建 API Key”。

![火山方舟创建 API Key 的界面](/mirror/85/85a7d8a31bfd95b43471872dba158c58e9c65710.webp)

点击小眼睛查看 API Key，并复制备用。

![火山方舟查看 API Key 的界面](/mirror/3a/3a183a2ef2344b7e4ea17bf415e17f96d016507b.webp)

**第二步：创建 Endpoint**

进入“在线推理”页面，选择“自定义推理接入点”，点击“创建推理接入点”。

![火山方舟创建推理接入点的界面](/mirror/69/696066de53264aaaebe1312f2ff7dff68d005059.webp)

输入接入点名称，建议按模型命名，然后点击“添加模型”。

![火山方舟设置推理接入点名称并添加模型的界面](/mirror/52/52ce4120feb609dc724d407dee0a733da6f06231.webp)

模型可选择豆包、DeepSeek、Kimi、Qwen 等。这里以豆包 1.6 为例。

![火山方舟选择豆包模型接入点的界面](/mirror/67/670999e196c538a3b42ce9cb32f0d148ac7d6e57.webp)

也可以从另一个入口添加模型：

![火山方舟添加模型的备选入口界面](/mirror/ef/efa8a6e098b616b84da76072cc21e85b466604f2.webp)

勾选协议，点击“开通模型并接入”。

![火山方舟勾选协议并开通模型接入的界面](/mirror/ff/ffaa1c4ad14ea911bb7c7e1b9081d28e1e71ba61.webp)

如果是首次开通模型，平台可能会要求实名认证，按页面提示完成即可。

创建完成后复制 Endpoint ID。注意：模型名称下方的 ID 就是后面要填入配置文件的 Endpoint。

![火山方舟复制 Endpoint ID 的界面](/mirror/bc/bcccc801aa7b4411c86c5889984946ff57197ff0.webp)
...
![编辑 Coze Studio 模型配置文件的界面](/mirror/b8/b803737d8311719d253a8f04936f857091d1be1c.webp)

再把 API Key 和 Endpoint 写入对应位置，保存并关闭文件。

![在 Coze Studio 模型配置文件中填写 API Key 和 Endpoint 的界面](/mirror/b0/b072586556137029dcb2e161708c285568b06d05.webp)
...
![本地安装 Ollama 并准备拉取模型的界面](/mirror/06/06ab8af8c970e1f5111c8edffe1aa0d28aa3a84d.webp)

然后在 Coze Studio 项目中找到 Ollama 模型配置模板 `model_template_ollama.yaml`，复制并重命名为 `model_ollama.yaml`，放到 `backend/conf/model/` 目录下。

![复制 Ollama 模型配置模板文件的界面](/mirror/0e/0ee4c257f624a8e5d0f484baa5303978bb002c74.webp)
...
![修改 Ollama 模型配置文件 base_url 和 model 的界面](/mirror/e1/e11a529c6583d9ce28e21fc5efec66a13998f2ac.webp)

### 2.3 启动 Coze Studio

模型配置完成后，进入项目中的 `docker` 目录。

![Coze Studio 项目中 docker 目录位置的界面](/mirror/1a/1ae833415f8292b26deb43aefd28fec661024cb0.webp)

先在终端输入 `docker` 并回车。如果能看到类似帮助信息，说明 Docker 命令可用。

![在终端中验证 Docker 是否安装成功的界面](/mirror/1f/1fb0b9a41feb8f70f48f1a6c4a87f4cb44a8f346.webp)
...
![Coze Studio 启动成功后的日志界面](/mirror/0c/0c8f2fe18941f4e19e155aeaf2adb7005757dfc6.webp)
...
![查看 Coze Studio 容器运行状态的界面](/mirror/0d/0da1b1cd8010858fe42995e52475dce28e058f8d.webp)
...
![浏览器访问 Coze Studio 时的登录界面](/mirror/7c/7c03e3dd07a6705bab1581adae9eb0aef37abe48.webp)

登录后即可进入 Coze Studio 主界面。

![登录后的 Coze Studio 主界面](/mirror/21/21ef1f85b2cebf066a9726f9575c8cde3fe83971.webp)
...
![修改 Dify .env 端口配置的界面](/mirror/29/293707474d2ad0217a99f7f61be7c6e1e2a3cc84.webp)
...
![在终端中进入 Dify docker 目录的界面](/mirror/93/93db2f46b0de75d6f6b62fff67f6343ae42a85bf.webp)
...
![执行 docker compose up -d 启动 Dify 的界面](/mirror/59/59b3a1d876683eb221e17666c260bd638946fafd.webp)

首次启动时，它会自动拉取镜像、创建容器并初始化服务，这个过程可能比较慢。

安装完成后：

![Dify 首次安装完成后的命令行界面](/mirror/05/0590b4d16b288341c041ee6935f6c53a67258933.webp)

后续再次执行 `docker compose up -d`，通常就是启动已有服务。

也可以在 Docker Desktop 的 Containers 页面查看当前运行的容器。

![Docker Desktop 中查看 Dify 容器运行状态的界面](/mirror/ac/ac5837c7d433c53fc58d1e1200bffdaa0bdf4358.webp)
...
![Dify 首次访问时设置管理员账户的界面](/mirror/ff/ffee80c0aff58fa6c156c0b405199ba38964a113.webp)
...
![Coze Loop 产品介绍界面](/mirror/eb/eb9b91eda8d0dc738dfcbc936491fd77a2cb1c2b.webp)
...
![Go 语言官网下载页面](/mirror/d4/d431ea06816b89d9b70a62d820a611a8d2bea398.webp)

下载完成后双击运行安装程序。

![Go 安装程序启动界面](/mirror/3e/3ecc9391c570c4a35ebfa36731f71c98ee6fac27.webp)

按提示一路点击“Next”即可。

安装完成后，确保 Go 安装目录下的 `bin` 目录，或 `GOPATH/bin`，已加入环境变量 PATH。

![Go 安装完成后的环境变量配置示意图](/mirror/4e/4e36d4949c7a113697c9b232ff3654340870d76a.webp)
...
![通过 Gitee 克隆 Coze Loop 仓库的界面](/mirror/f1/f1a359edfda8284fe5490c1db70fade54348b449.webp)
...
![配置 Coze Loop model_config.yaml 的界面](/mirror/5f/5fd5d9f60e43a2e6dbf09a945dd169e9661ed582.webp)
...
![Coze Loop 首次启动时的日志界面](/mirror/1d/1d6fd328e55556c5525b3e3cfb1cf833162ee04b.webp)

![Coze Loop 启动完成后的日志界面](/mirror/45/4550d4643b1bccc2779463cfc8dc9b3cc48bf5a9.webp)
...
![Docker Desktop 中 Coze Loop 容器运行状态的界面](/mirror/e6/e6bcd18dd0ad1fe42818fbe5c65f087b0d957866.webp)

注册完成后即可进入应用详情页。

![浏览器访问 Coze Loop 开源版时的界面](/mirror/f5/f55e649c7df2e083b4a8c90e05ab632579bed93c.webp)

### 4.6 Coze Loop 主要功能速览

下面以在线版 `https://www.coze.cn/loop` 的 Demo 空间为例，看一下 Coze Loop 的核心功能。开源版和在线版界面可能略有差异，但功能理解是一致的。

![Coze Loop 在线版 Demo 空间界面](/mirror/71/7171a2ec55bc346afb0054dc4c9cc243844ca0d8.webp)

#### 4.6.1 Prompt 开发

用于提示词预览与调试。

![Coze Loop 的 Prompt 开发模块界面](/mirror/da/dacb182ceb0c300f9bd221731433e945c79915bf.webp)

点击详情：

![Coze Loop Prompt 开发详情入口界面](/mirror/f1/f1b7161755643f288c330757595a19f01d14359e.webp)

![Coze Loop Prompt 开发详情页界面](/mirror/99/9964b057d66e96fc41eed534e41b97e0eae5e55e.webp)

#### 4.6.2 Playground

Playground 和 Prompt 开发类似，但更适合做自由对比。

![Coze Loop Playground 模块界面](/mirror/5c/5c3df44ad91c48679337c8a737332ef0a57f6f64.webp)

Demo 空间中无法开启自由对比模式，可切换到个人空间使用。

![Coze Loop 在 Demo 空间中的 Playground 界面](/mirror/a9/a9e759aa65bb7971e074e0e7f457587abc6c793b.webp)

![Coze Loop 切换到个人空间的界面](/mirror/c1/c1cbab58c45f7d380de72603d551b2e34345f203.webp)

![Coze Loop 自由对比模式的界面](/mirror/ae/ae5215e6dd8d5038551ad0ca688fbf339991b6c3.webp)

#### 4.6.3 评测集

评测集用于管理测试样本，是后续实验和自动化评测的基础。

![Coze Loop 评测集模块界面](/mirror/7e/7e389b77fec3a605f93c833d9c7d18eeae1cd0f9.webp)

![Coze Loop 评测集详情界面](/mirror/4a/4afcdd6a5bb61322a06c24f53e838697172d38b9.webp)

![Coze Loop 新建评测集的界面](/mirror/ca/ca23f729f46169d91ba9cf871530cd459523769e.webp)

#### 4.6.4 评估器

该模块用于构建评估 **Prompt 开发**实例的工具，可以理解为带有评分提示词的大模型。

![Coze Loop 评估器模块界面](/mirror/f1/f163fda840db88ef443ba3358489b75fd3b30afe.webp)

![Coze Loop 新建评估器的界面](/mirror/bf/bf5912353bab13e67e69d50ab1646a486497c198.webp)

![Coze Loop 评估器配置详情界面](/mirror/6f/6f1bd98af318ba47424f1902c5ea8960e1299e0a.webp)

#### 4.6.5 实验

实验用于把评测集、评测对象和评估器串起来，做版本对比和效果分析。

![Coze Loop 实验模块界面](/mirror/92/9255a245eec243eef0ed2a48ee75b550c98335d2.webp)

Demo 空间无权创建实验，切换到个人空间即可。

![Coze Loop 切换到个人空间创建实验的界面](/mirror/bc/bcebcf1d6ea24998ea168e6e7b85d6c4a258a9de.webp)

实验详情如下：

![Coze Loop 实验详情页界面](/mirror/bb/bbbdb1ab54f22e00ada1e6bba53d22b8b6841a6e.webp)

指标统计模块会以可视化方式展示评估结果。

![Coze Loop 实验指标统计界面](/mirror/30/30bb69d7e78f6fc40a04e9dbce7971c95dee4cbf.webp)

新建实验时，依次设置基础信息、评测集、评测对象和评估器即可。

![Coze Loop 新建实验的界面](/mirror/28/286ee8e608c51069d8c343c72de5597fa9c18054.webp)

#### 4.6.6 Trace

Trace 用于记录运行链路里的详细信息，适合排查模型调用、工具调用和执行步骤。

![Coze Loop Trace 模块界面](/mirror/e3/e3efb2b14bda2206f5556549893169dc3564f6be.webp)

![Coze Loop Trace 详情界面](/mirror/0d/0dac4df96d59838d9e7062c091202b75f7175ed4.webp)

![Coze Loop Trace 运行明细界面](/mirror/20/20f7cfba1da1ddc460763cb31c0687a7e76426b8.webp)

#### 4.6.7 统计与自动化任务

统计模块用于查看空间整体运行情况；自动化任务用于周期性执行评测。

![Coze Loop 自动化任务模块界面](/mirror/dd/ddb052e71f7c479a228105687259d32d66317582.webp)

详情页：

![Coze Loop 自动化任务详情页界面](/mirror/14/14d79d541a089262e9ae4c2e5c83a53eb9fb391f.webp)

![Coze Loop 自动化任务配置明细界面](/mirror/57/576e276ab0137def9c7ccfe145818a02d7eeee3e.webp)
