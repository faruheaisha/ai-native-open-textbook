...
- **Format** 主要会在 [第 13 章 提示词与消息模板](/lib/08-agents/ai-agents-from-zero/13-提示词与消息模板) 详细展开
- **Predict** 是本章的重点
- **Parse** 主要会在 [第 14 章 输出解析器](/lib/08-agents/ai-agents-from-zero/14-输出解析器) 详细展开
...
后面你在 [第 18 章 向量数据库与 Embedding 实战](/lib/08-agents/ai-agents-from-zero/18-向量数据库与Embedding实战) 和 [第 19 章 RAG 检索增强生成](/lib/08-agents/ai-agents-from-zero/19-RAG检索增强生成) 会深入学习它。这里先建立一个概念即可：**Embedding 也是模型，只是它的输出不是自然语言，而是向量。**
...
[ModelIO_Params.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/02-models_io/ModelIO_Params.py ":include :type=code")
...
本章先点到为止，后面会在 [第 13 章 提示词与消息模板](/lib/08-agents/ai-agents-from-zero/13-提示词与消息模板) 专门展开。你现在只需要先建立一个概念：
...
[ModelIO_OpenAI.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/02-models_io/ModelIO_OpenAI.py ":include :type=code")
...
[ModelIO_ChatOpenAI.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/02-models_io/ModelIO_ChatOpenAI.py ":include :type=code")
...
[ModelIO_Init_chat_model.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/02-models_io/ModelIO_Init_chat_model.py ":include :type=code")
...
[ModelIO_DeepSeek.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/02-models_io/ModelIO_DeepSeek.py ":include :type=code")
...
这也是你在 [第 10 章 HelloWorld](/lib/08-agents/ai-agents-from-zero/10-LangChain快速上手与HelloWorld) 里已经接触过的主要写法。
...
[ModelIO_Qwen.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/02-models_io/ModelIO_Qwen.py ":include :type=code")
...
**建议下一步：** 先把本章的 `02-models_io` 目录按顺序跑一遍，推荐顺序是：`ModelIO_OpenAI.py` → `ModelIO_ChatOpenAI.py` → `ModelIO_Init_chat_model.py` → `ModelIO_Params.py` → `ModelIO_DeepSeek.py` → `ModelIO_Qwen.py`。跑完之后，再进入 [第 12 章 Ollama 本地部署与调用](/lib/08-agents/ai-agents-from-zero/12-Ollama本地部署与调用)、[第 13 章 提示词与消息模板](/lib/08-agents/ai-agents-from-zero/13-提示词与消息模板)、[第 14 章 输出解析器](/lib/08-agents/ai-agents-from-zero/14-输出解析器)，把本章中的 “模型接入” 和后续的 “输入组织”“输出解析” 串成完整闭环。
