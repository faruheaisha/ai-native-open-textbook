...
这里有一个容易混淆的点：
**并不是说 1.x 以后“链”就没用了，而是说很多旧式高层 Chain 类不再是官方推荐主线。**课程后面的 [第 15 章 LCEL 与链式调用](/lib/08-agents/ai-agents-from-zero/15-LCEL与链式调用) 仍然是主线内容，因为“把多个步骤组合起来”的思想从来没有过时，只是实现方式从“某个现成 Chain 类”逐渐转向 **Runnable / LCEL / Graph**。
...
- [第 10 章 HelloWorld](/lib/08-agents/ai-agents-from-zero/10-LangChain快速上手与HelloWorld)：先学会把模型调起来
- [第 11 章 Model I/O](/lib/08-agents/ai-agents-from-zero/11-Model-I-O与模型接入)：理解模型接入与参数
- [第 13 章 提示词与消息模板](/lib/08-agents/ai-agents-from-zero/13-提示词与消息模板)：理解 Prompt、Message
- [第 14 章 输出解析器](/lib/08-agents/ai-agents-from-zero/14-输出解析器)：理解结构化输出
...
- [第 15 章 LCEL 与链式调用](/lib/08-agents/ai-agents-from-zero/15-LCEL与链式调用)
...
- [第 16 章 记忆与对话历史（含 Redis 基础）](/lib/08-agents/ai-agents-from-zero/16-记忆与对话历史_含Redis基础_)
...
- [第 18 章 向量数据库与 Embedding 实战](/lib/08-agents/ai-agents-from-zero/18-向量数据库与Embedding实战)
- [第 19 章 RAG 检索增强生成](/lib/08-agents/ai-agents-from-zero/19-RAG检索增强生成)
...
- [第 17 章 Tools 工具调用](/lib/08-agents/ai-agents-from-zero/17-Tools工具调用)
- [第 21 章 Agent 智能体](/lib/08-agents/ai-agents-from-zero/21-Agent智能体)
...
**建议下一步：** 直接进入 [第 10 章 LangChain 快速上手与 HelloWorld](/lib/08-agents/ai-agents-from-zero/10-LangChain快速上手与HelloWorld)，先把 **API Key、模型名、Base URL、HelloWorld 调用、多模型共存、企业级封装与流式输出** 跑通。等你真正把第一个 LangChain 调用写出来，再回头看这一章的“定位、架构、模块”，理解会明显更扎实。
