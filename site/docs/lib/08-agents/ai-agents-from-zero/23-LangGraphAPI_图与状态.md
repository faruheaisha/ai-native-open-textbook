...
entryUrl: "https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/23-LangGraphAPI：图与状态.md"
sourceRel: "23-LangGraphAPI：图与状态.md"
rawUrl: "/raw/08-agents/ai-agents-from-zero/23-LangGraphAPI：图与状态.md"
sourceSha256: "a14cde8d2cb33713250db5c3654e98576dbfcc39d01ead975ca05da49e3221b1"
pageSha256: "a14cde8d2cb33713250db5c3654e98576dbfcc39d01ead975ca05da49e3221b1"
...
- 能运行并理解本章全部案例，为后续 [第 24 章 LangGraph API：节点、边与进阶](/lib/08-agents/ai-agents-from-zero/24-LangGraphAPI_节点_边与进阶) 打基础。
...
![有向图在工作流中的含义示意](/mirror/09/09de6bf1e7d811c684502c8785c142bbbcb83a61.jpeg)
...
[第 22 章](/lib/08-agents/ai-agents-from-zero/22-LangGraph概述与快速入门) 已经讲过 LangGraph 四要素。这里继续讲 Graph，是为了把“图构建器、状态接口、状态合并规则”讲清楚。上一章解决“能跑起来”，本章解决“为什么这样设计更稳”。
...
如果先不看条件边、`Send`、`Command`、`Runtime` 等进阶能力（这些在 [第 24 章](/lib/08-agents/ai-agents-from-zero/24-LangGraphAPI_节点_边与进阶) 系统讲），构建一张最基础的 `StateGraph`，最核心的流程还是下面 5 步：
...
[BuildWholeGraphSummary.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-3-LangGraph框架/02-graph/BuildWholeGraphSummary.py ":include :type=code")
...
[DefState.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-3-LangGraph框架/03-state/DefState.py ":include :type=code")
...
![StateGraph 输入输出隔离：外部调用先经过 input_schema 过滤，内部普通节点围绕 state_schema 运行，出图时再由 output_schema 过滤返回](/mirror/37/37a28b9f16bc0c3188bcaf2532d74e9c6f3ebb48.svg)
...
[StateSchema.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-3-LangGraph框架/03-state/schema/StateSchema.py ":include :type=code")
...
[StateReducer_Default.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-3-LangGraph框架/03-state/reducers/StateReducer_Default.py ":include :type=code")
...
[StateReducer_AddMessages.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-3-LangGraph框架/03-state/reducers/StateReducer_AddMessages.py ":include :type=code")
...
[StateReducer_OperatorAdd.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-3-LangGraph框架/03-state/reducers/StateReducer_OperatorAdd.py ":include :type=code")

**字符串连接案例：**

【案例源码】`案例与源码-3-LangGraph框架/03-state/reducers/StateReducer_OperatorAdd2.py`

[StateReducer_OperatorAdd2.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-3-LangGraph框架/03-state/reducers/StateReducer_OperatorAdd2.py ":include :type=code")

**数值累加案例：**

【案例源码】`案例与源码-3-LangGraph框架/03-state/reducers/StateReducer_OperatorAdd3.py`

[StateReducer_OperatorAdd3.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-3-LangGraph框架/03-state/reducers/StateReducer_OperatorAdd3.py ":include :type=code")
...
[StateReducer_OperatorMul.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-3-LangGraph框架/03-state/reducers/StateReducer_OperatorMul.py ":include :type=code")
...
[StateReducer_Custom.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-3-LangGraph框架/03-state/reducers/StateReducer_Custom.py ":include :type=code")
...
[StateReducersMyChatBot.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-3-LangGraph框架/03-state/reducers/StateReducersMyChatBot.py ":include :type=code")

### 3.8 Reducer 选型

学完这些案例后，不要只背 Reducer 名字。真实项目里更重要的是：先看字段语义，再选合并方式。

![Reducer 选型示意：默认覆盖适合最新值，add_messages 适合消息历史，operator.add 适合列表或数值累加，自定义 Reducer 适合去重和按 ID 合并](/mirror/67/67d46149c8834ffb135f7996be37acc2844e35a8.png)
...
![逻辑图编译为 Pregel 图：节点和边转换成 Actors 与 Channels，状态字段 aggregate 也成为运行时通道之一](/mirror/f8/f88c4cddb742e3a971e7bf0b2e411332229ed65f.png)
...
![Actor 内部订阅与写入示意：节点 a 订阅 branch:to:a，写入 branch:to:b 与 branch:to:c；节点 d 订阅 branch:to:d，执行后不再写入分支通道](/mirror/7b/7b7c51540eee2d56776407be0c0a2f0d4ae5efa6.png)
...
![Superstep 执行过程：Step0 执行 a，Step1 执行 b/c，Step2 执行 b_2/d，Step3 再执行 d 后结束](/mirror/2d/2d9f037a5692cec02a8a8d26d01b0a26c9e7d1e8.png)
...
**建议下一步：** 先按顺序跑一遍 `BuildWholeGraphSummary.py`、`DefState.py`、`StateSchema.py` 和 `03-state/reducers` 目录下所有 Reducer 案例，然后自己做两个小练习：把 `BuildWholeGraphSummary.py` 的 `process_data` 改成用 `operator.add` 追加历史记录；把 `StateReducersMyChatBot.py` 再加一个 `latest_summary` 字段，用默认覆盖保存最新摘要。做完后进入 [第 24 章 - LangGraph API：节点、边与进阶](/lib/08-agents/ai-agents-from-zero/24-LangGraphAPI_节点_边与进阶)，继续学习节点、普通边、条件边和 `Command` / `Send` / `Runtime`。
