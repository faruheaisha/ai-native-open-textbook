...
如果说 [第 21 章 Agent 智能体](/lib/08-agents/ai-agents-from-zero/21-Agent智能体) 主要解决的是“**Agent 在原理和代码框架里到底怎么工作**”，那么这一章更关心的是“**不会写太多代码时，怎样先把一个可用的智能体应用做出来**”。
...
- 如果你需要更复杂的状态控制、多轮任务编排、可测试的代码架构，再进入 [第 21 章 Agent 智能体](/lib/08-agents/ai-agents-from-zero/21-Agent智能体) 和后续 LangGraph 主线会更合适。
...
这些概念以后在代码框架里都会再出现，只是名字和写法更工程化。也正因为如此，本章和 [第 21 章 Agent 智能体](/lib/08-agents/ai-agents-from-zero/21-Agent智能体) 不是重复关系，而是**先平台、再原理、再代码**的递进关系。
...
这个定义在 [第 21 章 Agent 智能体](/lib/08-agents/ai-agents-from-zero/21-Agent智能体) 会讲得更完整；而在本章里，你只需要先记住平台智能体最常见的五个组成部分：
...
> **这一节和 [第 1-2 章 提示词工程基础](/lib/08-agents/ai-agents-from-zero/1-2-提示词工程基础)、[第 13 章 提示词与消息模板](/lib/08-agents/ai-agents-from-zero/13-提示词与消息模板) 的关系：**
>
> - 第 1-2 章解决“提示词怎么写”
...
这也是为什么很多真实项目并不是“先做一个很聪明的 Agent”，而是先把稳定链路搭成工作流。后面你在本章案例区，以及 [第 4 章 Python 调用 Dify 平台工作流](/lib/08-agents/ai-agents-from-zero/4-Python调用Dify平台工作流)、[第 5 章 Python 调用 Coze 平台工作流](/lib/08-agents/ai-agents-from-zero/5-Python调用Coze平台工作流) 里会不断看到这一点。
...
这里我们不再重复展开 Agent 的完整理论定义，因为那部分会在 [第 21 章 Agent 智能体](/lib/08-agents/ai-agents-from-zero/21-Agent智能体) 系统讲解。本节只从平台使用角度理解：Dify 的 Agent Assistant 是一种“**让模型围绕目标生成结果，并可按需接工具的应用形态**”。
...
- **提示词**：角色（旅行顾问）+ 技能、目标、限制 + 工作流程（理解需求 → 调用搜索工具 → 用 Markdown 输出计划）+ 示例模板 + 变量（<code v-pre>{{destination}}</code>、<code v-pre>{{num_day}}</code>、<code v-pre>{{budget}}</code>）。
...
详见 [**第 4 章（Python 调用 Dify 工作流）**](/lib/08-agents/ai-agents-from-zero/4-Python调用Dify平台工作流) 与 [**第 5 章（Python 调用 Coze 工作流）**](/lib/08-agents/ai-agents-from-zero/5-Python调用Coze平台工作流)。
