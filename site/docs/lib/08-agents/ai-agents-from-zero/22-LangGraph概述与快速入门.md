...
entryUrl: "https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/22-LangGraph概述与快速入门.md"
sourceRel: "22-LangGraph概述与快速入门.md"
rawUrl: "/raw/08-agents/ai-agents-from-zero/22-LangGraph概述与快速入门.md"
sourceSha256: "2a00b0b10c10ca331c7d1cd1cd1418d8692cedaa64d5fe671a42c957b153f281"
pageSha256: "2a00b0b10c10ca331c7d1cd1cd1418d8692cedaa64d5fe671a42c957b153f281"
...
**学习建议：** 这一章先跑通第一张 LangGraph 图，不急着背完所有 API。先抓住四个词：State 保存数据，Node 处理任务，Edge 决定流转，Graph 把它们编排起来。状态合并放到 [第 23 章](/lib/08-agents/ai-agents-from-zero/23-LangGraphAPI_图与状态)，节点和控制流放到 [第 24 章](/lib/08-agents/ai-agents-from-zero/24-LangGraphAPI_节点_边与进阶)，高级能力再看 [第 25 章](/lib/08-agents/ai-agents-from-zero/25-LangGraph高级特性) 和 [第 26 章](/lib/08-agents/ai-agents-from-zero/26-LangGraph多智能体与A2A)。
...
这一章先建立“**图 + 状态 + 最小运行方式**”的入门认知。上表里的持久化、流式、人机介入等能力，会在 [第 25 章](/lib/08-agents/ai-agents-from-zero/25-LangGraph高级特性) 展开。
...
![基于 LLM 的智能体工作流示意：开始 → 大模型节点 → 边节点判断下一步 → 结束或执行 Action 并回到大模型](/mirror/c3/c3ff90f3559857c79fd09f6c6cde1154c7664c44.jpeg)
...
![人机协作（HITL）：课程内容经 AI 分析师建议 → AI 改写器执行修改，不确定时交由人类专家审核决策再回到改写器，最终输出](/mirror/49/4951a01afb6c214954702a73aa061ea6b34cc0f9.svg)

![多智能体协作：分层规划与共创协作两种模式，模拟现实团队工作方式](/mirror/66/665a1afe16159bb85020d6882953024524784841.svg)
...
![构建基石：State 为中心，Nodes 执行逻辑，Edges 定义流转顺序](/mirror/3f/3f344993b8326068d5ac26175f0a9c15085bde5c.png)
...
- **如果某个字段要“追加”而不是“覆盖”，就要考虑 Reducer。** 本章会通过 `add_messages` 先建立直觉，完整的 Schema / Reducer 机制会在 [第 23 章](/lib/08-agents/ai-agents-from-zero/23-LangGraphAPI_图与状态) 系统讲。
...
![LangGraph 把复杂 AI 应用从“一条直线”变成“一张网”](/mirror/21/21edae62ac9aa8201d3608c49c098944d3cafdf1.png)
...
![Quickstart 关键步骤示意：官方提供 Graph API 与 Functional API 两种方式，图中红框突出 Graph API（本教程采用）](/mirror/c3/c3dbc821650a03cb3f91c9a8bcee7bb8f0a63bb0.jpeg)
...
![LangGraph 核心功能划分：预构建 Agent API、Agent Node API、构建图语法与 API](/mirror/18/18d299b11c5aa6aec7832d4a4c9687ec1a7376b8.jpeg)
...
[LangGraphHello.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-3-LangGraph框架/01-helloworld/LangGraphHello.py ":include :type=code")
...
![Mermaid 在线编辑器：左侧为 draw_mermaid() 生成的代码，右侧为渲染的流程图（__start__ → greeting → add_emoji → __end__）](/mirror/95/95ad57752c2cc71f13d0c01e91f75e1a5eca3d66.png)
...
[LangGraphBiz.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-3-LangGraph框架/01-helloworld/LangGraphBiz.py ":include :type=code")
...
- `init_chat_model(...)` 这一段其实复用了 [第 10 章](/lib/08-agents/ai-agents-from-zero/10-LangChain快速上手与HelloWorld) 的“调用三件套”思维：**模型名、API Key、Base URL**。
...
[LangGraphLLM.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-3-LangGraph框架/01-helloworld/LangGraphLLM.py ":include :type=code")
...
![企业问答助手并行检索与融合生成流程：用户问题同时进入知识库检索和联网搜索，两路结果写入 State 后交给融合生成节点输出最终回答](/mirror/be/bec7d639a6415e4115f58272399fa40bf4c8135e.png)
...
**建议下一步：** 先在本地把 `案例与源码-3-LangGraph框架/01-helloworld` 这 3 个脚本都跑一遍，然后做两个小改造：给 `LangGraphBiz.py` 再加一个乘法节点；给 `LangGraphLLM.py` 多传一轮历史消息，观察 `messages` 列表是怎么增长的。做完之后，再进入 [第 23 章 - LangGraph API：图与状态](/lib/08-agents/ai-agents-from-zero/23-LangGraphAPI_图与状态)，系统学习 State Schema 和 Reducer。
