...
![无记忆时两轮对话相互独立，模型无法利用上一轮信息](https://gh-proxy.com/https://raw.githubusercontent.com/didilili/ai-agents-from-zero/ea7f28ffe0b2c2650e3936f3bb591560225702b3/images/16/16-1-1-1.jpeg)
...
![LangChain 官方文档「Core components → Short-term memory」概述：记忆用于保存先前交互信息，支撑智能体在多轮交互中保持效率与体验](https://gh-proxy.com/https://raw.githubusercontent.com/didilili/ai-agents-from-zero/ea7f28ffe0b2c2650e3936f3bb591560225702b3/images/16/16-1-1-2.jpeg)
...
- **为链式流程提供上下文**：历史消息最终通常通过 [第 13 章](/lib/08-agents/ai-agents-from-zero/13-提示词与消息模板) 的 `MessagesPlaceholder` 注入模板，与 [第 15 章](/lib/08-agents/ai-agents-from-zero/15-LCEL与链式调用) 中的链组合在一起使用。
...
- **不是 RAG 检索记忆**：把外部文档查出来再回答，属于 [第 19 章 RAG](/lib/08-agents/ai-agents-from-zero/19-RAG检索增强生成) 的主线。

- **不是 Agent 决策状态机**：更复杂的循环式状态与持久化，会在 [Agent](/lib/08-agents/ai-agents-from-zero/21-Agent智能体) 与 LangGraph 相关章节里更自然地出现。
...
[Memory_IDontKnow.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/07-memory/Memory_IDontKnow.py ":include :type=code")
...
![记忆在链中的位置：读历史 → 拼入 Prompt → 调模型 → 写回历史](https://gh-proxy.com/https://raw.githubusercontent.com/didilili/ai-agents-from-zero/ea7f28ffe0b2c2650e3936f3bb591560225702b3/images/16/16-3-1-1.jpeg)
...
**第二步：把历史消息插入 Prompt。**  
这一步通常会和 [第 13 章](/lib/08-agents/ai-agents-from-zero/13-提示词与消息模板) 里的 `MessagesPlaceholder("history")` 配合使用。
...
在对话历史场景里，你通常不会把历史消息一条条手写死在模板中，因为历史轮数与内容每轮都在变。更合理的做法是在模板里留一个“历史消息插槽”，运行时再把当前会话历史整块塞进去——这正是 [第 13 章](/lib/08-agents/ai-agents-from-zero/13-提示词与消息模板) 中 `MessagesPlaceholder` 的典型用法，与本章的“读写历史”前后衔接。
...
[Memory_RunnableWithMessageHistory.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/07-memory/Memory_RunnableWithMessageHistory.py ":include :type=code")
...
[Memory_RunnableWithMessageHistoryV2.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/07-memory/Memory_RunnableWithMessageHistoryV2.py ":include :type=code")
...
[Memory_InMemoryChatMessageHistory.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/07-memory/Memory_InMemoryChatMessageHistory.py ":include :type=code")
...
![Redis Stack 与原生 Redis 的关系](https://gh-proxy.com/https://raw.githubusercontent.com/didilili/ai-agents-from-zero/ea7f28ffe0b2c2650e3936f3bb591560225702b3/images/16/16-5-2-1.jpeg)
...
[RedisEnvCheck.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/07-memory/RedisEnvCheck.py ":include :type=code")
...
[Memory_RedisChatMessageHistory.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/07-memory/Memory_RedisChatMessageHistory.py ":include :type=code")
...
[Memory_RedisStackChatMessageHistory.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/07-memory/Memory_RedisStackChatMessageHistory.py ":include :type=code")
...
![RedisInsight 中查看 LangChain 写入的会话：键 message_store:user-001 类型为 LIST，元素为序列化后的 human/ai 消息（JSON），便于对照代码理解持久化结构](https://gh-proxy.com/https://raw.githubusercontent.com/didilili/ai-agents-from-zero/ea7f28ffe0b2c2650e3936f3bb591560225702b3/images/16/16-5-2-2.png)
...
**建议下一步：** 先把无记忆、内存版、多 session 版、Redis 版案例都跑一遍，重点观察“同一个 `session_id` 下历史是怎么被延续的”；然后继续学习 [第 17 章 Tools 工具调用](/lib/08-agents/ai-agents-from-zero/17-Tools工具调用)，你会更容易理解为什么 Agent 场景不仅需要工具，还需要会话状态与历史管理。
