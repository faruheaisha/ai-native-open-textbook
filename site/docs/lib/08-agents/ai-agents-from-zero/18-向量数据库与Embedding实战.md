...
- 建立一条清晰主线：**文本先向量化，再写入向量库，再做相似性检索**；学完本章后，可以自然衔接到 [第 19 章 RAG 检索增强生成](/lib/08-agents/ai-agents-from-zero/19-RAG检索增强生成)。
...
![文本、图像等通过嵌入模型转为向量，便于相似性计算与检索](/mirror/c3/c367bb182f38b86e37ba450a2e8c96ef53919366.webp)
...
![图像向量化与对比示意](/mirror/71/712f391db1235627936285e881efcbb08c12a7c2.webp)

> **图意说明：** 同品种毛色不同、同毛色体型不同——类比向量检索中「按特征维度」区分样本；用于建立「**向量 = 特征坐标**」的直觉（本章主线仍以文本 Embedding 为主）。

![向量维度与检索效果](/mirror/bb/bbb4aa72c8329c4bacfa9cbae31f5f7d4889f1c5.webp)
...
![传统数据库与向量数据库的查询方式对比：字段精确查询侧重值匹配，向量检索侧重语义相似](/mirror/53/53eec70dda91726246f74316b936d434dadf49b2.webp)
...
![稠密向量与稀疏向量对比：稠密向量表达整体语义，稀疏向量更接近关键词权重](/mirror/2e/2e04d8088babf1c4fab882757e1e8875ff23e0bb.webp)
...
![向量与向量库在知识体系中的位置](/mirror/9f/9fdcc8109d047c81b99c67a9acc8985ae28ab5dc.webp)
...
- 项目前面已经在 [第 16 章](/lib/08-agents/ai-agents-from-zero/16-记忆与对话历史_含Redis基础_) 引入了 Redis
...
如果你还没有装好 Redis Stack，请先回看 [第 16 章](/lib/08-agents/ai-agents-from-zero/16-记忆与对话历史_含Redis基础_) 里关于 Docker、Redis Stack、RedisInsight 的部分；本章不重复安装过程，只聚焦“Embedding + 向量检索”这条主线。
...
[Text2Embedding_DashScopeHello.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/09-embedding/Text2Embedding_DashScopeHello.py ":include :type=code")
...
[Text2Embedding_OpenAiHello.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/09-embedding/Text2Embedding_OpenAiHello.py ":include :type=code")
...
[Text2Embedding_DashScope.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/09-embedding/Text2Embedding_DashScope.py ":include :type=code")
...
[Text2Embedding_DashScopePro.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/09-embedding/Text2Embedding_DashScopePro.py ":include :type=code")
...
![HNSW 索引示意：通过分层近邻图减少全量比较，在召回率与查询速度之间折中](/mirror/a0/a0cb533ba8eb25afb987129adcb5f912c65d0377.webp)
...
[Text2Embedding_CosSimilarity.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/09-embedding/Text2Embedding_CosSimilarity.py ":include :type=code")
...
这些会在 [第 19 章 RAG 检索增强生成](/lib/08-agents/ai-agents-from-zero/19-RAG检索增强生成) 继续展开。
...
1. 先准备若干 `Document`（`page_content` + `metadata`，在完整 RAG 中常由加载器与切分器产生，见 [第 19 章](/lib/08-agents/ai-agents-from-zero/19-RAG检索增强生成)）
...
[EmbeddingStoreRedis.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/09-embedding/EmbeddingStoreRedis.py ":include :type=code")

运行后，你可以在 RedisInsight 里看到类似下图的结构：

![RedisInsight 中查看写入的文档与向量](/mirror/fc/fcbfe8e8723ea76eabf10ed1fce6d738385581ae.webp)
...
而 [第 19 章](/lib/08-agents/ai-agents-from-zero/19-RAG检索增强生成) 会在此基础上补齐剩余三步：
...
3. 学完本章后继续阅读 [第 19 章 RAG 检索增强生成](/lib/08-agents/ai-agents-from-zero/19-RAG检索增强生成)，把本章的向量能力接到完整 RAG 流程里。
