...
- 会运行并理解本章全部案例：**输入类型、同步与异步调用、文本模板、对话模板、消息占位符、从 JSON / YAML 加载提示词**，为后续 [输出解析器](/lib/08-agents/ai-agents-from-zero/14-输出解析器)、[LCEL 与链式调用](/lib/08-agents/ai-agents-from-zero/15-LCEL与链式调用)、[记忆与对话历史](/lib/08-agents/ai-agents-from-zero/16-记忆与对话历史_含Redis基础_) 打基础。
...
**官方文档与资源**：详见 [工具导航与参考资料索引 - 提示词与结构化输出](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/工具导航与参考资料索引.md#提示词与结构化输出)。

---

## 1、Prompt 简介

本章对应 [第 11 章 Model I/O](/lib/08-agents/ai-agents-from-zero/11-Model-I-O与模型接入) 里的 **输入格式化（Format）** 与 **模型调用（Predict）** 两部分。模型并不是“凭空思考”，它始终是在读取我们提供的输入后再生成输出；而 Prompt、Message、Template，正是组织这份输入的核心工具。

如果你已经学过 [1-2 提示词工程基础](/lib/08-agents/ai-agents-from-zero/1-2-提示词工程基础)，那么本章可以直接理解为它的代码实现版：前一章讲“怎么把角色、任务、上下文、输入、输出和约束说清楚”，这一章讲“这些内容进入 LangChain 后，应该以什么输入形态存在、如何复用、如何插入多轮历史、如何外置到文件”。
...
![DeepSeek 官方提示词示例库入口](https://gh-proxy.com/https://raw.githubusercontent.com/didilili/ai-agents-from-zero/ea7f28ffe0b2c2650e3936f3bb591560225702b3/images/13/13-1-1-1.jpeg)
...
这一章不是零散的 API 介绍，而是在回答一个问题：**开发 LLM 应用时，怎样把 [1-2 提示词工程基础](/lib/08-agents/ai-agents-from-zero/1-2-提示词工程基础) 里学到的写法和原则，组织成一套可复用、可维护、可扩展的结构？**

---

## 2、调用大模型的入参类型

初学者容易误以为：调用聊天模型时，输入永远只能是“一段字符串”。实际上，LangChain 聊天模型为了适配真实对话场景，通常支持多种输入形态。这些写法表面不同，核心都一样：**把 [1-2 章](/lib/08-agents/ai-agents-from-zero/1-2-提示词工程基础) 里讲过的角色、任务、上下文、输入和约束，以合适的消息结构交给模型。**

### 2.1 入参形态总览

同一次 `invoke`，左侧可以是多种类型的输入，中间由聊天模型处理，右侧典型返回值是 `AIMessage`。这也是为什么你在 [第 11 章](/lib/08-agents/ai-agents-from-zero/11-Model-I-O与模型接入) 会经常看到“正文一般通过 `.content` 读取”。

![聊天模型 invoke：常见入参类型与 AIMessage 输出](https://gh-proxy.com/https://raw.githubusercontent.com/didilili/ai-agents-from-zero/ea7f28ffe0b2c2650e3936f3bb591560225702b3/images/13/13-2-1-1.png)
...
[LLM_Invoke_InputTypes.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/04-prompt/invoke/LLM_Invoke_InputTypes.py ":include :type=code")
...
这正对应了 [1-2 提示词工程基础](/lib/08-agents/ai-agents-from-zero/1-2-提示词工程基础) 里强调的那条原则：**稳定约束和动态输入要拆开写。**在 LangChain 里，这条原则最直接的落地方式就是把稳定规则放进 `SystemMessage`，把当前任务放进 `HumanMessage`。
...
- **本地开源模型（Ollama 等）**：实例化与端点配置见 [第 12 章 Ollama 本地部署与调用](/lib/08-agents/ai-agents-from-zero/12-Ollama本地部署与调用)，本章的 `invoke` / `stream` / `batch` 用法同样适用。

[LLM_Invoke.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/04-prompt/invoke/LLM_Invoke.py ":include :type=code")

[LLM_aInvoke.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/04-prompt/invoke/LLM_aInvoke.py ":include :type=code")
...
[LLM_Stream.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/04-prompt/invoke/LLM_Stream.py ":include :type=code")

[LLM_aStream.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/04-prompt/invoke/LLM_aStream.py ":include :type=code")
...
[LLM_Batch.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/04-prompt/invoke/LLM_Batch.py ":include :type=code")

[LLM_aBatch.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/04-prompt/invoke/LLM_aBatch.py ":include :type=code")
...
[PromptTemplate_FormatMethod.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/04-prompt/prompt_templates/method/PromptTemplate_FormatMethod.py ":include :type=code")

[PromptTemplate_InvokeMethod.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/04-prompt/prompt_templates/method/PromptTemplate_InvokeMethod.py ":include :type=code")

[PromptTemplate_PartialMethod.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/04-prompt/prompt_templates/method/PromptTemplate_PartialMethod.py ":include :type=code")
...
[PromptTemplate_Constructor.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/04-prompt/prompt_templates/PromptTemplate_Constructor.py ":include :type=code")

[PromptTemplate_FromTemplate.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/04-prompt/prompt_templates/PromptTemplate_FromTemplate.py ":include :type=code")
...
【案例源码】`案例与源码-2-LangChain框架/04-prompt/prompt_templates/PromptTemplate_Combined.py`

[PromptTemplate_Combined.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/04-prompt/prompt_templates/PromptTemplate_Combined.py ":include :type=code")

**第二，比较 `partial_variables` 与 `partial()`。**  
二者都能做到“先固定一部分变量，后续只传剩余变量”，但一个发生在模板创建阶段，一个发生在已有模板基础上。

【案例源码】`案例与源码-2-LangChain框架/04-prompt/prompt_templates/PromptTemplate_PartialVariables.py`

[PromptTemplate_PartialVariables.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/04-prompt/prompt_templates/PromptTemplate_PartialVariables.py ":include :type=code")
...
[ChatPromptTemplate_TupleParam.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/04-prompt/chat_prompt_template/parameter/ChatPromptTemplate_TupleParam.py ":include :type=code")

[ChatPromptTemplate_DictParam.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/04-prompt/chat_prompt_template/parameter/ChatPromptTemplate_DictParam.py ":include :type=code")

[ChatPromptTemplate_MessageParam.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/04-prompt/chat_prompt_template/parameter/ChatPromptTemplate_MessageParam.py ":include :type=code")
...
[ChatPromptTemplate_FormatMessages.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/04-prompt/chat_prompt_template/ChatPromptTemplate_FormatMessages.py ":include :type=code")
...
[ChatPromptTemplate_Constructor.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/04-prompt/chat_prompt_template/ChatPromptTemplate_Constructor.py ":include :type=code")
...
![MessagesPlaceholder 将历史消息动态插入 ChatPromptTemplate：模板先留占位，运行时再填入多轮消息](https://gh-proxy.com/https://raw.githubusercontent.com/didilili/ai-agents-from-zero/ea7f28ffe0b2c2650e3936f3bb591560225702b3/images/13/13-7-5-1.svg)
...
这段代码的价值在于，它让模板本身变得非常稳定，而把“历史有几轮、具体内容是什么”延迟到运行时再决定。后面学 [第 16 章 记忆与对话历史](/lib/08-agents/ai-agents-from-zero/16-记忆与对话历史_含Redis基础_) 时，你会频繁看到这种模式。
...
[ChatPromptTemplate_ExplicitPlaceholder.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/04-prompt/chat_prompt_template/placeholder/ChatPromptTemplate_ExplicitPlaceholder.py ":include :type=code")

[ChatPromptTemplate_ImplicitPlaceholder.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/04-prompt/chat_prompt_template/placeholder/ChatPromptTemplate_ImplicitPlaceholder.py ":include :type=code")
...
运行这类案例时，先确认**当前工作目录**，否则相对路径可能找不到文件。这一点和 [第 10 章 HelloWorld](/lib/08-agents/ai-agents-from-zero/10-LangChain快速上手与HelloWorld) 中的 `.env` 读取问题本质很像，都是“脚本运行位置”和“相对路径”的关系。
...
[PromptLoadDemo01.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/04-prompt/load_external/PromptLoadDemo01.py ":include :type=code")

[PromptLoadDemo02.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/04-prompt/load_external/PromptLoadDemo02.py ":include :type=code")
...
**建议下一步：** 继续学习 [第 14 章 输出解析器](/lib/08-agents/ai-agents-from-zero/14-输出解析器)，把本章的“输入组织”与下一章的“输出结构化”连起来；再配合 [第 15 章 LCEL 与链式调用](/lib/08-agents/ai-agents-from-zero/15-LCEL与链式调用)，就能形成 LangChain 中最核心的“输入 -> 模型 -> 输出 -> 链式编排”主线。如果你读到这里，仍然对“为什么要这样拆 Prompt”不够踏实，也可以回看 [1-2 提示词工程基础](/lib/08-agents/ai-agents-from-zero/1-2-提示词工程基础) 中的六要素、Few-shot 和结构化组织方式部分。
