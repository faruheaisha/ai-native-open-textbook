...
- 跑通并理解本章全部案例：**基础加法工具、Pydantic 参数 schema、天气查询工具、天气助手完整链路**，为后续 [记忆与对话历史](/lib/08-agents/ai-agents-from-zero/16-记忆与对话历史_含Redis基础_)、[Agent 智能体](/lib/08-agents/ai-agents-from-zero/21-Agent智能体)、[MCP 模型上下文协议](/lib/08-agents/ai-agents-from-zero/20-MCP模型上下文协议) 打基础。
...
![未开启联网或工具时，对话产品无法给出实时天气，只能提示用户开启能力（示意图）](https://gh-proxy.com/https://raw.githubusercontent.com/didilili/ai-agents-from-zero/ea7f28ffe0b2c2650e3936f3bb591560225702b3/images/17/17-1-2-1.jpeg)
...
所以本章的重点，是先把 **“工具本身”** 和 **“单轮/手动工具调用链路”** 学明白。到了 [第 21 章 Agent 智能体](/lib/08-agents/ai-agents-from-zero/21-Agent智能体)，你再去理解“让模型自己循环地、多步地调用工具”，就会轻松很多。
...
![工具调用泳道图：用户、程序、工具、大模型四栏协作（用户提问 → 程序带工具定义调模型 → 模型判断是否调用工具 → 程序执行工具 → 结果再交模型生成回复）](https://gh-proxy.com/https://raw.githubusercontent.com/didilili/ai-agents-from-zero/ea7f28ffe0b2c2650e3936f3bb591560225702b3/images/17/17-2-1-1.jpeg)
...
如果你看过 [第 11 章](/lib/08-agents/ai-agents-from-zero/11-Model-I-O与模型接入) 和 [第 13 章](/lib/08-agents/ai-agents-from-zero/13-提示词与消息模板)，这里会看到一条连续的消息主线：
...
![bind_tools 与 Agent 自动执行工具的边界：绑定工具只提供 schema，Agent 才负责循环执行工具并写回观察结果](https://gh-proxy.com/https://raw.githubusercontent.com/didilili/ai-agents-from-zero/ea7f28ffe0b2c2650e3936f3bb591560225702b3/images/17/17-2-1-2.svg)
...
而本课程当前案例为了更适合初学者理解，也保留了一种更直观的教学路径：**通过[输出解析器](/lib/08-agents/ai-agents-from-zero/14-输出解析器)把工具参数取出来，然后手动执行工具，再把结果交给下一条 [LCEL](/lib/08-agents/ai-agents-from-zero/15-LCEL与链式调用) 链去整理自然语言。**
...
![LangChain 文档摘录：使用 @tool 装饰器创建工具，默认以函数 docstring 作为工具描述（Basic tool definition）](https://gh-proxy.com/https://raw.githubusercontent.com/didilili/ai-agents-from-zero/ea7f28ffe0b2c2650e3936f3bb591560225702b3/images/17/17-3-1-1.jpeg)
...
[Tool_AddNumberTool.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/08-tools/Tool_AddNumberTool.py ":include :type=code")
...
![@tool 与函数名、类型注解、docstring 在工具定义中的角色示意（装饰器注册、函数名为工具 ID、类型帮助生成参数、文档字符串为「使用说明」）](https://gh-proxy.com/https://raw.githubusercontent.com/didilili/ai-agents-from-zero/ea7f28ffe0b2c2650e3936f3bb591560225702b3/images/17/17-3-3-1.jpeg)
...
[PydanticDemo.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/08-tools/PydanticDemo.py ":include :type=code")
...
[Tool_AddNumberToolPro.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/08-tools/Tool_AddNumberToolPro.py ":include :type=code")
...
![OpenWeather 用户后台「API keys」页：查看已有密钥状态，或通过 Create key 生成新密钥（教程中用于配置天气工具）](https://gh-proxy.com/https://raw.githubusercontent.com/didilili/ai-agents-from-zero/ea7f28ffe0b2c2650e3936f3bb591560225702b3/images/17/17-5-1-1.png)
...
[QueryWeatherTool.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/08-tools/QueryWeatherTool.py ":include :type=code")
...
![Function calling 技术思路：普通对话（用户 ↔ 大模型）与「查询天气」场景下，大模型发出函数调用请求 → 外部函数 get_weather → 请求 OpenWeather API → 响应回到模型再生成用户可见回复](https://gh-proxy.com/https://raw.githubusercontent.com/didilili/ai-agents-from-zero/ea7f28ffe0b2c2650e3936f3bb591560225702b3/images/17/17-5-3-1.jpeg)
...
[LLMQueryWeatherDemo.py](https://github.com/didilili/ai-agents-from-zero/blob/ea7f28ffe0b2c2650e3936f3bb591560225702b3/案例与源码-2-LangChain框架/08-tools/LLMQueryWeatherDemo.py ":include :type=code")

其中第 4 步「解析工具调用参数」依赖 `JsonOutputKeyToolsParser`，其角色是把模型输出中的工具调用片段解析成可执行的 Python 结构，详见 [第 14 章 输出解析器](/lib/08-agents/ai-agents-from-zero/14-输出解析器)。
...
- 和 [第 16 章记忆与对话历史](/lib/08-agents/ai-agents-from-zero/16-记忆与对话历史_含Redis基础_) 的关系：工具调用结果经常也要进入消息流，与对话历史一起参与后续推理
- 和 [第 21 章Agent智能体](/lib/08-agents/ai-agents-from-zero/21-Agent智能体) 的关系：Agent 会在 Tool 基础上进一步解决“什么时候调、调几次、按什么顺序调”
- 和 [第 20 章MCP模型上下文协议](/lib/08-agents/ai-agents-from-zero/20-MCP模型上下文协议) 的关系：MCP 解决的是“如何用标准协议把外部能力开放给模型”，可以看作更通用、更标准化的工具接入方式
...
**建议下一步：** 先把本章 5 个案例全部跑一遍，重点观察“工具描述、参数 schema、模型返回 tool_calls、程序执行工具”这四个环节；然后继续学习 [第 21 章 Agent 智能体](/lib/08-agents/ai-agents-from-zero/21-Agent智能体)，你会更容易理解为什么 Agent 的核心不是“多了几个 API”，而是在 Tool 基础上增加了自主决策与多步编排能力。
