...
Ten questions covering the concepts and notebook code are in [`quiz.md`](/lib/01-foundations/ai-engineering-lab/curriculum-week-06-quiz), the passing bar is **8/10**.

## Exercises

Four graded exercises (easy / standard / stretch / portfolio), a model swap, a CoT fourth version, a tiered eviction policy, and a `bol_extractor.py` with a CI gate. Hints for each are in [`exercises.md`](/lib/01-foundations/ai-engineering-lab/curriculum-week-06-exercises).
