---
title: "06 · Lakeflow: Pipelines, Jobs & Connect"
...
entryUrl: "https://github.com/zorost/AI-Engineering-Lab/blob/cdd8dbdf559f72211a7c068e8877918441531e52/reference/platforms/databricks/06-pipelines-jobs.md"
sourceRel: "reference/platforms/databricks/06-pipelines-jobs.md"
rawUrl: "/raw/01-foundations/ai-engineering-lab/reference/platforms/databricks/06-pipelines-jobs.md"
sourceSha256: "cdddae869a736f06a460761431defc633e2a164ad0e47d9c0ab9b8e1cd0f2910"
pageSha256: "cdddae869a736f06a460761431defc633e2a164ad0e47d9c0ab9b8e1cd0f2910"
...
- `STREAM(LIVE.)` / `LIVE.` references an upstream dataset; `STREAM(...)`
...
- Resolves the **DAG** from your `LIVE.` references,
...
- Declare a parameter on the job (e.g., `run_date`), reference it in task configs as
  <code v-pre>{{job.parameters.run_date}}</code>.
...
2. **Reference with <code v-pre>{{job.parameters.&lt;name>}}</code>** in task configs; the notebook reads the
...
| **Conditional** | A → if(month-end) → B | Month-end close, special runs | **If/else** task on <code v-pre>{{job.parameters.run_date}}</code> |
...
- [ ] Passed <code v-pre>{{job.parameters.*}}</code> into a notebook via `dbutils.widgets`.
