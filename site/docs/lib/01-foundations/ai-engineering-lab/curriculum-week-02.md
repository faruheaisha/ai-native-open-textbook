...
Open [`quiz.md`](/lib/01-foundations/ai-engineering-lab/curriculum-week-02-quiz) and answer all 10 questions. The passing bar is **8/10**; each question names the Concepts subsection or notebook cell it comes from.

## Exercises

Four graded exercises, **Easy** (run the cleaning notebook and record the counts), **Standard** (document the five planted issues with evidence), **Stretch** (rewrite the on-time query with a window function and assert agreement), and **Portfolio** (turn cleaning into a runnable `make_silver.py`). Hints for each live in [`exercises.md`](/lib/01-foundations/ai-engineering-lab/curriculum-week-02-exercises).
