...
Open [`quiz.md`](/lib/01-foundations/ai-engineering-lab/curriculum-week-03-quiz) and answer all 10 questions. The passing bar is **8/10**; each question names the Concepts subsection or notebook cell it comes from.

## Exercises

Four graded exercises, **Easy** (run the regression notebook and record MAE/RMSE), **Standard** (add a fourth regressor and state the delta), **Stretch** (time-series cross-validation), and **Portfolio** (commit the model card + experiment log). Hints for each live in [`exercises.md`](/lib/01-foundations/ai-engineering-lab/curriculum-week-03-exercises).
