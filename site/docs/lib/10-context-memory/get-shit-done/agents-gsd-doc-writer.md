...
5. Include the GSD marker `` as the very first line of the file.
6. Follow the Required Sections from the matching template section.
7. Place `` markers on any infrastructure claim (URLs, server configs, external service details) that cannot be verified from the repository contents alone.
&lt;/create_mode>
...
3. For each failure:
   ...
   d. If the correct value cannot be determined, replace the claim with a `` marker.
...
**VERIFY marker guidance:** Use `` for:
