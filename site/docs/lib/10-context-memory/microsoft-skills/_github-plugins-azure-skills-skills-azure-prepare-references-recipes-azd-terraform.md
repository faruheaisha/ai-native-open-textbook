...
entryUrl: "https://github.com/microsoft/skills/blob/cf77b1efbf3117501f4727c476894751311ee885/.github/plugins/azure-skills/skills/azure-prepare/references/recipes/azd/terraform.md"
sourceRel: ".github/plugins/azure-skills/skills/azure-prepare/references/recipes/azd/terraform.md"
rawUrl: "/raw/10-context-memory/microsoft-skills/.github/plugins/azure-skills/skills/azure-prepare/references/recipes/azd/terraform.md"
sourceSha256: "33794b8e1b4cb4dc6bf5badebaa8a90def9394a80729c4c1df29ada995c8f2a4"
pageSha256: "33794b8e1b4cb4dc6bf5badebaa8a90def9394a80729c4c1df29ada995c8f2a4"
...
> \}
> ```
> See [Terraform Functions patterns](/lib/10-context-memory/microsoft-skills/_github-plugins-azure-skills-skills-azure-prepare-references-services-functions-terraform) for Flex Consumption examples.

### 4. Variables and Outputs

> ⚠️ **WARNING: Use `${VAR}` syntax in `main.tfvars.json`, NOT Go-style <code v-pre>{{ .Env.* }}</code>**
>
> azd's template engine processes `azure.yaml` and service manifests — it does **NOT** interpolate
> Go-style <code v-pre>{{ .Env.* }}</code> template variables in `.tfvars.json` or any Terraform variable files.
> Literal strings like <code v-pre>{{ .Env.AZURE_ENV_NAME }}</code> will be passed directly to Terraform, causing
> deployment failures.
>
...
> ⚠️ **Use `${VAR}` syntax in `main.tfvars.json`, NOT Go-style <code v-pre>{{ .Env.* }}</code>.** azd substitutes
...
| Literal <code v-pre>{{ .Env.* }}</code> in Terraform errors | Use `${VAR}` syntax in `main.tfvars.json`, not Go-style <code v-pre>{{ .Env.* }}</code>. azd substitutes `${VAR\}` references via envsubst |
| `main.tfvars.json` interpolation failure | Ensure `main.tfvars.json` uses `${VAR}` syntax (e.g., `${AZURE_ENV_NAME\}`), not Go-style <code v-pre>{{ .Env.* }}</code> templates |
