...
<div class="tb-zh"><p>欢迎来到 MCP 课程的第 1 单元！在这个单元中，我们将探讨模型上下文协议的基础知识。</p></div>
...
<div class="tb-zh"><p>理解什么是模型上下文协议，以及它为什么重要；掌握与 MCP 相关的核心概念与术语；了解 MCP 所解决的集成难题；梳理 MCP 的主要收益与目标；看一个 MCP 集成的简单实例。</p></div>
...
<div class="tb-zh"><p>学完本单元，你将对 MCP 的基础概念有扎实的理解，并准备好进入下一个单元，深入探讨它的架构与实现。</p></div>
...
<div class="tb-zh"><p>AI 生态正在快速演进，大语言模型（LLM）与其他 AI 系统的能力越来越强。但这些模型往往受限于训练数据，无法访问实时信息或专用工具。这一限制使 AI 系统在许多场景下难以给出真正切题、准确、有用的回答。</p></div>
...
<div class="tb-zh"><p>模型上下文协议（MCP）正是为此而生。MCP 让 AI 模型能够连接外部数据源、工具与环境，使信息与能力得以在 AI 系统与更广阔的数字世界之间顺畅流转。这种互操作性，对于真正有用的 AI 应用的成长与普及至关重要。</p></div>

## Overview of Unit 1

Here's a brief overview of what we'll cover in this unit:

<div class="tb-zh"><p>下面是本单元内容的一个简要概览：</p></div>
...
<div class="tb-zh"><p>1. 什么是模型上下文协议？——先定义 MCP，并讨论它在 AI 生态中的位置。2. 核心概念——探讨与 MCP 相关的基础概念与术语。3. 集成难题——审视 MCP 想要解决的问题，尤其是「M×N 集成问题」。4. 收益与目标——讨论 MCP 的主要收益与目标，包括标准化、AI 能力增强与互操作性。5. 简单示例——最后通过一个简单的 MCP 集成示例，看看它在实践中如何运作。</p></div>

Let's dive in and explore the exciting world of Model Context Protocol!

<div class="tb-zh"><p>让我们一同深入探索模型上下文协议的精彩世界吧！</p></div>
