...
sourceUrl: ""
entryUrl: null
sourceRel: "docs/cozeloop_evaluate-multi-modal-agent-image.md"
rawUrl: "/raw/04-work/coze-official-docs/docs/cozeloop_evaluate-multi-modal-agent-image.md"
sourceSha256: "eefcd98397906c447bd1dacb6dc8899de47c9c78a7c2379db584a225eec5de37"
pageSha256: "eefcd98397906c447bd1dacb6dc8899de47c9c78a7c2379db584a225eec5de37"
...
* 使用扣子罗盘 SDK 为这个 Agent 上报 Trace 数据。扣子罗盘 SDK使用说明，参见SDK 概述。关于如何使用多模态上报数据协议，可参考 [MessageList-Image Input](https://platform.openai.com/docs/api-reference/chat/create)。多模态 Trace 示例结构如下：
   ```JSON
   [
       {
           "type": "text",
...
4. 根据页面提示配置字段映射，依次将用户问题、多模态数据、Agent 回复回流至评测集的对应字段。
   需要注意的是，对于多模态字段（图片），需要为其映射评测集的**多模态**类型字段。关于回流 Trace 到评测集的详细操作步骤、字段映射配置可参考Trace 数据回流。
