...
sourceUrl: ""
entryUrl: null
sourceRel: "md/developer-users-user-sync-start-user-sync.md"
rawUrl: "/raw/04-work/qwenwork-official-help/md/developer-users-user-sync-start-user-sync.md"
sourceSha256: "762b0caba67012c69c4ba8e11e299ad9a3411d250d2b61cdf47b44d71a9a8b11"
pageSha256: "762b0caba67012c69c4ba8e11e299ad9a3411d250d2b61cdf47b44d71a9a8b11"
...
为已完成配置并处于启用状态的数据源提交一次手动同步任务。响应表示任务已受理，可使用 run.id 查询进度和统计结果；SCIM 通过外部推送同步，不支持此手动任务。

## 所需权限
<table> <thead> <tr> <td>所需权限</td> <td>资源</td> <td>Scope</td> <td>资源服务</td> </tr> </thead> <tbody> <tr> <td>执行用户同步（执行）</td> <td>directory-sync-runs</td> <td>admin.v1.directory-sync-runs.execute</td> <td>qwenwork-biz-service</td> </tr> </tbody> </table>

```
HELPCODEESCAPE-http
POST /api/openapi/v1/directory-sync/runs
```
...
## 请求参数
<table> <thead> <tr> <td>名称</td> <td>位置</td> <td>类型</td> <td>必填</td> <td>说明</td> <td>示例</td> </tr> </thead> <tbody> <tr> <td>Idempotency-Key</td> <td>header</td> <td>string</td> <td>必填</td> <td>本次写操作的幂等键。同一操作重试时复用相同键和请求内容，避免重复处理。</td> <td>"\&lt;string\&gt;"</td> </tr> <tr> <td>source_id</td> <td>body</td> <td>string · uuid</td> <td>必填</td> <td>本次同步使用的数据源标识。</td> <td>"\&lt;uuid\&gt;"</td> </tr> </tbody> </table>
...
HELPCODEESCAPE-text
curl --request POST \
  --url '<BASE_URL>/api/openapi/v1/directory-sync/runs' \
