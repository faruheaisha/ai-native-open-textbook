...
1. **`originalFile: string` → `string | null`**：Edit 工具的 `originalFile` 字段放宽为可空，支持新建文件（无原始文件可参照）的编辑操作
2. **`toolStats` 统计字段**：新增 7 维度的会话级工具使用统计，为成本分析和行为洞察提供量化数据（完整字段定义和 Dream 系统的关联分析详见第24章）
