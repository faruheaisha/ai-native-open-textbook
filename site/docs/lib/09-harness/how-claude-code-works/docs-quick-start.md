...
无论你是想了解 Claude Code 的内部机制，还是想借鉴它的设计思路来构建自己的 coding agent，这篇文章都会帮你快速建立全局认知。文章不逐行读代码，只讲关键设计决策——那些让 Claude Code 从"能用"变成"好用"的工程选择。

## Claude Code 是什么

Claude Code 是 Anthropic 的 CLI 编程 Agent。它不是代码补全工具，而是一个**受控工具循环 Agent**——能理解代码库、编辑文件、执行命令、管理 git 的自主编程助手。

一个 CLI 工具，为什么需要 512K+ 行代码？这正是它有趣的地方。"如何调用大模型 API"只是最简单的一步，真正费工夫的是一系列工程挑战：

- 让 Agent 自主完成复杂任务，靠 Agent Loop 的多轮决策与错误恢复
- 在有限的上下文窗口里高效工作，靠 4 级渐进式上下文压缩
- 让 AI 安全地执行 Shell 命令，靠 7 层纵深防御加 AST 级命令分析
- 让 Agent 跨会话学习，靠记忆系统和技能系统
- 处理超出单 Agent 能力的任务，靠 3 种多 Agent 协作模式

这些问题的答案构成了一套完整的 Agent 工程方法论——这正是本系列文档想帮你理解的。

> 深入阅读：[概述](/lib/09-harness/how-claude-code-works/docs-01-overview)
...
这个流程看起来简单，但魔鬼在细节里。想象你让 Claude Code "重构这个文件里的所有函数名为 snake_case"——模型会先调工具读取文件，用文本推理分析当前命名，然后发起多次工具调用逐个编辑函数名，每次编辑后回到循环里检查是否有遗漏，直到确认全部完成才返回结果。整个过程可能循环十几次，但对用户来说只是一条指令。

### 双层架构

Agent Loop 实现为 `async function*` 异步生成器，采用双层设计：

- QueryEngine（外层）管理对话生命周期——持久化、预算检查、用户中断。它关心的是"这个对话要不要继续"
- query()（内层）管理单次循环——API 流式、工具执行、错误恢复。它关心的是"这一轮怎么跑完"

为什么要分两层？因为会话管理和查询执行的关注点完全不同。会话层需要处理用户中断、Token 预算耗尽、对话持久化等生命周期问题；而查询层只管一个紧凑循环：调 API、解析响应、执行工具、拼装结果。分层让每一层的逻辑都保持清晰。

### 自动错误恢复

query() 有 7 个 Continue Sites，每一处对应一种故障恢复路径：模型输出被截断，就自动用更高的 Token 限制重试；上下文快满了，触发压缩后继续；API 返回错误，按退避策略重试。所以用 Claude Code 时很少遇到报错。

核心设计原则是**错误扣留**——可恢复的错误不暴露给调用者，自动修复后继续。Agent 应该像一个靠谱的同事，遇到小问题自己解决，而不是每次都来问你。

### 流式并行执行

模型还在生成输出时，已完成解析的工具调用就立即开始执行——这叫工具预执行，它把约 1 秒的工具延迟藏在模型生成的 5-30 秒窗口里，用户感受到的是工具"瞬间完成"。执行侧靠 StreamingToolExecutor：边流式解析边并发执行，只读工具自动并行。当模型同时调用多个只读工具，比如同时读三个文件，它们会并发执行而不是排队等待。

> 深入阅读：[系统主循环](/lib/09-harness/how-claude-code-works/docs-02-agent-loop)
...
Snip 把大块工具输出替换成占位符，最轻量，几乎无信息损失。Microcompact 对工具结果做局部压缩，保留关键信息、去掉冗余。Context Collapse 把整段对话折叠成摘要，腾出的空间大得多。Autocompact 是最后手段：在 Token 使用达到约 87% 时触发，fork 一个子 Agent 来生成结构化摘要。
...
Claude Code 包含 50+ 内置工具，全部统一为 `Tool` 接口。核心设计是 **fail-closed 默认值**——新工具如果不显式声明安全属性，默认被当作不安全处理。于是遗漏声明只会让功能受限，不会造成安全漏洞。
...
工具执行走一条 8 阶段管道，依次是查找、校验、并行启动、权限检查、执行、结果处理、后置 Hook、事件发射。当工具输出超过 100K 字符时自动落盘到临时文件，模型只拿到摘要和文件路径，避免超大输出撑爆上下文。
...
Claude Code 选择了 **search-and-replace 策略**，理由有四：
...
> 深入阅读：[代码编辑策略](/lib/09-harness/how-claude-code-works/docs-05-code-editing-strategy)
...
这是整个系统中最复杂的部分：用 tree-sitter 对命令做 AST 级别的分析，加上 23 项静态检查，覆盖命令注入、环境变量泄露、Shell 元字符等攻击向量。黑名单匹配停在字符串层面，它则真正"理解"命令的结构。

### 权限决策竞速

权限确认走竞速机制：UI 对话框、Hook 和 LLM 分类器同时运行，第一个完成的决定生效。明显安全的操作由分类器快速判定，用户不需要等待；需要人工判断的操作，才会弹出 UI 对话框。用户交互始终优先于自动结果，另有 200ms 防误触宽限期。
...
> 深入阅读：[权限与安全](/lib/09-harness/how-claude-code-works/docs-11-permission-security)
...
Hook 执行引擎有一处关键的快路径优化：当所有匹配的 Hook 都是 callback/function 类型时，框架跳过 JSON 序列化和进度事件，延迟降低约 70%。

> 深入阅读：[Hooks 与可扩展性](/lib/09-harness/how-claude-code-works/docs-06-hooks-extensibility)
...
最常用。通过 AgentTool fork 出独立子任务，每个子 Agent 有自己的上下文窗口和工具集。子 Agent **不继承父对话历史**，只接收自包含的任务描述，隔离性和成本都因此可控。工具经过 4 层过滤管道——先移除元工具，再依次套上自定义限制、异步白名单和 Agent 级禁止列表——不同类型的子 Agent 拿到不同的工具集。每个子 Agent 还可以分到独立的 Git Worktree 代码副本，多 Agent 同时编辑不同文件不会冲突。

内置类型有 3 种：Explore 只读，用 Haiku 模型降低成本；Plan 也是只读，走结构化输出；General-purpose 拿完整工具集。

### 协调器模式（Coordinator）

纯指挥官——**只能分配任务，不能自己读文件、写代码**。这个设计看似限制，实际上防止了协调器"顺手"做本该交给 worker 的事情，确保职责清晰。标准工作流分 4 个阶段，依次是研究、综合、实施、验证。

### Swarm 团队模式

最灵活也最复杂。命名 Agent 间通过对等信箱通信，不需要中央协调器。适合多个 Agent 长时间并行工作的场景。

> 深入阅读：[多 Agent 架构](/lib/09-harness/how-claude-code-works/docs-07-multi-agent)
...
Claude Code 的记忆系统是一个有结构、有分类、有智能召回的知识库。记忆类型封闭为 4 种——user 存用户画像，feedback 存行为校正，project 存项目上下文，reference 存外部资源指针——封闭分类防止标签膨胀。召回按语义做，由 Sonnet 模型评估每条记忆与当前任务的相关性，不做关键词匹配。

它同样明确"不记什么"：代码模式、git 历史、已有文档里的内容，都可以从当前项目状态推导出来，记下来只会变成过时信息。记忆与 CLAUDE.md 各管一头：CLAUDE.md 是团队共享的项目规则，签入 git；记忆是个人的跨会话学习，存在本地。

### 技能系统

如果说记忆是 Agent 的"长期知识"，技能就是它的"可复用能力"——可以理解为"AI Shell 脚本"。

技能既可以由用户通过 `/commit` 这样的斜杠命令手动调用，也可以由模型根据上下文自动触发——用户说"帮我提交"，模型就判断该调用 commit 技能。加载分 6 层优先级（托管 > 项目 > 用户 > 插件 > 内置 > MCP），团队可在项目级别定义共享技能，个人用户可以覆盖或扩展。技能是懒加载的：注册时只读 frontmatter 元数据，技能内容在真正调用时才加载，保持启动速度。技能列表自身的 Token 预算走 3 阶段算法：先给全量描述，超预算就退到分区描述——内置技能保留完整，其余共享剩余——再不够就只留名称，确保技能列表不会挤占上下文空间。

> 深入阅读：[记忆系统](/lib/09-harness/how-claude-code-works/docs-08-memory-system) | [技能系统](/lib/09-harness/how-claude-code-works/docs-09-skills-system)

---

## 从最小到完整

看完上面的介绍，你可能觉得构建一个 coding agent 是一件非常复杂的事。其实核心概念很简单，一个最小可用的 coding agent 只需要 7 个组件：

1. Prompt Orchestration — 运行时组装环境信息、git 上下文和项目规则
2. Tool Registry — JSON Schema 声明加 switch/case 分发
...
> 深入阅读：[最小必要组件](/lib/09-harness/how-claude-code-works/docs-13-minimal-components)

---

## 核心架构图

下图展示了 Claude Code 各模块的关系。绿色的 Agent Loop 是整个系统的枢纽，连接着蓝色的上下文压缩、红色的安全系统、橙色的记忆系统和紫色的 Hook 系统：
