...
Claude Code 是一个生产级系统，512K+ 行代码覆盖了从 OAuth 到 MCP 到 Vim 模式的方方面面。如果你试图通过阅读全部源码来理解 coding agent 的本质，你会迷失在大量的边界情况处理、UI 优化和平台适配代码中。这就像想通过研究波音 747 的全部蓝图来弄懂"飞行"原理：你需要先掌握伯努利方程和四个基本力。

Fred Brooks 在《人月神话》中区分了本质复杂性和偶然复杂性。对于 coding agent 来说：

- 本质复杂性（essential complexity）：循环调用模型、执行工具、管理上下文——这 7 个组件是任何 coding agent 都必须解决的问题
- 偶然复杂性（accidental complexity）：MCP 协议集成、Vim 模式、OSC 8 超链接、OAuth 认证——这些是生产环境和用户体验驱动的需求
...
- 15.2.1 - 15.2.3（提示词编排、工具注册表、Agent 循环）是核心循环层——这三个组件构成了 agent 的骨架
- 15.2.4 - 15.2.6（文件操作、Shell 执行、编辑策略）是能力层——赋予 agent 具体的编程能力
- 15.2.7（CLI 交互）是交互层——让人类能够使用这个 agent
...
这里有一个容易被忽视的点：系统提示词不是一个静态文本文件，而是每次运行时临时组装出来的文档。每次启动 agent，当前目录、git 状态、项目指令都不一样，提示词只能动态生成。所以它需要的是一个 builder 函数，而不是一个常量字符串。
...
为什么要向上遍历？因为在 monorepo 里，根目录可能有全局规则，比如"所有代码用 TypeScript"，子项目目录则有更具体的规则，比如"这个包用 Vitest 测试"。`unshift` 保证祖先规则在前、子目录规则在后，后者能覆盖前者，符合直觉。
...
这些指令的实现成本为零，只是几行文本，但对模型行为的影响巨大。它们其实是在用自然语言给模型的行为编程。
...
最后是多层 CLAUDE.md 发现。生产版本不止向上遍历目录树，还会搜索 `~/.claude/` 目录的全局指令、处理 `.claude/` 子目录的项目配置、支持 `CLAUDE.local.md` 这类不提交到 git 的本地指令。这些都由真实用户场景驱动：团队把共享规则提交到 repo，个人偏好设置留在本地文件，组织级规范落在用户目录。
...
这三个问题藏着一条有趣的演进规律：在最小实现里，工具是数据，也就是 JSON 对象加 switch/case；到了生产系统，工具是行为，也就是带方法的类实例。这个从"数据"到"行为"的转变，对应着一条通用的软件成熟规律：一个实体要关联的行为超过 5-8 个时，就该从数据结构升级成对象。

#### 最小实现如何工作

最小版本的工具系统分为两部分：定义和执行，共 326 行。
...
这一步演进看着繁琐，其实是三个生产需求逼出来的。

第一个需求是安全分类。`isReadOnly()`、`isConcurrencySafe()`、`needsPermission()` 三个方法各管一层安全判断：`isReadOnly()` 决定能否跳过权限检查，`isConcurrencySafe()` 决定能否和其他工具并行执行，`needsPermission()` 决定要不要弹出确认对话框。在最小版本里，所有工具串行执行、统一检查权限，用不上这些区分；可一旦工具有数十个、又想要高性能，这些分类就变得关键。

第二个需求是 fail-closed 默认值。Claude Code 的 `buildTool()` 工厂函数给新工具设了保守的默认值：
...
第三个需求是并发工具编排。Claude Code 的 `partitionToolCalls()` 函数（在 `toolOrchestration.ts` 里）实现了一套优雅的并发策略：
...
此外，Claude Code 还有 ToolSearch 延迟加载机制。几十个工具全塞进系统提示词会太费 token，于是把不常用的工具标记为 `shouldDefer`，交给一个专门的 ToolSearch 工具按需发现。这就像操作系统的动态链接：库用到时才加载，不必一次性全进内存。
...
`chatAnthropic()` 方法是整个最小实现的核心，逐段走查：
...
1. 流式和最终消息分离：`stream.on("text")` 负责实时显示，是给人看的；`stream.finalMessage()` 拿到完整响应，是给后续代码用的。
...
可重试的错误码是精心挑的：429 是速率限制，503 是服务不可用，529 是 API 过载。这三个都是临时性错误，等一等通常就能恢复。而 400 是请求格式错误、401 是认证失败，都不重试——它们是永久性错误，重试没有意义。
...
这个策略有三个关键点。它保留最后一条用户消息，确保当前任务不丢失；用合成的 user-assistant 对话对替换历史，保持 Anthropic API 要求的消息交替格式；总结指令还要求"preserve key decisions, file paths, and context"，这些都是继续工作所必需的信息。

#### Claude Code 的做法与为什么

Claude Code 的 `query()` 是一个 1,728 行的异步生成器。它之所以这么大，不是因为代码写得冗余，而是因为每一段代码都对应一种在生产环境中发现的真实故障场景。

7 个 Continue Sites：这个循环里有 7 个不同的"重新进入点"，而不是一个 `while(true)` 从头跑到尾。遇到不同类型的错误，恢复策略也不同：
...
这最后一点——错误扣留（error withholding）——是一个特别聪明的设计。当一个工具执行失败时，错误信息不直接展示给用户，而是作为工具结果反馈给模型。模型经常能够自己修复问题——比如换一个文件路径、修改命令参数、或者尝试不同的方法。只有模型无法自修复的错误才最终呈现给用户（详见[第 2 章 系统主循环](/lib/09-harness/how-claude-code-works/docs-02-agent-loop)）。
...
文件操作是 coding agent 的"眼睛"。一个读不了代码的 agent，就像闭着眼睛的程序员：推理能力再强，也没法有效干活。
...
FileReadTool 的多格式支持：生产版本不只读文本文件，还支持三类内容——图片会 base64 编码后作为多模态内容发给模型，PDF 会提取指定页面的文本，Jupyter Notebook 会解析 JSON 结构展示 cell 内容。为什么要支持图片？因为调试 UI 问题时，"看一眼截图"是最自然的动作。模型本来就是多模态的，限制它只吃文本是白白浪费。

GrepTool 基于 ripgrep：它比系统 grep 快 10-100 倍，代码库越大差距越明显；它默认尊重 `.gitignore`，自动排除构建产物和依赖，还支持更丰富的正则语法。对于需要频繁搜索的 coding agent，这点性能差异直接影响体验。
...
但它同时也是最危险的。一个能执行任意 Shell 命令的程序，等于握有用户的全部权限。`rm -rf ~` 能删掉用户的所有文件，`curl ... | bash` 能执行任意远程代码。这里有一个根本性的张力：能力要拉满，风险要压到最低，两者直接冲突。
...
这 10 个模式覆盖了最常见的危险操作。注意 `\b` 单词边界的用法——`/\brm\s/` 匹配 `rm -rf`，却不会误伤 `perform`。但正则方式有明显的局限：`r''m -rf /` 靠引号打断、`$(rm -rf /)` 靠命令替换、`echo rm | bash` 靠间接执行，都能绕过检测。这在最小版本里可以接受——用户确认是第二道防线，而且最小版本的用户群通常就是开发者自己。
...
Claude Code 的 Shell 安全系统在整个项目里数一数二复杂，因为它面对的是一个开放式的攻击面——Shell 语法几乎无限灵活。
...
这是 base64 编码的 `rm -rf /`，正则完全无法检测。AST 分析可以识别 `eval` + 命令替换的模式，将其标记为潜在危险（详见[第 12 章 权限与安全](/lib/09-harness/how-claude-code-works/docs-11-permission-security)）。
...
- `count > 1`，也就是匹配到多处：说明 `old_string` 不够具体，无法确定改哪一处。例如 `old_string: "return null"` 可能在文件里出现 5 次。强制唯一性会逼模型提供更多上下文，比如带上函数签名和周围几行代码，这反而提高了编辑精度。
- `count === 1`：唯一匹配，安全替换。
...
`replace_all` 选项：当你要把一个变量从 `oldName` 改成 `newName`，它可能在文件里出现 20 次，这时唯一性约束反而碍事。`replace_all: true` 放松了唯一性要求，允许批量替换。这是一种"平时限制严格、特殊情况留个逃生口"的设计。

`readFileState` 集成：Claude Code 维护了一个文件读取状态缓存。模型调用 `edit_file` 时，系统会检查这个文件读过没、读过之后有没有被外部改动过。如果模型想编辑一个从没读过的文件，也就是盲改，系统直接拒绝；如果文件读过之后又被改动，系统会警告。这两个检查拦下了盲改和读后失效这两类最常见的编辑错误，是从最小版本迈向生产版本时最值得补的一项增强。

Diff 生成和彩色显示：每次编辑后，Claude Code 会生成并显示一个彩色 diff，删除的行标红、新增的行标绿。这不改变功能，但用户能一眼看到"agent 改了什么"，信任感随之上来，不必自己去对比文件。透明度是用户信任 agent 的关键（详见[第 10 章 代码编辑策略](/lib/09-harness/how-claude-code-works/docs-05-code-editing-strategy)）。
...
一次性模式，比如敲 `mini-claude "fix the bug in app.ts"`，适合脚本集成和快速任务；REPL 模式，也就是直接敲 `mini-claude`，适合探索性工作和多轮对话。两种模式共享同一个 Agent 实例，区别只在输入来源。

Ctrl+C 处理是 agent CLI 里最讲究的一处交互设计：
...
React + Ink 终端渲染器：Claude Code 用 React 组件来构建终端 UI。这看起来像过度工程，其实有个合理的理由：agent UI 天生是高度有状态的。同一时刻可能有好几件事在并行：流式文本在输出、工具调用进度在更新、权限确认弹窗在等响应、状态栏在刷新 token 计数。用 `console.log` 去管这些并发的界面更新，很快就写成意大利面。React 的声明式模型——"给定这些状态，UI 就该长这样"——让复杂的界面状态管理重新变得可维护。
...
为什么从这三个工具开始？因为它们覆盖了 agent 的基本操作周期：用 `read_file` 理解现状，用 `run_shell` 执行命令（测试、编译、git），用 `write_file` 创建或修改文件。至于 `edit_file`（search-and-replace）、`grep_search`、`list_files`，都是"更好的方式"去做 `read_file` + `run_shell` 已经能做的事。
...
多 Agent（AgentTool）：将复杂任务分解给子 agent 并行执行。主 agent 负责规划和协调，子 agent 负责具体执行。这是处理大型项目级任务的关键架构（详见[第 8 章 多 Agent 架构](/lib/09-harness/how-claude-code-works/docs-07-multi-agent)）。
...
定制建议：如果你想基于这个最小实现构建自己的 agent，系统提示词模板 `src/system-prompt.md` 就是杠杆最高的定制点。改行为指令、加领域知识、调工具使用偏好——这些零成本的文本改动，就能实实在在地改变 agent 的行为方式。
...
1. 循环才是核心：Agent 的价值不在单次调用，而在持续的工具循环。最小版本的 `while (true)` 循环只有 15 行，却是整个系统的心脏。从 15 行长到 1,728 行，多出来的每一段都在防御一种真实故障场景——每段新增代码都对应一个"在生产环境中发现的、导致 agent 卡死或崩溃的问题"。

2. 编辑策略决定可用性：选 search-and-replace 而非行号编辑，真正的考量是它贴合模型的认知方式，而非实现难易。模型"思考"的单位是文本内容，让它指定"改什么"比指定"改哪里"更可靠。唯一性约束再加一道安全：宁可拒绝一次编辑，也不要改错位置。

3. 上下文管理决定上限：没有压缩系统的 agent，对话长度卡在 ~200K tokens 的硬上限内，大约 20-30 轮复杂的工具调用就到头了。生产级压缩能让对话几乎无限延续。这是玩具和工具的分界线：前者只能处理简单任务，后者扛得下复杂项目。

4. 安全不是可选的：在用户环境里执行代码，安全是前提。即使最小版本也不该省掉基本的危险命令确认——10 个正则加用户确认只要 30 行代码，就能挡住灾难性的误操作。从正则到 AST 分析，这一步防的是更聪明的攻击向量，而非单纯把工程做漂亮。

5. 体验是乘数效应：同样的模型能力，流式输出让等待感消失，彩色终端让信息层次分明，进度提示让用户安心。这些不改变 agent 的实际能力，但改变用户对它的信任度和使用意愿。一个用户不信任的 agent，无论多强大都不会被使用。

6. 从数据到行为的演进：最小版本的工具是 JSON 对象，也就是数据；生产版本的工具是类实例，也就是行为。这是一个自然的软件成熟过程——当一个实体要关联的行为超过 5-8 个，验证、权限、并发声明、UI 渲染、动态提示词接连找上来，它就从数据结构演进成对象。识别这个"演进时机"，是软件设计里数一数二重要的判断力。
...
上一章：[用户体验设计](/lib/09-harness/how-claude-code-works/docs-12-user-experience) | 返回：[快速入门](/lib/09-harness/how-claude-code-works/docs-quick-start)
