...
- 自研 React 终端渲染器：Claude Code 的终端 UI 复杂度远超普通 CLI——权限确认对话框、流式代码高亮、嵌套工具进度指示器都需要组件化的状态管理。团队维护了一个 ~1.0MB 的定制 Ink 渲染器（而非使用上游库），详见 [第 14 章：用户体验设计](/lib/09-harness/how-claude-code-works/docs-12-user-experience)。
...
> 上图是 Bash 命令的核心决策路径（一个代表性切片）。完整的权限系统共 7 层——在上述环节之外还有工作区信任确认、权限模式、沙箱与隔离等——完整模型见[第 12 章](/lib/09-harness/how-claude-code-works/docs-11-permission-security)。
...
下一章：[系统主循环](/lib/09-harness/how-claude-code-works/docs-02-agent-loop)
