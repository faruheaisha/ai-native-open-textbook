...
本章覆盖 Claude Code 终端 UI 的完整技术栈，围绕三层核心展开：
...
- 交互层（14.4–14.8）：工具调用透明度、错误自动恢复、键盘快捷键、Vim 模式，以及带虚拟滚动、权限防误触、会话恢复的 REPL 主界面

此外还涵盖终端协议支持、诊断界面、成本追踪、搜索与文本选择四部分（14.9–14.12），最后在 14.13 提炼设计洞察。
...
- 实时可见：所有工具调用流式展示，你能在执行过程中看到参数和进度，而不是等执行完才拿到结果。好处是用户可以在 Agent 走错方向的前 3 秒就按 Ctrl+C 中断，不必等 20 秒执行完再撤销——中断成本远低于撤销成本。
- 最小化打断：只在真正需要权限确认时才中断用户流。权限弹窗甚至有 200ms 防误触延迟（14.8 节详述），可见团队对"中断的代价"有多在意。
- 流式输出支持决策：用户边看流式输出边判断方向是否正确。如果模型输出了 3 秒发现方向不对，立即 Ctrl+C 可以节省剩余 15 秒的生成时间和 Token 成本。

可以用一句话总结这个哲学：**信任但实时验证**——给 Agent 充分的行动自由，但让每一个操作都在玻璃箱里进行、看得见，而不是关进黑箱。
...
4. 复用 React 生态。React 的 Hooks（`useState`、`useMemo`、`useEffect`）、用于全局状态共享的 Context、避免重复渲染的 Memo，这些年积累的优化模式直接可用，团队不需要为终端场景重新发明状态管理方案。
...
- Blitting 优化：更进一步，对于前一帧中完全没变化的连续行，直接从旧的 Screen Buffer 复制（blit），跳过 cell 级别的比较。这在大量静态内容加少量动态内容（比如流式输出末尾几行在增长）的场景下能省掉大部分逐 cell 比较。
...
`useBlink(enabled)`（`src/hooks/useBlink.ts`）：所有闪烁的元素——比如多个正在执行的 ToolUseLoader——天然同步，因为它们用同一个数学公式从共享时钟推导状态：
...
以 `useInput()` 为例，它处理原始键码解析，包括 Escape 序列和 Kitty 扩展键码，再根据当前模式（Normal/Vim/搜索）把键盘事件分发到正确的处理器。开发者只需关心"按下了什么键"和"当前在什么模式"，不需要了解底层终端协议的细节。

## 14.3 流式输出

Claude Code 的流式输出是真正的实时流式渲染，不必等整段生成完才显示。
...
- 与 `async function*` 的配合：`Stream` 负责推式（push）生产的场景，比如 SSE 回调由 API 决定何时 push 数据。`async function*` 生成器则负责拉式（pull）的管道阶段，由消费者决定何时拉取下一个值。两者在 `callModel()` 层连接：SSE 回调 push 到 Stream，`callModel()` 的 `for await...of` 从 Stream 拉取，再 yield 给上层。
...
流式输出面临一个性能挑战：模型每输出一个 Token，累积文本就增长一点。如果每次 delta 都对全量文本重新跑一遍 `marked.lexer()` 做 Markdown 解析，一个 10KB 的响应就意味着数千次 O(n) 的完整解析，足以造成明显卡顿。

`StreamingMarkdown`（`src/components/Markdown.tsx`）的办法是：在最后一个顶层 block 边界处切分，前面已经稳定的部分不再重新解析。
...
这样一来，用户不需要阅读任何文字就能感知"有点不对劲"——Spinner 从正常颜色逐渐变红，潜意识里就收到了"可能卡住了"的信号。如果终端不支持 RGB，则在 `stalledIntensity > 0.5` 时离散跳变为 error 颜色。
...
当多个工具并行执行时，所有"正在执行"的 ToolUseLoader 圆点会同步闪烁——它们在同一瞬间亮起或熄灭。这里并没有一个"闪烁协调器"在统一它们，而是靠 `useBlink` Hook 的数学同步（详见 14.2 节）。同步闪烁给用户一种"系统在统一运作"的感觉，比各自独立闪烁更有秩序感。
...
后台查询——摘要生成、标题建议、命令补全建议——在收到 529 时立即放弃，不重试。理由：
...
这是一种负载感知的重试策略——系统健康时全量重试，容量紧张时只保护最关键的查询路径。
...
10 分钟的最小冷却期（`MIN_COOLDOWN_MS`）是为了防止模式翻转：冷却期太短的话，系统会在快速和标准模式之间反复横跳，每次切换都丢一次 prompt cache，反而更慢。
...
`useVirtualScroll`（`src/hooks/useVirtualScroll.ts`）的方案是：只挂载视口可见范围加上下缓冲区内的消息，其余用空白 Spacer 占位保持滚动高度。
...
终端 resize 时并不清空缓存重新测量（那会导致约 600ms 的渲染高峰——190 个新挂载 × 3ms/个），而是按列数比例缩放已缓存的高度。缩放值不完全精确，但在下一次 Yoga 布局时会被真实高度覆盖。

### 权限确认的 200ms 防误触

`PermissionRequest` 弹出后，前 200ms 有一个 grace period（`interactiveHandler.ts` 的 `GRACE_PERIOD_MS = 200`）会忽略用户交互。

场景：用户正在快速输入一段话。Agent 此时决定执行一个 Bash 命令，弹出了权限确认对话框。此时安全分类器可能正在后台判定这个操作能否自动放行（auto-approve）。如果对话框一弹出就立刻响应按键，用户手上残留的击键（本意是继续打字）会被当成「用户已介入」，从而把正在跑的分类器/自动判定提前取消掉。grace period 的作用就是：弹窗后 200ms 内的交互一律不算数（不置 `userInteracted`、不清除分类器指示），等这段窗口过去再开始接受输入。
...
模型级别汇总：使用 `getCanonicalName()` 将不同日期后缀的模型 ID（如 `claude-sonnet-4-20250514`、`claude-sonnet-4-20250601`）归一化为同一个短名称，按短名称汇总显示。这样用户看到的不是一堆 API 版本号，而是清清楚楚的"哪个模型花了多少钱"。

### 缓存命中的意义

输出里 `cache read` 和 `cache write` 两个指标直接反映成本优化效果：

- cache_read_tokens 的成本只有正常 input tokens 的 1/10。长对话里，系统提示词和早期对话历史会被 API 缓存，命中缓存时这部分 Token 只按十分之一计费。
- 高 cache_read、低 cache_write 说明缓存效率好：prompt 结构稳定，缓存反复命中，成本被压下来了。
- 高 cache_write、低 cache_read 说明缓存频繁重建：可能是上下文变化太频繁（如每轮都有大量新工具结果），缓存来不及命中就被更新了。
...
1. React in Terminal 不是玩具：251KB 的自研 Ink 渲染器证明了终端 UI 可以做到 Web 级别的交互体验。但真正的价值不在渲染本身，而在开发效率——新功能（Shimmer 动画、虚拟滚动、搜索高亮）可以从现有 React 原语组合而成，不需要触碰渲染管线。

2. 流式是用户体验的核心：实时看到模型思考过程，比等 10 秒看到完整结果更好。`StreamingMarkdown` 的增量解析只 memoize 稳定前缀、只重新解析末尾 block，可见团队对这一点的用心——不只是把字符一个个打出来，而是构建了完整的增量渲染管线，让流式保持 O(delta) 而非 O(total)。

3. 工具透明度建立信任：每个工具自带 4 种渲染方法（调用/结果/拒绝/错误），意味着所有可能的状态都被显式设计，不会退化成一个 "Something went wrong" 的通用错误页。用户能看到每一步操作，才愿意给 Agent 更多权限。

4. 自动恢复减少干扰：但不是"重试一切"——前台/后台查询的重试区分表明这是有意识的设计。在容量紧张时，后台查询主动放弃以减少级联放大，把资源留给用户正在等待的前台查询。这是负载感知的降级策略，而非天真的"出错就重来"。

5. 渲染与逻辑耦合：每个工具自带渲染方法，确保展示与行为一致。新增工具时开发者被迫思考"这个工具的每种状态应该怎么展示给用户"，而不是事后补一个通用的展示。

6. 动画即信息：Spinner 从主题色渐变为红色来指示停滞、Shimmer 用 50ms 和 200ms 两档速度分别表示请求中和思考中、多个 ToolUseLoader 同步闪烁——这些都不是装饰性动画。每个动画都编码了系统状态，用户会在潜意识里学会读取这些视觉信号，不用看文字说明就知道系统在做什么。

7. 非对称误差预算：虚拟滚动的多个常量都朝优雅降级的方向取舍——`DEFAULT_ESTIMATE=3` 宁可低估、多挂载几项，也不高估、露出空白；`PESSIMISTIC_HEIGHT=1` 同理，宁可多挂载也不挂载不足；`OVERSCAN_ROWS=80` 宁可多缓冲，也不让快速滚动看到空白。拿不准时，总是选择多做一点无用功，而不是让用户看到瑕疵。
