...
>
> Concretely: when your query says "Iteration &#123;&#123; iteration &#125;&#125; evaluation completed", it means the eval of **iteration &#123;&#123; iteration - 1 &#125;&#125;'s changes** is done (baseline if <code v-pre>{{ iteration }}</code> = 1). You are now making changes that will be labeled iteration <code v-pre>{{ iteration }}</code> and evaluated next loop.
...
The `iteration` field below MUST be <code v-pre>{{ iteration }}</code> (the current loop — the one PRODUCING these changes). Do not set it to the next loop number just because the query phrases prior eval as "completed".
...
Run after all changes: <code v-pre>python evolve_agent/skills/nexau-evolution-guide/scripts/validate_agent.py {{ ws }}/code_agent.yaml</code>
