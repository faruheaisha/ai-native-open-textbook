---
title: "Agent Note: Auto-title on by default, re-derived on resume"
...
entryUrl: "https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/.agents/notes/archived/feature/2026-07-21-tui-auto-title-default-on.md"
sourceRel: ".agents/notes/archived/feature/2026-07-21-tui-auto-title-default-on.md"
rawUrl: "/raw/09-harness/deepseek-harness/.agents/notes/archived/feature/2026-07-21-tui-auto-title-default-on.md"
sourceSha256: "0caad9db58e031f9f667e93a3f53ebcf3c1f0700efc6decd51acafa3657372ac"
pageSha256: "0caad9db58e031f9f667e93a3f53ebcf3c1f0700efc6decd51acafa3657372ac"
...
English | [中文](/lib/09-harness/deepseek-harness/_agents-notes-archived-feature-2026-07-21-tui-auto-title-default-on.zh)

> **Superseded** by the [session-title consolidation Agent Note](https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/.agents/notes/archived/simplification/2026-07-22-tui-titles-from-session-title-service.md): the TUI-local `autoTitle` generation is removed; titles come from the log-backed session-title service, and the terminal rename consumes `session/title` events.

## Problem

The [auto-title Agent Note](/lib/09-harness/deepseek-harness/_agents-notes-archived-feature-2026-07-21-tui-auto-pane-title) shipped `autoTitle` off by default and, on a resumed session, kept the static title because the first `user/message` was already logged. In use both choices defeated the feature's purpose. A per-session descriptive pane title is what makes one tmux pane or terminal tab distinguishable from the next; leaving it off by default means the product ships an inert feature that almost no user turns on, and skipping re-derivation on resume means a resumed session — exactly the long-lived session most worth labelling — falls back to the shared static string. The user asked for a descriptive per-session name to be the normal experience.
...
- Everything else from the [auto-title Agent Note](/lib/09-harness/deepseek-harness/_agents-notes-archived-feature-2026-07-21-tui-auto-pane-title) stands unchanged: the OSC 0 `runtime.terminal.setTitle` path, the model-summary shape (two-to-five lowercase words, first non-empty line, 40-char cap), the fire-and-forget `ctx.llm.stream` call that never touches the session or transcript, the shutdown `AbortController`, and every failure fallback (empty reply, missing `llm`, missing provider/model, whitespace-only prompt).

## Alternatives considered

**Keep the feature off by default.** Rejected: this is a direct reversal of the [auto-title Agent Note](/lib/09-harness/deepseek-harness/_agents-notes-archived-feature-2026-07-21-tui-auto-pane-title)'s "default off" decision at the user's request. Off-by-default ships an inert feature; the descriptive name is only useful if it is the normal experience. The keyless-replay concern that motivated off-by-default is addressed by pinning `autoTitle: false` in the replay-backed snapshot scenarios rather than by suppressing it for every deployment.

**Persist the derived title in the session header.** Rejected: the header has no title field and adding one would make a terminal label into session metadata — the boundary the [auto-title Agent Note](/lib/09-harness/deepseek-harness/_agents-notes-archived-feature-2026-07-21-tui-auto-pane-title) already drew against the log-backed session-title work. Re-deriving from the stored first message costs one tool-less call on resume and keeps the label a pure function of the conversation.
