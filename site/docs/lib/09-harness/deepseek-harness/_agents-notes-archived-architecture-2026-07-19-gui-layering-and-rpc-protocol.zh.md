---
title: "Agent Note: GUI 分层与 RPC 协议——host/client 按能力提供方分层、四象限消息模型与 fetch 载体"
...
entryUrl: "https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/.agents/notes/archived/architecture/2026-07-19-gui-layering-and-rpc-protocol.zh.md"
sourceRel: ".agents/notes/archived/architecture/2026-07-19-gui-layering-and-rpc-protocol.zh.md"
rawUrl: "/raw/09-harness/deepseek-harness/.agents/notes/archived/architecture/2026-07-19-gui-layering-and-rpc-protocol.zh.md"
sourceSha256: "8181386d957fa6d6b3eb9b05d29adb10804b5a926853425415d368cc7fceaefa"
pageSha256: "8181386d957fa6d6b3eb9b05d29adb10804b5a926853425415d368cc7fceaefa"
...
[English](/lib/09-harness/deepseek-harness/_agents-notes-archived-architecture-2026-07-19-gui-layering-and-rpc-protocol) | 中文

> 分工线：本篇 = 分层模型 + 通道无关的 RPC 协议；协议的 Web 实现由 HTTP 上行加 [WebSocket 下行载体](/lib/09-harness/deepseek-harness/_agents-notes-archived-architecture-2026-08-04-websocket-downlink-carrier.zh)组成，浏览器对象层见 [Web 客户端架构笔记](https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/.agents/notes/archived/architecture/2026-07-19-gui-web-client-architecture.zh.md)。
...
- `packages/client/*`：包只提供 Client 侧能力，每包单边不混。这里住三类包（两条轴归 [client 插件装载笔记](https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/.agents/notes/archived/architecture/2026-07-23-client-plugin-loading-model.zh.md) 所有）：
...
- `apps/` 作为对外导出的应用入口，可以由 Client / Host 混合组装。
    ...
    - `apps/cli`（`@deepseek-ai/dsh`）分发命令：`dsh web` = Host + webserver + 构建出的 `dsh-web-frontend` dist；`dsh --profile headless` = [直接使用核心 Agent／Session 的入口](/lib/09-harness/deepseek-harness/_agents-notes-archived-architecture-2026-08-09-headless-direct-core-entry-point.zh)，不含 Host、HTTP 或浏览器层。
...
- client 侧跨包 import 插件包一律走 `/client` 子路径，且插件包之间只限类型 import——跨插件值 import 在 tsdown 纯度门禁处即构建错误（值层面的协作走 cordis 服务；边规则归 [client 插件装载笔记](https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/.agents/notes/archived/architecture/2026-07-23-client-plugin-loading-model.zh.md) 所有）。

TypeScript 以 solution 根引用的**两个聚合 program** 检查（`tsconfig.json` = solution；`tsconfig.host.json` = host 侧 + 测试，排除 `packages/client`；`tsconfig.client.json` = client 各包及其测试）：两侧在相同键（`sessions`、`loader`）下以不同服务合并 cordis `Context` 接口，单一 program 会同时看到两份声明合并而报冲突。共享叶子包（session/llm/tools/apiproxy 等）只构建一次，由两个 program 共同引用（[拓扑](https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/.agents/notes/archived/process/2026-07-22-tsconfig-solution-root-two-aggregates.zh.md)）。
...
域接口签名只感知窄形：`RpcRequest = \{ rpcId, payload \}`、`RpcResponse<T> = \{ rpcId, result: RpcResult<T> \}`。载体层把窄形补全为全形（补 `type` tag 与 `method`），方向不靠通道推断。`RpcResult<T> = \{ ok: true; value \} | \{ ok: false; error: RpcError \}`——方法不 throw 业务错误。
...
两条逻辑流：mux 流（`/api/events.mux`，全 session 聚合）与 host 流（`/api/events.host`，host 级事件）。浏览器通过每流一条下行 WebSocket 消费，进程内 fetch 载体以 SSE 保持同构；物理边界见 [WebSocket 下行载体](/lib/09-harness/deepseek-harness/_agents-notes-archived-architecture-2026-08-04-websocket-downlink-carrier.zh)。帧示例一行：
...
| `WebApiClient` | dsh-client-connection | `globalThis.fetch` 上行 + 每逻辑流一条同源 WebSocket 下行 | 浏览器客户端；物理边界见 [WebSocket 下行载体](/lib/09-harness/deepseek-harness/_agents-notes-archived-architecture-2026-08-04-websocket-downlink-carrier.zh) |
