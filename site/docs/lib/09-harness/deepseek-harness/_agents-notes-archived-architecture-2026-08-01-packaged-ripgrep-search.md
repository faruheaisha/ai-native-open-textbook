---
title: "Agent Note: Packaged ripgrep spawn for glob/grep"
...
entryUrl: "https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/.agents/notes/archived/architecture/2026-08-01-packaged-ripgrep-search.md"
sourceRel: ".agents/notes/archived/architecture/2026-08-01-packaged-ripgrep-search.md"
rawUrl: "/raw/09-harness/deepseek-harness/.agents/notes/archived/architecture/2026-08-01-packaged-ripgrep-search.md"
sourceSha256: "36cf2d6daa654abced690965338aec4628069dc77be3a1da6368a9b097a59ff4"
pageSha256: "36cf2d6daa654abced690965338aec4628069dc77be3a1da6368a9b097a59ff4"
...
English | [中文](/lib/09-harness/deepseek-harness/_agents-notes-archived-architecture-2026-08-01-packaged-ripgrep-search.zh)

> Supersedes [bash-backed grep/glob discovery](/lib/09-harness/deepseek-harness/_agents-notes-archived-feature-2026-07-09-bash-backed-grep-glob-discovery): the v1 decision's explicitly deferred alternative — directly spawning ripgrep — is now what ships.

## Problem

The `glob`/`grep` tools ran through the bash executor seam, which made a system `rg` install a host dependency. On Windows and container images there is no `rg` on `PATH` by default, so the tools silently vanished there; a deployment could only discover that from the load-time probe warning. The bash seam also forced the whole model-visible argument surface through one shell-quoting helper, because a shell sat between the tool and ripgrep — the [bash-backed note](/lib/09-harness/deepseek-harness/_agents-notes-archived-feature-2026-07-09-bash-backed-grep-glob-discovery) recorded that coupling as the v1 trade-off and named direct spawn as the reasonable follow-up if the shell-string domain ever proved too sensitive. It did: every model value had to survive POSIX single-quoting, the probe had to be scripted in tests, and the executor's own timeout classification duplicated what the cooperative tool-timeout policy already owns.
...
- The discovery tools work on every platform the packaged binary covers (darwin/linux/win32, x64/arm64) with no host install; the shipped TUI/Web rosters gain `glob`/`grep` as fixed members ([even-out-shipped-tool-rosters](https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/.agents/notes/archived/feature/2026-07-31-even-out-shipped-tool-rosters.md)).
