---
title: "Agent Note: 由 Bash 支持的 grep 与 glob 发现工具"
...
entryUrl: "https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/.agents/notes/archived/feature/2026-07-09-bash-backed-grep-glob-discovery.zh.md"
sourceRel: ".agents/notes/archived/feature/2026-07-09-bash-backed-grep-glob-discovery.zh.md"
rawUrl: "/raw/09-harness/deepseek-harness/.agents/notes/archived/feature/2026-07-09-bash-backed-grep-glob-discovery.zh.md"
sourceSha256: "52eec49e454357a7e0c3f27c8aaa63840b5822b5d103f49ed48a5c4f25081b00"
pageSha256: "52eec49e454357a7e0c3f27c8aaa63840b5822b5d103f49ed48a5c4f25081b00"
...
[English](/lib/09-harness/deepseek-harness/_agents-notes-archived-feature-2026-07-09-bash-backed-grep-glob-discovery) | 中文
...
`globMaxResults` 和 `grepMaxMatches` 使用 `ItemRetainer(\{ kind: 'head' \})`。`grepMaxLineBytes` 针对每条匹配行使用 `TextRetainer(\{ kind: 'head', maxBytes: grepMaxLineBytes \})`，使预览截断保留 UTF-8 边界。这遵循[工具结果保留库](/lib/09-harness/deepseek-harness/_agents-notes-archived-architecture-2026-07-06-tool-result-retention-library)对发现条目的映射：收集完整结果，在内联结果中保留头部条目，并将路径映射、分组和逐行预览放在保留器外部。v1 的 `grep` 不公开 `case_insensitive`、`head_limit`、`offset`、`count`、多行、上下文行、输出模式或文件类型过滤器。模型如需周边上下文，可使用 `read` 读取匹配文件；如需后续结果，则遵循返回的落盘定位符所给出的检索提示。
