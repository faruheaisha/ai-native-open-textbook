---
title: "Agent Note: Result-time applied-hunk diffs for file mutations"
...
entryUrl: "https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/.agents/notes/archived/architecture/2026-07-02-result-time-applied-hunk-diffs.md"
sourceRel: ".agents/notes/archived/architecture/2026-07-02-result-time-applied-hunk-diffs.md"
rawUrl: "/raw/09-harness/deepseek-harness/.agents/notes/archived/architecture/2026-07-02-result-time-applied-hunk-diffs.md"
sourceSha256: "537f57877cc2e86b2d3a8a482043db8d7a51880159b1077c5a45b89cd5d0114c"
pageSha256: "537f57877cc2e86b2d3a8a482043db8d7a51880159b1077c5a45b89cd5d0114c"
...
English | [中文](/lib/09-harness/deepseek-harness/_agents-notes-archived-architecture-2026-07-02-result-time-applied-hunk-diffs.zh)

## Problem

The [tagged render-intent union](https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/.agents/notes/archived/architecture/2026-07-02-tool-render-intent-union.md) gives `dsh-tool-fs` write/edit a `card:'diff'` at call time, derived purely from the tool's args: write ⇒ `\{oldText:null, newText:content\}` (the whole new file), edit ⇒ `\{oldText:old_string, newText:new_string\}` (the bare replaced snippet). A UI can render that as an inline diff, but it is a **context-free** diff — the bare `old_string`→`new_string` with no surrounding lines, and a `replace_all` that touched five scattered sites still renders as one snippet pair.
...
Per the [capability-seam split](https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/.agents/notes/archived/architecture/2026-06-13-capability-seams.md), the storage backend returns only **storage facts** and the model-facing tool owns **presentation**:
...
`ToolResultView` includes `DiffResultView \{ card:'diff'; title?; diffs: FileDiff[] \}`. TUI and JSON-RPC/Web consumers switch on the same tagged view and replace the pending call's context-free snippet with the applied result hunk. The [automation-only ACP bridge](https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/.agents/notes/archived/simplification/2026-07-23-acp-automation-only-protocol.md) does not carry tool presentation.
...
- Completes the one remaining representation difference named as a non-goal in [Tagged render-intent union](https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/.agents/notes/archived/architecture/2026-07-02-tool-render-intent-union.md) — that Agent Note's Non-goals section is updated to record that applied-hunk diffs shipped here.
- Builds on the [filesystem capability seam](/lib/09-harness/deepseek-harness/_agents-notes-archived-architecture-2026-06-17-filesystem-capability-seam) (the before/after are storage facts the backend returns) and [event-sourced sessions](https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/.agents/notes/archived/architecture/2026-06-11-event-sourced-sessions.md) (the `meta` payload persists on the `tool/result` event, so replay reproduces the card).
