---
title: "Agent Note: subagent 列表经投影单元读取身份"
...
entryUrl: "https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/.agents/notes/archived/architecture/2026-08-06-subagent-list-identity-projection.zh.md"
sourceRel: ".agents/notes/archived/architecture/2026-08-06-subagent-list-identity-projection.zh.md"
rawUrl: "/raw/09-harness/deepseek-harness/.agents/notes/archived/architecture/2026-08-06-subagent-list-identity-projection.zh.md"
sourceSha256: "23f067d9bc31469df0f4445592678df075fb1d24efb22fda5965335f623cd307"
pageSha256: "23f067d9bc31469df0f4445592678df075fb1d24efb22fda5965335f623cd307"
...
[English](/lib/09-harness/deepseek-harness/_agents-notes-archived-architecture-2026-08-06-subagent-list-identity-projection) | 中文
...
根因在于 [durable-subagent-catalog 决策](/lib/09-harness/deepseek-harness/_agents-notes-archived-feature-2026-07-22-durable-subagent-catalog-and-list-agents.zh)把描述符事件（`subagent/descriptor`）定为目录的唯一持久权威，却没有为描述符读取配任何缓存层，并把逐 child 双读明确接受为「无索引的正确性基线」。[web subagent conversations](https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/.agents/notes/archived/feature/2026-07-27-web-subagent-conversations.zh.md)（#1569）已把「是不是 subagent」放进了 header（`SessionHeader.origin`），身份判定不再读日志；mode 与 label 仍然要扫。
...
- 本记录取代 [durable-subagent-catalog](/lib/09-harness/deepseek-harness/_agents-notes-archived-feature-2026-07-22-durable-subagent-catalog-and-list-agents.zh) 中列表读路径的两项设计：经 `sessionQuery.traceSession` 枚举，与逐 child 读取描述符事件（`listEvents` 加精确 `readEvent` 双读、就地诊断分类）。diagnostic 行语义保留，分类改由列表按投影值缺席与 activity 派生；描述符事件仍是 mode/label 的唯一持久权威与折叠输入，恢复鉴权与激活约定不动。属部分取代，两记录保持交叉链接。
- [session-projection RFC](https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/.agents/notes/proposed/architecture/2026-07-27-session-projection-and-command-log.zh.md) 是 registry 约定的权威，其后由 [state-and-client-views 记录](/lib/09-harness/deepseek-harness/_agents-notes-archived-architecture-2026-08-19-session-projection-state-and-client-views.zh)拆分为 host 状态与客户端视图；本记录新增客户端可见的 `subagent` 身份 unit，并经 live 与 cold 快照消费它。折叠规则只在 registry 注册一份；任何消费面都经这一份已注册 unit 计算，不存在第二份折叠逻辑。

### `subagent` projection unit

挂在现有 `subagentTiming` 旁（[projection.ts](https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/packages/subagent/subagent/src/projection.ts)、[projection-types.ts](https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/packages/subagent/subagent/src/projection-types.ts)），key 为 `subagent`。两个 unit 都提供客户端 wire view；身份 unit 在 host 状态中保留可选的包装值，并把缺席映射为客户端哨兵：
...
`listChildren`（[list-children.ts](https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/packages/subagent/subagent/src/list-children.ts)）通过 `sessionQuery.listSessions()` 取得 canonical live-preferred corpus，再把每个 listed id 与可能存在的 `ctx.sessions.get(id)` 配对；同 id 存在 live Session 时使用 live header。枚举所需全部是 header 事实：
...
- [durable-subagent-catalog 与 list_agents](/lib/09-harness/deepseek-harness/_agents-notes-archived-feature-2026-07-22-durable-subagent-catalog-and-list-agents.zh)——被本记录部分取代：描述符仍是 mode/label 的持久权威与折叠输入，取值改为共享 query corpus 上的 projection 阶梯。
- [session projections 与命令生命周期日志](https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/.agents/notes/proposed/architecture/2026-07-27-session-projection-and-command-log.zh.md)——registry 约定的权威；本记录为其新增 `subagent` 身份 unit，并消费其 live 与 cold wire 快照。
- [session projection 状态与客户端视图](/lib/09-harness/deepseek-harness/_agents-notes-archived-architecture-2026-08-19-session-projection-state-and-client-views.zh)——state/client 拆分；`subagent` 与 `subagentTiming` 都提供客户端 wire view。
- [session projections 作为必需接缝](https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/.agents/notes/archived/architecture/2026-08-19-session-projection-mandatory-seam.zh.md)——`sessionProjections` 转为必需注入；列表的错误约定随其变化（registry 缺席是激活期失败，投影错误码删除）。
- [web subagent conversations](https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/.agents/notes/archived/feature/2026-07-27-web-subagent-conversations.zh.md)——`SessionHeader.origin` 的出处（#1569），身份判定去日志化的前半步；其 history 冷读（inspect 前缀加 registry 折叠）是本记录取值阶梯的同款先例。
- [发布前可复用的 Session 准备阶段](/lib/09-harness/deepseek-harness/_agents-notes-archived-architecture-2026-08-05-session-preparation.zh)——`inspect()` 冷读与 LRU 复用；cold child 整读的成本模型建立其上。
