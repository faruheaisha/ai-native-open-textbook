---
title: "Agent Note: Branded IDs everywhere they belong"
...
entryUrl: "https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/.agents/notes/archived/architecture/2026-06-20-branded-ids.md"
sourceRel: ".agents/notes/archived/architecture/2026-06-20-branded-ids.md"
rawUrl: "/raw/09-harness/deepseek-harness/.agents/notes/archived/architecture/2026-06-20-branded-ids.md"
sourceSha256: "7f304dc81d57d9a8db598dfb4f55dcf64e56d97cb5be646e317f57838237ee87"
pageSha256: "7f304dc81d57d9a8db598dfb4f55dcf64e56d97cb5be646e317f57838237ee87"
...
English | [中文](/lib/09-harness/deepseek-harness/_agents-notes-archived-architecture-2026-06-20-branded-ids.zh)

## Problem

The harness brands `ToolCallId` (`packages/llm/llm/src/brand.ts`) and the shared agent/session `SessionId` (`packages/core/session/src/types.ts`) using `Branded<B> = string & \{ readonly [BRAND]: B \}` and the stateless `brandString<T>()` constructor from `@deepseek-ai/dsh-brand` at `packages/util/brand/` — see its [README](https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/packages/util/brand/README.md). `dsh-brand` also states the governing policy: *"Branding is for ids that cross package boundaries and could plausibly be confused; not every string needs a brand."* That policy is right; the problem is that it is only half-applied. Two gaps let a structurally-identical-but-semantically-wrong string slip through the type checker.
...
- **Other numeric ordinals** — the [Session sequence and log-offset decision](/lib/09-harness/deepseek-harness/_agents-notes-archived-architecture-2026-08-31-session-sequence-and-log-offset-brands) brands event identities and log gaps because they cross persistence and reference seams. Turn and step numbers remain plain numbers: they are payload-local ordinals and are not interchangeable with Session event positions.
