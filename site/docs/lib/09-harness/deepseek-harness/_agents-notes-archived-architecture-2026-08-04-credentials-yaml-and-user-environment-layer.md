---
title: "Agent Note: Splitting the credential store from the user environment layer"
...
entryUrl: "https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/.agents/notes/archived/architecture/2026-08-04-credentials-yaml-and-user-environment-layer.md"
sourceRel: ".agents/notes/archived/architecture/2026-08-04-credentials-yaml-and-user-environment-layer.md"
rawUrl: "/raw/09-harness/deepseek-harness/.agents/notes/archived/architecture/2026-08-04-credentials-yaml-and-user-environment-layer.md"
sourceSha256: "943dc33ac6cc47c9e6e273787fa4187bea3a5268cefc9b5fca31df417e2e8c7f"
pageSha256: "943dc33ac6cc47c9e6e273787fa4187bea3a5268cefc9b5fca31df417e2e8c7f"
...
English | [中文](/lib/09-harness/deepseek-harness/_agents-notes-archived-architecture-2026-08-04-credentials-yaml-and-user-environment-layer.zh)

## Problem

`$DSH_HOME/.env` carried two incompatible jobs. It was the writable secret store of [`credentials-local`](https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/packages/credentials/credentials-local/README.md), so no surface could hoist it into `process.env` — hoisting would make every stored key read as a read-only launch override and block rotation from the Models page. But its name and dotenv format promise an environment file, so users put non-secrets in it and those values reached nothing: a `DEEPSEEK_BASE_URL` beside a working `DEEPSEEK_API_KEY` in the same file was silently ignored, because only the credential provider read the document and it addresses credential references alone.

One file cannot be both a store the Harness owns and isolates and a layer that propagates by ordinary environment rules. The [request-level credential decision](/lib/09-harness/deepseek-harness/_agents-notes-archived-architecture-2026-07-29-request-level-llm-config-credentials) chose dotenv to match peer products' home `.env`, and the conflation was not visible until a non-secret needed the same file.
...
- Given up: a key left in `$DSH_HOME/.env` is materialized into `process.env`, so it reaches subprocesses under the [subprocess credential scrub](https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/packages/subprocess/subprocess/README.md) rather than staying inside the provider. It remains a writable fallback below `.credentials.yaml`; a secret the Harness should own and isolate belongs in the managed document, which is never materialized.
...
- The `0600` boundary still stops other OS users and not the model, unchanged by this split — the [provider README](https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/packages/credentials/credentials-local/README.md) owns that limit and the keychain-provider deferral.

## Alternatives considered

**Keep one `$DSH_HOME/.env` and teach the CLI to hoist it.** Rejected: hoisting the store is precisely what makes stored keys unrotatable, which is why [app-boot documented the exclusion](https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/packages/boot/app-boot/README.md) in the first place. The conflict is the file's two jobs, not the loader.
