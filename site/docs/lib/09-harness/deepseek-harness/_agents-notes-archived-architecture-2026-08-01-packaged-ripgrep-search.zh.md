---
title: "Agent Note: glob/grep 改用打包的 ripgrep 二进制直接 spawn"
...
entryUrl: "https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/.agents/notes/archived/architecture/2026-08-01-packaged-ripgrep-search.zh.md"
sourceRel: ".agents/notes/archived/architecture/2026-08-01-packaged-ripgrep-search.zh.md"
rawUrl: "/raw/09-harness/deepseek-harness/.agents/notes/archived/architecture/2026-08-01-packaged-ripgrep-search.zh.md"
sourceSha256: "45b2fe8e0d492eeff313d2ef3c98fd69761b3257eee3476b8bf53312e8b6095f"
pageSha256: "45b2fe8e0d492eeff313d2ef3c98fd69761b3257eee3476b8bf53312e8b6095f"
...
[English](/lib/09-harness/deepseek-harness/_agents-notes-archived-architecture-2026-08-01-packaged-ripgrep-search) | 中文

> 取代 [bash 承载的 grep/glob 发现工具](/lib/09-harness/deepseek-harness/_agents-notes-archived-feature-2026-07-09-bash-backed-grep-glob-discovery)：v1 决策中明确延期的方案——直接 spawn ripgrep——现在成为实际交付的实现。

## 问题

`glob`/`grep` 工具经由 bash 执行器 seam 运行，这使系统 `rg` 安装成为宿主依赖。Windows 和容器镜像的 `PATH` 默认没有 `rg`，工具在那里会静默消失；部署方只能从加载期探针警告里发现这一点。bash seam 还迫使整个模型可见参数面经过一个 shell 引号工具，因为工具与 ripgrep 之间隔着一层 shell——[bash 承载决策](/lib/09-harness/deepseek-harness/_agents-notes-archived-feature-2026-07-09-bash-backed-grep-glob-discovery) 把这种耦合记为 v1 的取舍，并把直接 spawn 列为 shell 字符串域一旦被证明过于敏感时的合理后续。它确实被证明了：每个模型值都要经受 POSIX 单引号转义，探针要在测试里脚本化，执行器自身的超时分类还与协作式工具超时策略已有的职责重复。
...
**改用纯 JS 的 glob/搜索引擎（如 `picomatch`/`tinyglobby`）。** 否决：[依赖替换审计](https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/.agents/notes/rejected/simplification/2026-07-26-dependency-swaps-rejected-by-nih-audit.zh.md) 已基于「不存在 glob 引擎」的证据否决过该方向；ripgrep 语义（`--sort=modified`、VCS 剪枝、JSON 传输、正则方言）就是工具约定。

## 后果

- 发现工具在打包二进制覆盖的每个平台（darwin/linux/win32，x64/arm64）上开箱即用，无需宿主安装；交付的 TUI/Web 工具清单把 `glob`/`grep` 变为固定成员（见 [拉平交付的工具清单](https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/.agents/notes/archived/feature/2026-07-31-even-out-shipped-tool-rosters.zh.md)）。
