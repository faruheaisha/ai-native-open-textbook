---
title: "Agent Note: Preset health resolves the rows it can prove will start"
...
entryUrl: "https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/.agents/notes/archived/architecture/2026-08-26-preset-health-resolves-rows.zh.md"
sourceRel: ".agents/notes/archived/architecture/2026-08-26-preset-health-resolves-rows.zh.md"
rawUrl: "/raw/09-harness/deepseek-harness/.agents/notes/archived/architecture/2026-08-26-preset-health-resolves-rows.zh.md"
sourceSha256: "750fcec1735664da2ee8eaab1f8f5121ae6398302b8e1fb3c3fc2863f8a72a3e"
pageSha256: "750fcec1735664da2ee8eaab1f8f5121ae6398302b8e1fb3c3fc2863f8a72a3e"
...
[English](/lib/09-harness/deepseek-harness/_agents-notes-archived-architecture-2026-08-26-preset-health-resolves-rows) | 中文

## Problem

名单列为健康的 preset，仍可能根本无法组装。发现过程的健康检查只证明组装能以加载器方言解析、由具名行组成，并刻意止步于此——它不解析任何插件名，也不应用任何配置。

本 note 部分取代了[损坏的 preset 是名单行](/lib/09-harness/deepseek-harness/_agents-notes-archived-bug-fix-2026-08-09-broken-preset-roster-rows.zh)：那份 note 在 Alternatives 中否决的「深度校验」正是这里落地的做法，而且原因也已移出卡片正面；它同时放宽了[插件自带内置 preset 根](/lib/09-harness/deepseek-harness/_agents-notes-archived-bug-fix-2026-08-20-plugin-owned-shipped-preset-root.zh)记录的随附名单断言。两份都已就地更新。
...
这个缺口在[仓库命名契约](/lib/09-harness/deepseek-harness/_agents-notes-archived-architecture-2026-08-11-repository-naming-contract-and-rename-ledger.zh)按预发布立场重命名包时暴露出来。仓库内的引用随之更新；写在 `<dshHome>/.agent-presets` 下的 preset 没有，于是引用 `@deepseek-ai/dsh-workspace-context` 的那一个保住了健康的卡片、保住了在选择器里的位置，直到有人切换过去才失败。引用了被后续版本改名或卸载的包，正是手写 preset 真正的腐化方式，而它恰好是这项检查排除掉的那一类。
...
**可能永远不会启动的行被跳过。** `disabled` 是[加载器唯一会插值](/lib/09-harness/deepseek-harness/_agents-notes-archived-architecture-2026-08-11-loader-entry-disabled-interpolation.zh)的条目字段：`!!js` 表达式在挂载时对加载器上下文求值，而发现过程无法仅凭文件做到。凡该字段不是缺失、null 或 `false` 的行都不做检查，被禁用的 group 连同其子行一起跳过。每个随附 preset 都用这种方式为 shell 行设门，所以这是常见形状，不是边角。
