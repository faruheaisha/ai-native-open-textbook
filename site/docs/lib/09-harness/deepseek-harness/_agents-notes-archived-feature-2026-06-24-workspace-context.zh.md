...
entryUrl: "https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/.agents/notes/archived/feature/2026-06-24-workspace-context.zh.md"
sourceRel: ".agents/notes/archived/feature/2026-06-24-workspace-context.zh.md"
rawUrl: "/raw/09-harness/deepseek-harness/.agents/notes/archived/feature/2026-06-24-workspace-context.zh.md"
sourceSha256: "2c96fd3386cf09e7e18a78bf828e21b6a8647eb875214c30883394ba03bb10ee"
pageSha256: "2c96fd3386cf09e7e18a78bf828e21b6a8647eb875214c30883394ba03bb10ee"
...
[English](/lib/09-harness/deepseek-harness/_agents-notes-archived-feature-2026-06-24-workspace-context) | 中文
...
仓库文本仍是不受信任的输入。低权威 user 角色框架、显式优先级说明和分隔符转义可以降低风险，但无法消除提示词注入。跟随候选符号链接到目标，会把该攻击面扩大至树外内容；因此，把 `ctx.fs` 限制在可信根目录内的权限与沙箱层才是真正的边界，它们让系统把工作区文件当作数据而不是权威（[跟随指令符号链接记录](https://github.com/deepseek-ai/deepseek-harness/blob/c291e7961a515f6d7af9304e7fd1d257929aef26/.agents/notes/archived/feature/2026-07-21-follow-instruction-symlinks.zh.md)负责说明残余风险）。

系统由事件驱动，而不是文件监视器驱动。除非文件系统变更通过结构化工具完成，否则编辑不会在确切的文件系统变更时刻可见；外部文件变更会在下一次成功的结构化触碰、恢复对账，或恢复被遮蔽的基线时被发现。这使设计保持确定性并且与提供方无关。

## 延后事项

从 bash 派生路径报告、递归启动扫描、文件监视器、小写默认名称、`.claude/CLAUDE.md`、`.claude/rules/*.md`、导入指令、ACP `additionalDirectories`、信任确认和模型生成摘要均延后处理。项目目录中的 `.local.` 覆盖层现已默认加载（[默认本地覆盖层记录](/lib/09-harness/deepseek-harness/_agents-notes-archived-feature-2026-07-21-local-instruction-overlay.zh)负责说明该决策）；用户全局覆盖层、目录规则系统和导入仍需要各自的优先级与信任设计。
