...
| `prUrlTemplate` | string | **New in v2.1.119.** Custom URL template for the footer PR badge; useful for GitLab, Bitbucket, or internal code-review platforms. Supports <code v-pre>{{owner}}</code>, <code v-pre>{{repo}}</code>, and <code v-pre>{{number}}</code> placeholders. |
