---
title: "Claude Code Feature Catalog"
...
entryUrl: "https://github.com/luongnv89/claude-howto/blob/97bfb0685e03112ad39845889061d02cef6e534c/CATALOG.md"
sourceRel: "CATALOG.md"
rawUrl: "/raw/09-harness/claude-howto/CATALOG.md"
sourceSha256: "956e959fdee5945003e416681751763bdb616b3cfcf63e2edd0766fedcc0d088"
pageSha256: "956e959fdee5945003e416681751763bdb616b3cfcf63e2edd0766fedcc0d088"
...
|---------|----------|----------|-------|-----------|
| **Slash Commands** | 60+ | 8 | 68+ | [01-slash-commands/](/lib/09-harness/claude-howto/01-slash-commands) |
| **Subagents** | 6 | 9 | 15 | [04-subagents/](/lib/09-harness/claude-howto/04-subagents) |
| **Skills** | 10 bundled | 6 | 16 | [03-skills/](/lib/09-harness/claude-howto/03-skills) |
| **Plugins** | - | 3 | 3 | [07-plugins/](/lib/09-harness/claude-howto/07-plugins) |
| **MCP Servers** | 1 | 4 | 5 | [05-mcp/](/lib/09-harness/claude-howto/05-mcp) |
| **Hooks** | 33 events | 11 | 44 | [06-hooks/](/lib/09-harness/claude-howto/06-hooks) |
| **Memory** | 7 types | 3 | 10 | [02-memory/](/lib/09-harness/claude-howto/02-memory) |
| **Total** | **117** | **44** | **161** | |
...
**Reference**: [01-slash-commands/](/lib/09-harness/claude-howto/01-slash-commands) | [Official Docs](https://code.claude.com/docs/en/interactive-mode)
...
> **Note**: `auto` mode requires an eligible plan, model, and provider — see [09-advanced-features/](/lib/09-harness/claude-howto/09-advanced-features/index#auto-mode). Use `bypassPermissions` only in trusted, sandboxed environments.
...
**Reference**: [04-subagents/](/lib/09-harness/claude-howto/04-subagents) | [Official Docs](https://code.claude.com/docs/en/sub-agents)
...
**Reference**: [03-skills/](/lib/09-harness/claude-howto/03-skills) | [Official Docs](https://code.claude.com/docs/en/skills)
...
**Reference**: [07-plugins/](/lib/09-harness/claude-howto/07-plugins) | [Official Docs](https://code.claude.com/docs/en/plugins)
...
**Reference**: [05-mcp/](/lib/09-harness/claude-howto/05-mcp) | [MCP Protocol Docs](https://modelcontextprotocol.io)
...
| `TaskCompleted` | Task marked complete (only fires when the todo tools are enabled — see [Hooks](/lib/09-harness/claude-howto/06-hooks#hook-events)) | Task done | Post-task processing |
| `TaskCreated` | Task created via TaskCreate (only fires when the todo tools are enabled — see [Hooks](/lib/09-harness/claude-howto/06-hooks#hook-events)) | New task created | Task tracking, logging |
...
**Reference**: [06-hooks/](/lib/09-harness/claude-howto/06-hooks) | [Official Docs](https://code.claude.com/docs/en/hooks)
...
**Reference**: [02-memory/](/lib/09-harness/claude-howto/02-memory) | [Official Docs](https://code.claude.com/docs/en/memory)
...
| **/undo** | **No longer documented** — added as an alias for `/rewind` in v2.1.108, but it appears nowhere in the official commands reference | Use `/rewind` (or `Esc Esc`) instead |
| **Monitor Tool** | Watch a background command's stdout stream and react to events instead of polling (v2.1.98+) | Use the Monitor tool via [Advanced Features](/lib/09-harness/claude-howto/09-advanced-features/index) |
| **Output Styles** | Change Claude's role, tone, and default response format via the system prompt. Built-ins: Default, Proactive, Explanatory, Learning, Concise | `/config` → Output style, or set `outputStyle`. The `/output-style` command was removed in v2.1.91. See [Advanced Features](/lib/09-harness/claude-howto/09-advanced-features/index#output-styles) |
| **Status Line** | Render a custom status line from a command that receives session, model, cost, and context JSON on stdin | `/statusline` or the `statusLine` setting. See [Advanced Features](/lib/09-harness/claude-howto/09-advanced-features/index#status-line) |
...
| **Cross-Session Messaging** | Sessions can message each other — including your other machines and cloud sessions — discovered via `ListAgents` (v2.1.224+, macOS/Linux) | See [Advanced Features](/lib/09-harness/claude-howto/09-advanced-features/index#cross-session-messaging); control inbound with `crossSessionInbound` |
| **Self-Hosted Runner** | Run Claude Code web, mobile, and desktop sessions on your own machines or containers (v2.1.224+, Team/Enterprise) | `claude self-hosted-runner setup`; see [CLI](/lib/09-harness/claude-howto/10-cli/index) |
| **`archive` plugin source** | Install a plugin from an HTTPS zip, optionally pinned by `sha256` (v2.1.224+) | See [Plugins](/lib/09-harness/claude-howto/07-plugins#archive-source-v21224) |
| **`command` plugin source** | A locally installed tool prints the plugin directory path (v2.1.229+) | See [Plugins](/lib/09-harness/claude-howto/07-plugins#command-source-v21229) |
| **Marketplace owner wildcards** | `"owner/*"` allows or blocks every marketplace repo under a GitHub owner — `strictKnownMarketplaces` and `blockedMarketplaces` only (v2.1.223+) | See [Plugins](/lib/09-harness/claude-howto/07-plugins#marketplace-configuration) |
| **Sandbox credential masking** | Sandboxed commands read a sentinel while the proxy substitutes the real secret on egress (v2.1.221+, Linux/WSL) | See [Advanced Features](/lib/09-harness/claude-howto/09-advanced-features/index#credential-masking-v21221-v21224) |
...
| **Immediate Dialog Commands** | `/permissions` can be opened while Claude is working (rule changes apply to the rest of the turn), and the `/add-dir`, `/autocompact`, `/theme`, `/help`, `/config`, and `/advisor` dialogs open mid-turn **in the fullscreen TUI** instead of queuing until Claude finishes responding (`/bug` already did this since v2.1.232) (v2.1.234) | Run one of these commands while Claude is working. See [Slash Commands](/lib/09-harness/claude-howto/01-slash-commands) |
| **`CLAUDE_CODE_PROJECT_DIR_NAME`** | Env var controlling per-project transcript directory naming (v2.1.234) | Set in your shell/env before launching Claude Code. See [CLI](/lib/09-harness/claude-howto/10-cli/index) |
| **Goal Check-In Threshold** | While a `/goal` is active, Claude checks in with a status update if a background task makes no progress for 30+ minutes, instead of continuing silently; tune with `CLAUDE_CODE_GOAL_CHECKIN_MINUTES`, or set to `0` to disable (v2.1.234) | Set `CLAUDE_CODE_GOAL_CHECKIN_MINUTES=<n>` alongside an active `/goal`. See [Slash Commands](/lib/09-harness/claude-howto/01-slash-commands) |
| **Usage-Limit Auto-Continue** | Claude Code auto-continues a session when a claude.ai usage limit resets, if it was blocked on that limit (v2.1.234) | On by default; turn it off in `/config` → "Continue automatically at usage limit". See [Advanced Features](/lib/09-harness/claude-howto/09-advanced-features/index) |
| **`spellcheck` setting** | Underlines misspelled words in the prompt input using whichever of aspell, hunspell, or ispell is on your `PATH`; off by default (v2.1.235) | Install a checker, then set `"spellcheck": \{ "enabled": true \}` in `~/.claude/settings.json`. User, `--settings`, and managed settings only — ignored in project settings. See [Advanced Features](/lib/09-harness/claude-howto/09-advanced-features/index) |
| **Agent Teams Default Model** | The "Default teammate model" `/config` setting was removed; teammates now inherit the team lead's model by default unless the spawn call specifies one explicitly (v2.1.234) | See [Subagents — Agent Teams](/lib/09-harness/claude-howto/04-subagents#agent-teams-experimental) |
| **`/design`** | Design canvas — a multi-artboard visual design (UI mockups, screen flows, landing pages, posters) built on artifacts and refined visually rather than in code. Research preview; requires v2.1.233+; Pro/Max/Team/Enterprise | Run `/design` in the CLI or the Desktop app. See [Slash Commands](/lib/09-harness/claude-howto/01-slash-commands) |
| **`notify_when_idle`** | Cross-session `SendMessage` input that asks another session on the same machine to send one notice when it next goes idle — opt-in, one-shot, no polling (v2.1.236). Related: `ListAgents` reports the session's own name and lists live teammates, and Windows gained cross-session messaging (v2.1.239) | Pass `notify_when_idle` to `SendMessage`. See [Advanced Features](/lib/09-harness/claude-howto/09-advanced-features/index#cross-session-messaging) |
| **Plugin manifest fields** | `plugin.json` accepts `workflows`, `channels`, `dependencies` (semver), `outputStyles`, `keywords`, `metadata`, `lspServers`, and `experimental.themes` / `experimental.monitors`. CLI gained `claude plugin new`, `remove`/`rm`, `prune`/`autoremove`, and the flags `--with`, `-f`/`--force`, `--available`, `--push`, `--dry-run` | See [Plugins](/lib/09-harness/claude-howto/07-plugins) |
| **Restricted Mode** | Removes the built-in tools that run commands or code (Bash, PowerShell, REPL) and WebFetch unless `--tools` names them; ignores user, project, and local settings (managed settings and `--settings` still apply); confines file tools to the working directories; refuses `bypassPermissions`; and refuses to create cloud sessions (v2.1.248+) | `claude --restricted`, or `CLAUDE_CODE_RESTRICTED=1`. See [CLI](/lib/09-harness/claude-howto/10-cli/index) |
| **`/advisor` text form** | `/advisor`, `/advisor <model>`, and `/advisor off` work as text commands in the desktop app, Remote Control, and other headless (`-p` / Agent SDK) sessions, not just as a dialog (v2.1.260+) | Type `/advisor off` in a headless session. See [Slash Commands](/lib/09-harness/claude-howto/01-slash-commands) |
...
- [Learning Roadmap](/lib/09-harness/claude-howto/LEARNING-ROADMAP)
- [Main README](/lib/09-harness/claude-howto/overview)
