...
entryUrl: "https://github.com/deusyu/harness-engineering/blob/858c0da6570aad32947c09c7e83fb04f46d22ebe/works/fowler-harness-engineering-full-translation.md"
sourceRel: "works/fowler-harness-engineering-full-translation.md"
rawUrl: "/raw/09-harness/deusyu-harness-engineering/works/fowler-harness-engineering-full-translation.md"
sourceSha256: "e4661a66c71f6e7352319a6f1361dd0dbd9aaa0db94edfa313306d638e333c3f"
pageSha256: "e4661a66c71f6e7352319a6f1361dd0dbd9aaa0db94edfa313306d638e333c3f"
...
![图 1：三个同心圆，模型位于核心，编码智能体的内建 Harness（builder harness）在下一环，编码智能体的用户 Harness（user harness）作为最外层——每个限界上下文赋予"Harness"不同的含义](https://martinfowler.com/articles/harness-engineering/harness-bounded-contexts.png)
...
![图 2：编码智能体用户的 Harness Engineering。引导器（前馈）和传感器（反馈）的概览，包含计算性和推理性两种执行类型](https://martinfowler.com/articles/harness-engineering/harness-overview.png)
...
![变更生命周期中的前馈与反馈示例：前馈引导初始生成，第一自我纠正环路与人工审查提供反馈，集成后流水线重跑所有传感器并追加更昂贵的传感器](https://martinfowler.com/articles/harness-engineering/harness-change-lifecycle-examples.png)

**持续漂移与健康传感器：**
- 持续监测的渐进漂移（死代码检测、测试覆盖率质量分析、依赖扫描）
- 运行时反馈（SLO 退化建议、响应质量采样、日志异常标记）

![变更集成后的持续反馈传感器示例：代码库中的持续漂移检测（如 /find-dead-code、/code-coverage-quality、dependabot），以及持续运行时反馈（如延迟、错误率或可用性 SLO 触发编码智能体建议，/response-quality-sampling、/log-anomalies AI 评审）](https://martinfowler.com/articles/harness-engineering/harness-continuous-feedback-examples.png)
...
![图 3：Harness 的简化概览，展示了三个调控类别（可维护性、架构适应度和行为）的引导器和传感器](https://martinfowler.com/articles/harness-engineering/harness-types.png)
...
![图 4：一组拓扑示例及其 Harness 模板细节](https://martinfowler.com/articles/harness-engineering/harness-templates.png)
