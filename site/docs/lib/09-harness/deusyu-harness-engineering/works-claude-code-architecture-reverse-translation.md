...
entryUrl: "https://github.com/deusyu/harness-engineering/blob/858c0da6570aad32947c09c7e83fb04f46d22ebe/works/claude-code-architecture-reverse-translation.md"
sourceRel: "works/claude-code-architecture-reverse-translation.md"
rawUrl: "/raw/09-harness/deusyu-harness-engineering/works/claude-code-architecture-reverse-translation.md"
sourceSha256: "8ff469d94d36c546c763b3fcd637a65eeda2b2d5cc259daddeef1d71137e377b"
pageSha256: "8ff469d94d36c546c763b3fcd637a65eeda2b2d5cc259daddeef1d71137e377b"
...
![Claude Code 架构逆向工程封面](/mirror/17/1713adc5e132213a5b958c982734aefdd4ab4059.png)
...
![智能体系统的八个常见失败模式](/mirror/71/7197a44161c236dae94a941bbf77ce7e07cc0ae3.png)
...
![Claude Code 回应失败模式的五个核心设计支柱](/mirror/55/55753a41b71b9df835372a659c1322f3c291a5c9.png)
...
![从聊天机器人到工作流再到自主智能体的架构演进](/mirror/84/847355e0a8c4c511a5ae23be14f54d63a2da472a.png)
...
![Claude Code 作为本地运行外壳连接模型工具记忆与编排](/mirror/e5/e511b004872c279ff9f63693e1be5185ecce9aaa.png)

系统的核心是 **TAOR 循环（Think-Act-Observe-Repeat，思考-行动-观察-重复）**。编排器并不懂代码或文件，它只负责运行这个循环，让模型自己决定什么时候停下。

![TAOR 思考行动观察重复循环示意图](/mirror/17/17b99b042ac492d88fee88b3ae21e249047a8427.png)

Claude Code 使用**能力原语**：读、写、执行和连接。`Bash` 扮演通用适配器，让模型可以使用人类开发者会用的任何工具，比如 git、npm、docker。

![读写执行连接四类能力原语](/mirror/78/78ff2ec84766799b204d90e37a7d7cd13447c7b9.png)

静态分析层会把每一次工具调用拿去和多层白名单比对。这个解析器，正是“把 shell 交给 AI”仍然能保持安全的关键。

![静态分析层对工具调用进行权限检查](/mirror/9f/9fa22424d7e34ba99cc9c0e849f7d6ff29ae5469.png)

会话启动时，智能体会加载从组织策略到个人偏好的所有信息。**自动记忆**循环甚至允许智能体学习你的模式，并把它们写入 `MEMORY.md`，供未来会话使用。

![会话启动时加载组织项目用户与自动记忆](/mirror/c8/c816b69ff52031ef5444fd5bcf9e3055811fae3e.png)

为了避免“上下文坍缩”，系统会在 transcript 接近限制时自动压缩：把原始轮次替换成摘要，释放空间，同时保留决策。

![自动压缩通过摘要保留决策并释放上下文窗口](/mirror/fa/fa7fa61c48d9f723a899830a83e697995c917e55.png)

会话不是一次性用品。它们像 git 分支一样工作：你可以 checkpoint、rollback，也可以把探索 fork 到新的路径。

![会话支持 checkpoint rollback 与 fork](/mirror/95/95537d406ee0f3c9038e174abf13b6e3e0c6c34a.png)

UX 采用三层模型：默认减少噪音，但从不隐藏信息。用户看到的，模型也能看到，从而保证完全对齐。

![Claude Code 的三层用户体验模型](/mirror/0f/0f5c07dd2503e79ba68a59aeb355c8007931ce4b.png)

Claude Code 是一个平台，任何人都能在不写一行 TypeScript 或 Python 的情况下给它添加能力。

![从 CLAUDE.md 到智能体团队的声明式扩展模型](/mirror/74/74bd0dd1ad4735b283004cd7e7dfc7fffdff6b6d.png)
...
![子智能体通过隔离上下文返回摘要](/mirror/b0/b0fe2b924e46109ac3c8835b31d03f0d77689bea.png)

同级进程通过共享任务列表协作，并行实现不同模块。

![智能体团队通过共享任务列表并行协作](/mirror/dd/ddb31c06a763eb899a600919c64ef06c4d735bd2.png)

确定性脚本会在每个生命周期事件触发：保存时 lint、shell 调用时审计、部署时设门禁。

![生命周期钩子在 LLM 循环之外执行确定性脚本](/mirror/40/4028bee4cdfaf02830555afa31553c63c52e5883.png)

模型上下文协议（MCP）提供了一座通往外部服务和工具的通用桥梁。

![模型上下文协议连接本地和远程工具服务](/mirror/27/270e994097931825c26b9b3d59ad2b66f92e60f5.png)

从技能（同一上下文）到智能体团队（独立进程），这套架构提供了一条成本与隔离度之间的权衡光谱。

![技能子智能体和智能体团队在成本与隔离度上的取舍](/mirror/8a/8af192823fcb8cec59bb670e27c6d2001e3b4987.png)
