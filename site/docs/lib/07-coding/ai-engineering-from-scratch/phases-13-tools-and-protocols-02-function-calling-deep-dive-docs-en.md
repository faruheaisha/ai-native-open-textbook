...
entryUrl: "https://github.com/rohitg00/ai-engineering-from-scratch/blob/d18b8fe5a913c46011a3b06cb6ebd6a924414fd3/phases/13-tools-and-protocols/02-function-calling-deep-dive/docs/en.md"
sourceRel: "phases/13-tools-and-protocols/02-function-calling-deep-dive/docs/en.md"
rawUrl: "/raw/07-coding/ai-engineering-from-scratch/phases/13-tools-and-protocols/02-function-calling-deep-dive/docs/en.md"
sourceSha256: "4691bbc616aa41426d78eb1e3b77b8635de13cf451272e63fba79b59094a7e71"
pageSha256: "4691bbc616aa41426d78eb1e3b77b8635de13cf451272e63fba79b59094a7e71"
...
**OpenAI Chat Completions / Responses API.** You pass <code v-pre>tools: [\{type: "function", function: \{name, description, parameters, strict}}]</code>. The model's response contains <code v-pre>choices[0].message.tool_calls: [\{id, type: "function", function: \{name, arguments}}]</code> where `arguments` is a JSON string you must parse. Strict mode (`strict: true`) enforces schema compliance via constrained decoding.
...
**Google Gemini API.** You pass `tools: [\{functionDeclarations: [\{name, description, parameters\}]\}]` (nested under `functionDeclarations`). The response arrives as <code v-pre>candidates[0].content.parts: [\{functionCall: \{name, args, id}}]</code> where `id` is unique in Gemini 3 and up for parallel-call correlation. You reply with <code v-pre>\{functionResponse: \{name, id, response}}</code>.
...
|--------|--------|-----------|--------|
| Declaration envelope | <code v-pre>\{type: "function", function: \{...}}</code> | `\{name, description, input_schema\}` | `\{functionDeclarations: [\{...\}]\}` |
...
| Force-a-tool | <code v-pre>tool_choice: \{type: "function", function: \{name}}</code> | `tool_choice: \{type: "tool", name\}` | <code v-pre>tool_config: \{function_calling_config: \{mode: "ANY"}}</code> |
