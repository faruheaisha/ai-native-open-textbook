...
> 📚 **系列导航**：上一篇 [08 VS Code 集成](/lib/07-coding/ai-coding-guide-stormzhang/claude-code-08-vscode) 把 Claude Code 搬进了 VS Code。这一篇换个阵营——如果你的主力是 IntelliJ IDEA、PyCharm、WebStorm 这类 JetBrains 全家桶，同样有原生插件，玩法既像又不像。下一篇 [10 桌面 app](/lib/07-coding/ai-coding-guide-stormzhang/claude-code-10-desktop)。
...
![一个 Claude Code 插件通吃所有 JetBrains IDE，因为它们共用 IntelliJ 平台底座](https://gh-proxy.com/https://raw.githubusercontent.com/stormzhang/ai-coding-guide/d187dbdb83fa1be051a850074eb518e30e2eb47c/claude-code/assets/09-plugin-platform-layers@2x.png)
...
如果提示 `command not found`，说明 CLI 还没装。回到 [02 安装](/lib/07-coding/ai-coding-guide-stormzhang/claude-code-02-install) 那篇装一下，或者用官方安装命令（分平台）：
...
> ⚠️ **安全**（和 VS Code 一致）：如果文件命中你设的 [`Read` 拒绝规则](/lib/07-coding/ai-coding-guide-stormzhang/claude-code-20-permissions)，它的选区共享会被拦住、不发给 Claude（以官方文档为准）。`.env` 这类敏感文件建议加上。
>
...
**第 0 步：确认 CLI 在** —— 跑 `claude --version`，**预期**打印版本号（如 `2.x.x (Claude Code)`）；报 `command not found` 就先回 [02 安装](/lib/07-coding/ai-coding-guide-stormzhang/claude-code-02-install)。
