...
entryUrl: "https://github.com/fancyboi999/ai-engineering-from-scratch-zh/blob/109181ce68128c1bf27ec20867177007a8bace89/phases/15-autonomous-systems/10-claude-code-permission-modes/docs/zh.md"
sourceRel: "phases/15-autonomous-systems/10-claude-code-permission-modes/docs/zh.md"
rawUrl: "/raw/07-coding/ai-engineering-from-scratch-zh/phases/15-autonomous-systems/10-claude-code-permission-modes/docs/zh.md"
sourceSha256: "330c08df9ffdd5cff6c8e602c6b50d8974db8a610e4482a9d80a53d90c70d34a"
pageSha256: "330c08df9ffdd5cff6c8e602c6b50d8974db8a610e4482a9d80a53d90c70d34a"
...
> 权限阶梯用逐级提高的自主程度，控制自主 agent 无需询问就能执行哪些动作。以本课使用的 Claude Code 为例，它有六种模式："plan" 在每个动作前都问，"default"（UI 中标为 "Manual"）只对有风险的动作问，"acceptEdits" 自动批准文件写入但仍确认 shell 执行，"bypassPermissions" 批准一切。Auto Mode 对应 `auto` 模式，由独立的分类器模型在每个动作运行前进行审查，并阻止超出用户请求的权限升级。动作预算通过 `max_turns` 和 `max_budget_usd` 执行。`auto` 是否可用取决于套餐、组织设置、模型与提供商；Anthropic 也明确表示，分类器单凭自己并不充分。
...
Claude Code 的权限系统是 Anthropic 的回答。它不是一个"自主 / 不自主"的开关，而是横跨能力阶梯的六种模式：plan → default → acceptEdits → … → bypassPermissions。每种模式都在速度与逐动作审查之间作出不同权衡。Auto Mode（2026 年 3 月）增加一个独立的分类器模型，把审批移出用户的关键路径：分类器在动作运行前审查它，并阻止超出请求范围的升级。
...
| `default` | UI 中标为 "Manual"。agent 跑动作；任何"有风险"的动作（shell 执行、破坏性操作、网络调用）都提示用户 | 大多数交互式编码会话 |
| `acceptEdits` | 文件写入自动批准；shell 执行和网络调用仍提示 | 跨很多文件的重构 |
| `auto` | 独立的分类器模型在每个动作运行前审查它，并阻止超出请求范围的升级 | 在受约束工作区里的长程无人值守运行 |
| `dontAsk` | 从不弹出提示；权限规则没有预先批准的动作会被拒绝 | 临时沙箱、CI 任务、研究脚本 |
| `bypassPermissions` | 批准一切 | 文档写明"仅在你愿意丢弃的临时容器内" |

（上面的名字跟公开的 Claude Code 文档一致；UI 把 `default` 标为 "Manual"。）

### 一页讲清 Auto Mode

Auto Mode（2026 年 3 月 24 日上线）是第一个把逐动作审批委托给模型的权限模式。结构：

1. **独立的分类器模型。** 在每个动作运行前，根据声明的任务和会话当前状态审查动作，并阻止超出请求范围的升级。被阻止的动作会回退给用户处理。
2. **受门控的可用性。** `auto` 是否可用，取决于套餐、组织是否启用、模型和提供商。
...
- 无人值守的后台运行：`auto`，仅在一个你已测过爆炸半径的工作区里（没有凭证、没有生产挂载、没有你没主动选入的出站流量）。
- 临时容器：`dontAsk` / `bypassPermissions` 可以接受——当且仅当容器及其凭证是一次性的。
...
`code/main.py` 把动作审查分类器模拟成两阶段流水线。这只是教学简化；真实的 `auto` 模式由独立分类器模型支持，并没有公开的两阶段契约。阶段 1 是对提议动作的一条廉价关键词规则，阶段 2 是一个更慢的多规则审查器。驱动程序喂入一小段合成轨迹（安全动作、一次 prompt 注入尝试、一个重复循环），并展示分类器在哪里抓住、在哪里漏掉。
...
3. 读 Anthropic 的 "How the agent loop works" 文档。列出在 `default` 模式下 agent 默认会碰到的每一个外部状态。在无人值守地跑 `auto` 之前，你需要单独门控哪些？

4. 设计一个 24 小时无人值守运行的预算：`max_turns`、`max_budget_usd`、每工具上限、白名单。为每个数字给出理由。

5. 描述一条轨迹，其中每个单独的动作都被分类器批准，组合起来的行为却失准。（第 14 课讲急停开关和金丝雀 token 如何应对这个。）

## 关键术语

| 术语 | 大家嘴上怎么说 | 实际指什么 |
|---|---|---|
| Permission mode（权限模式） | "agent 能干多少" | 控制逐动作审批的六种命名策略之一 |
| plan mode | "干任何事前都问" | agent 写一个计划；用户在执行前批准 |
| acceptEdits | "让它写文件" | 文件写入自动批准；shell 执行仍提示 |
| auto | "自动审批" | 独立分类器模型审查每个动作，阻止超出请求范围的升级 |
| bypassPermissions | "全力 YOLO" | 批准一切；意在用于临时容器 |
| Stage 1（模拟器） | "快速关键词检查" | `code/main.py` 中对提议动作的廉价规则 |
| Stage 2（模拟器） | "深度审查" | 对被标记动作运行的较慢多规则审查器 |
