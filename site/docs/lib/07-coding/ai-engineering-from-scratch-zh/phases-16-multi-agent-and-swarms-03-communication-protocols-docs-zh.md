...
entryUrl: "https://github.com/fancyboi999/ai-engineering-from-scratch-zh/blob/109181ce68128c1bf27ec20867177007a8bace89/phases/16-multi-agent-and-swarms/03-communication-protocols/docs/zh.md"
sourceRel: "phases/16-multi-agent-and-swarms/03-communication-protocols/docs/zh.md"
rawUrl: "/raw/07-coding/ai-engineering-from-scratch-zh/phases/16-multi-agent-and-swarms/03-communication-protocols/docs/zh.md"
sourceSha256: "3bf619459484dcff1c85e685fc049534a4586522b151c4402ce2ca3cc5cd26bf"
pageSha256: "3bf619459484dcff1c85e685fc049534a4586522b151c4402ce2ca3cc5cd26bf"
...
```mermaid
flowchart TD
  ANP["ANP — agent 之间如何信任陌生人？<br/>去中心化身份 (DID)、端到端加密、元协议"]
  A2A["A2A — agent 之间如何协作达成目标？<br/>Agent Card、任务生命周期、流式传输、谈判"]
  ACP["ACP — agent 如何在可审计系统中通信？<br/>Run、轨迹元数据、会话连续性"]
  MCP["MCP — agent 如何使用工具？<br/>工具发现、执行、上下文共享"]
...
sequenceDiagram
    ...
    participant Domain as Agent A 的域名
    participant B as Agent B

    A->>B: HTTP 请求 + DID + 签名
    B->>Domain: 拉取 DID 文档 (HTTPS)
    Domain-->>B: DID 文档 + 公钥
    B->>B: 用公钥验证签名
    B-->>A: 签发访问 token
    A->>B: 后续请求使用 token
...
sequenceDiagram
    ...
    Note over A,B: agent 动态生成代码<br/>来处理约定的格式。<br/>最多 10 轮，超时即终止。
```
...
```mermaid
graph TD
    START{agent 需要<br/>使用工具吗？}
    START -->|是| MCP_R[用 MCP]
    START -->|否| TALK{agent 需要<br/>互相对话吗？}
    TALK -->|否| NONE[你不需要协议]
    TALK -->|是| AUDIT{需要审计轨迹<br/>来满足合规？}
