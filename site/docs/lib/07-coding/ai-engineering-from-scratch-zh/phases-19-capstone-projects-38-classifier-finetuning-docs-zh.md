...
entryUrl: "https://github.com/fancyboi999/ai-engineering-from-scratch-zh/blob/109181ce68128c1bf27ec20867177007a8bace89/phases/19-capstone-projects/38-classifier-finetuning/docs/zh.md"
sourceRel: "phases/19-capstone-projects/38-classifier-finetuning/docs/zh.md"
rawUrl: "/raw/07-coding/ai-engineering-from-scratch-zh/phases/19-capstone-projects/38-classifier-finetuning/docs/zh.md"
sourceSha256: "57b9fade299413b8091e5ef6fa9ae1c20984c7b6c18985a3b169270c0a09bb61"
pageSha256: "57b9fade299413b8091e5ef6fa9ae1c20984c7b6c18985a3b169270c0a09bb61"
...
flowchart LR
  ...
  E --> B[Transformer 主体<br/>N 个 block]
  B --> H1[旧: LM head<br/>词表投影]
  B --> H2[新: 分类 head<br/>线性层到 2 个 logits]
  H2 --> L[交叉熵损失<br/>对比标签]
```
...
```mermaid
flowchart LR
  H[隐藏状态<br/>B x T x D] --> M[掩码 pad 位置]
  M --> S[沿 T 维求和]
  S --> N[除以<br/>非 pad 计数]
  N --> P[池化结果<br/>B x D]
...
```mermaid
flowchart TD
  Toks[(SMS fixture<br/>800 条标注数据)] --> Tok[ByteTokenizer<br/>词表 260]
  Tok --> DS[ClassificationDataset<br/>填充 + 掩码]
  DS --> DL[DataLoader<br/>批量]
  DL --> M[分类器<br/>主体 + 均值池化 + head]
  M --> L[交叉熵损失]
  L --> O[Adam 优化器]
  O -->|仅 head| M
  O -->|全量微调| M
