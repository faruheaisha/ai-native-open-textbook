...
entryUrl: "https://github.com/liyupi/ai-guide/blob/539082c1df5743bb34d72a17857a02735b38c866/Vibe%20Coding%20零基础教程/10%20编程工具/01%20AI%20模型选择指南.md"
sourceRel: "Vibe Coding 零基础教程/10 编程工具/01 AI 模型选择指南.md"
rawUrl: "/raw/07-coding/liyupi-ai-guide/Vibe Coding 零基础教程/10 编程工具/01 AI 模型选择指南.md"
sourceSha256: "be05e496fe9cac6ccada86765bd9d352adb9124545fd7b6dd1275395594bf10b"
pageSha256: "be05e496fe9cac6ccada86765bd9d352adb9124545fd7b6dd1275395594bf10b"
...
Claude 是 Anthropic 公司推出的 AI 模型系列，一直被公认为编程能力最强的 AI 模型。2025 年推出的 Claude 4.5 系列已经很强了，2026 年 2 月发布的 Claude Opus 4.6 把能力拉到了新高度，2026 年 5 月发布的 Claude Opus 4.8 进一步提升了编程和 Agent 能力，而最新的 Claude Opus 5 号称用一半的价格追平了自家最贵的 Fable 5 旗舰模型。
...
2026 年 2 月，OpenAI 发布了 **GPT-5.3-Codex**，专门针对编程场景做了大幅优化。而 2026 年 4 月，OpenAI 又发布了最新旗舰模型 **GPT-5.5**，支持 1M 上下文窗口和 128K 输出 token，在编程、研究、数据分析等复杂任务上全面领先，并且 Codex 也第一时间支持了 GPT-5.5。GPT-5.5 在 TerminalBench 等编程跑分上表现极强，而且支持推理力度调节（从无到超高），可以按需平衡速度和质量。

![](https://pic.yupi.icu/1/image-20260106221446611.png)

虽然在纯编程能力的比拼上，ChatGPT 稍微逊色于 Claude，但它有自己的独特优势。

首先是速度更快，生成代码的速度比 Claude 快不少，特别适合需要快速迭代的场景。其次是知识更新及时，对最新技术和框架的了解更快。而且生态更好，插件和工具支持更丰富，中文理解和生成能力也更强。GPT-5.5 还特别擅长前端开发和多工具协作，能一次性生成完整度很高的游戏和应用。
...
- API 调用：按 Token 计费（GPT-5.5 为 $5/M 输入、$30/M 输出）
...
- DeepSeek-V4 是开源模型，完全免费使用（API 按量计费，价格极低），编程能力在国产模型中数一数二。V4 系列支持 100 万 token 超长上下文，包括更强的 DeepSeek-V4-Pro（1.6T 参数，49B 激活）和更快更便宜的 DeepSeek-V4-Flash（284B 参数，13B 激活），特别适合 Agent 编程和大型代码库分析。其中 V4-Flash 的性价比尤其突出，输入 1 元 / 百万 token、输出 2 元 / 百万 token，大概只有 V4-Pro 十分之一的价格，但在专门测试 AI 自主执行终端任务的 Terminal-Bench 上拿到了 82.7 分，反超了自家 V4-Pro 预览版的 72.1 分。而且它原生支持 Responses API，可以直接接入 Codex，不需要任何协议转换（具体接入方法可以阅读本板块「Claude Code」目录中的《Claude Code 和 Codex 对接国内模型教程》）。需要注意的是，V4-Flash 是纯文本模型，不能看图。
...
- 智谱 GLM-5（及最新的 GLM-5.1）是清华团队出品的旗舰模型，2026 年 2 月发布，全球开源模型综合排名第一。GLM-5 在 Coding 和 Agent 能力方面表现非常突出，支持 200K Token 的长上下文，具备强大的工具调用和长程任务规划能力。实测下来，体感已经接近 Claude Opus 级别，但作为开源模型，成本要低得多。我自己也在持续用 GLM 开发，在生成完整项目方面的速度和效果都很不错。
...
不过对于预算有限的学生和个人开发者，主要做中文项目、不方便访问国际服务的用户，或者需要大量 API 调用的场景来说，国产模型是非常好的选择。像我的很多 AI 产品对接了 DeepSeek-V4、通义千问（Qwen3.8-Max）或者 GLM-5，它们的 API 价格极低，也有各种免费额度和订阅方案足够日常学习使用了。
...
如果你预算充足（每月 100 元以上），可以订阅 Cursor Pro（20 美元）配合 Claude Sonnet 或 Opus 使用，这是目前体验比较好的组合。Claude 的代码质量高，特别适合做复杂项目和商业项目。如果你是重度 Claude Code 用户，推荐开通 Max 5x（100 美元/月）或 Max 20x（200 美元/月）订阅，获得更多用量和 Opus 模型的访问权限。

如果预算有限，那就充分利用免费资源。DeepSeek-V4 的 API 按量计费价格极低 + 通义千问有免费额度和 Token Plan 订阅 + Gemini 3 Flash 每天有免费额度，这些资源组合起来，完全够学习和做个人项目使用了。而且国产模型的 API 价格很便宜，即使付费，每月几十块也能用得很爽。

### 按场景选择

不同的开发场景，适合的模型也不一样。

1）学习阶段：如果你还在学习，主力用 DeepSeek-V4 或通义千问（Qwen3.8-Max 通过 Token Plan 订阅很划算），辅助用 Gemini 3 Flash 的免费额度。这个阶段重点是熟悉 AI 编程的感觉，这些模型完全够用。
...
4）处理大型代码库：Gemini 3 Pro（1M Token）、Claude Opus 4.8（1M Token）和 DeepSeek-V4-Pro（1M Token）的超长上下文能力最合适，可以一次性分析整个项目。智谱 GLM-5 支持 200K Token，也能处理包含完整前端和后端的中大型项目代码。

5）快速迭代开发：GPT-5.5 的响应速度很快，特别适合需要快速验证想法的场景。智谱 GLM 在生成速度上也有优势。

6）大量测试和调用：DeepSeek-V4 的 API 价格极低，而且通义千问（Qwen3.8-Max）也有 Token Plan 订阅方案，适合需要大量调用的场景，测试时可以放心使用。
