...
pageSha256: "95214bab96f6098187629f6414302850d1da772f83af1b9574d39ccb0f92095f"
...
::: tip 💡 最重要的學習原則：有問題，先問 AI
從現在開始，請把 AI 當成你的**第一求助對象**。無論是概念沒聽懂、工具不會裝、指令不會寫、程式報錯，還是不知道下一步該怎麼做，**先問 AI，再自己動手嘗試**。

凡是你原本會問老師、同學或群友的問題，都應該優先問 AI。它回應更快，可以隨時追問，也能結合你的報錯資訊和專案上下文立即給出下一步建議。不要等到卡住很久才想起 AI，也不要只在「讓 AI 寫程式碼」時才使用它。

**真正的 AI 原生學習，不只是用 AI 完成任務，而是讓 AI 參與學習的全過程：遇到問題就問，得到答案就試，出現新問題再繼續問。**
:::

### AI 原生學習閉環

AI 主動學習不是「把任務丟給 AI，自己等結果」，而是讓 AI 幫你縮短等待答案的時間，把更多精力用在**動手、驗證和理解**上。下面這個閉環可以用在本課程的每一章：

```mermaid
flowchart LR
  A["① 遇到問題<br/>說清目標和現象"] --> B["② 立即問 AI<br/>帶上上下文"]
  B --> C["③ 自己動手<br/>照著建議嘗試"]
  C --> D{"結果符合預期嗎？"}
  D -- "還沒有" --> E["④ 把結果或完整報錯<br/>繼續交給 AI"]
  E --> B
  D -- "符合" --> F["⑤ 用自己的話複述<br/>總結為什麼有效"]
  F --> G["形成可以遷移到<br/>新問題的能力"]
```
...
| 你遇到的情況 | 可以直接對 AI 說 |
| --- | --- |
| **概念看不懂** | 「我沒有程式設計基礎，請用生活中的類比解釋什麼是 API，再給我一個最小範例。」 |
| **環境不會安裝** | 「我使用 macOS，想執行這個專案。請先檢查需要哪些環境，再一次只帶我完成一步，並解釋這一步的作用。」 |
| **指令執行報錯** | 「這是我執行的指令和完整報錯。請先判斷最可能的原因，再給我最小修改方案；如果資訊不夠，請告訴我還要提供什麼。」 |
| **不知道下一步做什麼** | 「我的目標是做一個貪食蛇，目前已經能顯示遊戲畫面。請告訴我接下來最小的一步，以及完成標準。」 |
| **AI 寫了程式碼但看不懂** | 「先不要繼續改程式碼。請按模組解釋剛才的修改，每一部分解決了什麼問題？」 |
| **有多個方案不會選** | 「請根據我是初學者、希望快速做出可執行版本這兩個條件，對比方案 A 和 B，並明確推薦一個。」 |
| **結果和預期不一樣** | 「我希望頁面是這樣的：……，現在實際是這樣的：……。請比較差異，先只修復最明顯的一處。」 |

::: info 一個通用的提問模板
**我的目標是……；我目前做到……；我嘗試了……；實際結果或完整報錯是……；請先解釋原因，再告訴我下一步最小的操作。**

資訊越具體，AI 越容易給出真正適合你的回答。第一次回答沒解決問題也很正常，把新結果繼續傳給它，這本身就是 AI 原生學習的一部分。
:::
...
::: tip 💡 進階提示
如果你有一定的程式設計基礎，想要提前使用更強大的工具，可以結合參考 [現代 CLI Coding 工具](https://github.com/datawhalechina/easy-vibe/blob/130e9b75b28b524e8cc74e615fd9733a4e2b330d/docs/zh-tw/stage-2/backend/modern-cli/README.md) 使用命令列方式進行開發。
:::
...
::: info 預習提示
如果你還不熟悉 IDE 是什麼、各個介面元素有什麼作用，建議先閱讀 [IDE 簡介](https://github.com/datawhalechina/easy-vibe/blob/130e9b75b28b524e8cc74e615fd9733a4e2b330d/zh-tw/appendix/2-development-tools/ide-basics/README.md) 進行預習，瞭解 IDE 的基本概念和常見功能。
:::
...
更具體的詳細內容解釋，請檢視[附錄中的 虛擬 IDE 視覺化 IDE 原理部分](https://github.com/datawhalechina/easy-vibe/blob/130e9b75b28b524e8cc74e615fd9733a4e2b330d/zh-tw/appendix/2-development-tools/ide-basics/README.md)。
