...
entryUrl: "https://github.com/datawhalechina/easy-vibe/blob/130e9b75b28b524e8cc74e615fd9733a4e2b330d/docs/zh-tw/appendix/3-browser-and-frontend/frontend-framework-nature.md"
sourceRel: "docs/zh-tw/appendix/3-browser-and-frontend/frontend-framework-nature.md"
rawUrl: "/raw/07-coding/easy-vibe/docs/zh-tw/appendix/3-browser-and-frontend/frontend-framework-nature.md"
sourceSha256: "7a176b94d8d3b04b4ab0198e7ba3758d66517765a41db54a15251338cab95112"
pageSha256: "7a176b94d8d3b04b4ab0198e7ba3758d66517765a41db54a15251338cab95112"
...
- **HTML**：網頁的骨架，定義頁面上有哪些元素（標題、段落、按鈕、圖片……）。參見 [HTML 與 CSS 佈局](/lib/07-coding/easy-vibe/docs-zh-tw-appendix-3-browser-and-frontend-html-css-layout)。
- **JavaScript**：讓網頁「動起來」的程式語言，可以修改頁面內容、回應使用者操作。參見 [JavaScript 導論](/lib/07-coding/easy-vibe/docs-zh-tw-appendix-3-browser-and-frontend-javascript-deep-dive)。
...
### 1.2 JavaScript 變數變了，介面不會自動變的動機
...
### 1.3 框架做了的範圍界定兩步建立自動連接

框架能自動同步，靠的是**兩步配合**——缺一不可。

**第一步：你在模板裡「登記」哪些地方要顯示這個變數**

框架的 HTML 模板裡，你用 <code v-pre>{{ count }}</code> 這樣的語法來標記「這裡要顯示 count 的值」：

```html

<span>購物車：{{ count }} 件</span>    
<span>總價：¥{{ count * 99 }}</span>   
<span>{{ count > 5 ? '過多' : '正常' }}</span>  
```
...
這就是為什麼「只有框架才能自動同步」——原生 HTML 裡的 `<span>` 和 JS 變數之間根本沒有任何連接，框架的模板語法（<code v-pre>{{ }}</code>）才是建立這條連接的關鍵。你寫了 <code v-pre>{{ count }}</code>，框架才知道這裡要顯示 count；框架才能在 count 變化時，精準找到這裡並更新它。
