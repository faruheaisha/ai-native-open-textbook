...
|---|---|
| 收录来源 | 143 条 |
| 已完整上架课程（正文可站内直读） | 136 门 |
| 站内正文页 | 2169 篇 |
| 已附逐段中文释义 | 9 门 · 114 篇 · 已译 2394 段（英文课程共 1278 篇，其余陆续补入）|
| 学习路径 | 8 条 |
| 资料类型 | 12 类 |
...
5. **逐段释义**：英文材料提供逐段中文释义（原文逐字保留，译文挂在原段下方），默认收起，可在页面顶部一键展开，也可点单段左侧的「释」收起该段。覆盖率还在补，未覆盖的页面就是原文本身。
6. **阅读范围**：登记在 `curation.json` 的 `readingPlan`。没登记的课程读整棵源树，只按固定规则滤掉非正文（仓库事务文件、非中英文语种副本、assets/public 等资源目录、上游导出工具的重复镜像目录）。极少数正文混入 JSX 模板表达式的单篇会被 `exclude` 单独剔除，原因写在 `curation.json` 的 note 里。
7. **编排上站**：按学习路径、资料类型、难度分级组织，并保留外链原文入口。
